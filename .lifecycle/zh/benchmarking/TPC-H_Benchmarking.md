...
TPC-H 是由事务处理性能委员会（TPC）开发的决策支持基准测试。它包含了一系列面向业务的即席查询和并发数据修改操作。TPC-H 可以用来构建基于真实生产环境的模型，以模拟销售系统的数据仓库。此测试涉及 8 个表，数据量范围从 1 GB 到 3 TB。共测试了 22 个查询，其主要性能指标为每个查询的响应时间，即从提交查询到返回结果的持续时间。

## 1. 测试结论

我们对 TPC-H 100 GB 数据集进行了 22 个查询的测试。下图展示了测试结果的对比。
![comparison](../assets/7.2-1.png)

在测试中，StarRocks 从其本地存储和 Hive 外部表中查询数据。StarRocks 和 Trino 从 Hive 外部表中查询相同的数据副本。数据使用 ZLIB 压缩并以 ORC 格式存储。

StarRocks 从本地存储查询数据的延迟为 21 秒，从 Hive 外部表查询的延迟为 92 秒，而 Trino 从 Hive 外部表查询的延迟**长达 307 秒**。
...
测试使用了 TPC-H 100 GB 数据集。

|表|记录|StarRocks 上压缩后的数据大小|
|---|---|---|
|客户|1500万|2.4 GB|
|lineitem|6 亿|75 GB|
|国家|25|2.2 KB|
...
|查询|StarRocks-native (ms)|StarRocks-Hive external (ms)|Trino (ms)|
|---|---|---|---|
|q1|1291|5349|14350|
|q2|257|940|4075|
|q3|690|2585|7615|
...
### 4.1 查询 StarRocks 本地表

#### 4.1.1 生成数据

1. 下载 tpch-poc 工具包并进行编译。
   ...
   相关工具存放于 bin 目录，可以在 home 目录执行操作。
...
2. 运行以下脚本创建表：
   ...
   表已创建，默认的桶数已指定。您可以删除之前的表，并根据集群规模和节点配置重新规划桶数，以获得更佳的测试结果。
...
此步骤为可选。我们建议收集完整的统计信息以便获得更佳的测试结果。连接至 StarRocks 集群后，执行以下命令：
...
1. 在 Hive 中创建外部表，表格格式为 CSV。将 CSV 测试数据上传至 HDFS 上的数据存储目录。在本例中，Hive 外部表的 HDFS 数据存储路径为 /user/tmp/csv/。
...
2. 将每个 Hive 外部表中的 CSV 数据转换为 ORC 格式的 Hive 表。这有助于在 StarRocks 和 Trino 中创建 ORC 格式的 Hive 外部表，以便进行后续测试。
