...
TPC-DS 是由交易处理性能委员会（TPC）开发的决策支持基准测试。相较于 TPC-H，它使用了更全面的测试数据集和更复杂的 SQL 查询。

TPC-DS 对决策支持系统的多个普遍适用方面进行了建模，包括查询和数据维护。TPC-DS 旨在提供一个全面且逼真的工作负载，用于测试和评估零售环境中数据库系统的性能。TPC-DS 基准测试模拟了零售企业三个销售渠道（实体店、互联网和目录）的销售和退货数据。除了创建销售和退货数据模型的表格外，它还包含了一个简易的库存系统和促销系统。

该基准测试对 24 个表进行了 99 个复杂 SQL 查询的测试，这些表的数据大小范围从 1 GB 到 3 GB 不等。主要性能指标是每个查询的响应时间，即从提交查询到返回结果所经历的时间。

## 1. 测试结论

我们对 TPC-DS 100 GB 数据集进行了 99 个查询的测试。以下图显示了测试结果。
![tpc-ds](../assets/tpc-ds.png)

在测试中，StarRocks 既从其原生存储查询数据，也从 Hive 外部表查询数据。StarRocks 和 Trino 均从 Hive 外部表查询相同的数据副本。数据经过 LZ4 压缩并以 Parquet 格式存储。

StarRocks 从原生存储查询数据的延迟为 **174 秒**，查询 Hive 外部表的延迟为 **239 秒**，启用数据缓存（Data Cache）功能查询 Hive 外部表的延迟为 **176 秒**，而 Trino 查询 Hive 外部表的延迟为 **892 秒**。
...
|机器|云主机4台|
|---|---|
|CPU|8 核 Intel(R) Xeon(R) Platinum 8269CY CPU @ 2.50GHz|
|内存|32 GB|
|网络带宽|5 Gbit/s|
|磁盘|SSD|
...
StarRocks 和 Trino 都部署在配置相同的机器上。StarRocks 部署了 1 个前端（FE）和 3 个后端（BE）。Trino 部署了 1 个协调器和 3 个工作节点。

- 内核版本：Linux 3.10.0-1127.13.1.el7.x86_64
- 操作系统版本：CentOS Linux 7.8.2003 发布版
- 软件版本：StarRocks 社区版 3.1、Trino-419、Hive-3.1.2

> StarRocks 的前端（FE）可以单独部署，也可以与后端（BE）混合部署，这不会影响测试结果。
...
> **注意**
- 以下表格中查询延迟的单位为毫秒（ms）。
- StarRocks-3.0.5-native 表示 StarRocks 原生存储，StarRocks-3.0-Hive-external 表示 StarRocks 通过 Hive Catalog 查询 Hive 外部表，StarRocks-3.0-Hive-external-Cache 表示 StarRocks 通过 Hive Catalog 启用数据缓存查询 Hive 外部表。
- StarRocks 启用了聚合下推（SET global cbo_push_down_aggregate_mode = 0）。

|查询|StarRocks-3.0.5-native|StarRocks-3.0-Hive 外部|StarRocks-3.0-Hive 外部缓存|Trino-419|
|---|---|---|---|---|
|总和|174157|238590|175970|891892|
|第一季度|274|780|254|1681|
|第二季度|338|676|397|10200|
|第三季度|455|1156|628|3156|
|第四季度|16180|13229|12623|48176|
...
下载 tpcds-toolkit 工具包并生成标准 TPC-DS 测试数据集，比例因子为 100。
...
修改配置文件 conf/starrocks.conf，指定集群地址。注意 mysql_host 和 mysql_port 设置。
...
在 Hive 中创建外部表，其存储格式为 Parquet，压缩格式为 LZ4。有关详细的 [CREATE TABLE](#53-create-a-hive-external-tableparquet) 语句，请参见创建 Parquet 格式的 Hive 外部表。StarRocks 和 Trino 从这些外部表中查询数据。

#### 4.2.2 加载数据

将[4.1.1](#411-generate-data)步骤中生成的CSV数据加载到Hive指定的HDFS路径。本例中使用`/user/tmp/csv/`作为HDFS路径。在Hive中创建CSV格式的Hive外部表，并将表存储在`/user/tmp/csv/`目录中。有关详细的CREATE TABLE语句，请参见[创建CSV格式的Hive外部表](#54-create-a-hive-external-tablecsv)。

使用 INSERT INTO 语句将数据从 CSV 外部表加载到 Parquet 外部表中。这将生成经过 LZ4 压缩的 Parquet 格式数据。
...
StarRocks v3.0 不支持从外部表收集统计数据。因此，我们使用 Hive 来收集列级别的统计信息。
...
StarRocks 使用[Hive catalog](../data_source/catalog/hive_catalog.md)来查询Hive外部表。

如果在测试期间为 StarRocks 启用了[数据缓存](../data_source/data_cache.md)功能，建议对数据缓存进行以下配置。
...
有关 99 个 SQL 查询的更多信息，请参见 [TPC-DS SQL](./tpc_ds_99_sql.md) 文档。
