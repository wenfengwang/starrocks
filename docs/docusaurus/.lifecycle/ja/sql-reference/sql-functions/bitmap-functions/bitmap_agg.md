...
列内の値（NULLを除く）をビットマップに集計します（複数の行を1行にまとめます）。
...
`col`: 集計したい列。BOOLEAN、TINYINT、SMALLINT、INT、BIGINT、およびLARGEINTに評価される必要があります。
...
例2：列`c2`の値をビットマップに集計します（NULLは無視されます）。
...
例3：列`c6`の値をビットマップに集計します（値の範囲を超える最後の2つの値は無視されます）。
