...
* Stream Load：对于存储在文本文件中的 JSON 数据，使用 Stream Load 导入。
* Routine Load：对于 Kafka 中的 JSON 数据，使用 Routine Load 导入。
...
`format: json` 参数用于指定导入数据的格式。`jsonpaths` 用于指定数据导入路径。

相关参数：

* jsonpaths：选择每列的 JSON 路径
* json_root：选择 JSON 开始解析的列
* strip_outer_array：裁剪最外层的数组字段
* strict_mode：导入时对列类型转换进行严格过滤

当 JSON 数据模式与 StarRocks 数据模式不完全相同时，需要修改 `jsonpaths`。
...
导入时执行 k1 乘以 100 的 ETL 操作，并通过 `jsonpaths` 将列与原始数据匹配。
...
对于缺失的列，如果列定义允许为空，则会添加 `NULL`，或者可以通过 `ifnull` 添加默认值。
...
1. 如果一条消息包含数组格式的多行数据，所有行都会被导入，Kafka 的 offset 只会增加 1。
2. 如果一个数组格式的 JSON 代表多行数据，但由于 JSON 格式错误导致解析失败，则错误行只会增加 1（鉴于解析失败，StarRocks 实际上无法确定它包含多少行数据，只能将错误数据记录为一行）。

### 使用 Canal 从 MySQL 同步数据到 StarRocks 并实现增量同步 binlog

[Canal](https://github.com/alibaba/canal) 是阿里巴巴开源的 MySQL binlog 同步工具，通过它我们可以将 MySQL 数据同步到 Kafka。数据在 Kafka 中以 JSON 格式生成。这里演示如何使用 Routine Load 同步 Kafka 中的数据，实现与 MySQL 的增量数据同步。

* 在 MySQL 中，我们有一个数据表，其建表语句如下。
...
* 前提条件：确保 MySQL 启用了 binlog 并且格式为 ROW。

```bash
[mysqld]
log-bin=mysql-bin # 启用 binlog
binlog-format=ROW # 选择 ROW 模式
server_id=1 # 需要定义 MySQL 复制的 server_id，且不与 Canal 的 slaveId 重复
```

* 创建账户并授予 MySQL 从服务器权限：
...
# username/password，需要更改为您自己的数据库信息
...
canal.mq.retries = 0
# 在 flagMessage 模式下，此值可以增加，但不要超过 MQ 消息的最大大小。
canal.mq.batchSize = 16384
canal.mq.maxRequestSize = 1048576
# 在 flatMessage 模式下，请将此值改为更大的值，建议为 50-200。
canal.mq.lingerMs = 1
canal.mq.bufferMemory = 33554432
# Canal 的批量大小，默认值为 50K。由于 Kafka 的最大消息大小限制（小于 900K），请不要超过 1M
canal.mq.canalBatchSize = 50
# Canal 获取超时时间，以毫秒为单位。空表示无限超时。
...
对应的同步日志显示在 `logs/example/example.log` 和 Kafka 中，格式如下：
