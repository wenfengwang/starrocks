...
本主题描述了如何使用 query_dump 接口获取 SQL 查询的详细信息及其相关信息。

在使用 StarRocks 执行 SQL 查询时，如果遇到以下任何问题，您可以使用 query_dump 获取 SQL 详细信息，并将信息发送给 StarRocks 技术支持进行故障排除：

* 执行 SQL 查询或 EXPLAIN 时返回“未知错误”。
* 执行 SQL 查询时返回错误消息或异常。
* 执行 SQL 查询的效率不如预期，或者执行计划可以进行优化（例如，可以修剪分区或调整联接顺序）。

## 功能概述

query_dump 接口返回 FE 在执行 SQL 时所依赖的信息，包括：
...
为保证数据隐私，我们对数据库名、表名、列名等元信息进行脱敏处理。我们还使用脱敏的元数据来重写查询语句。

元信息脱敏默认处于启用状态。如果在脱敏过程中发生异常，则使用原始信息。如果需要绕过脱敏，可以在 HTTP URI 中添加“mock=false”。
...
* query_file：包含查询的文件
* dump_file：输出文件
* db：执行 SQL 查询的数据库。如果查询包括 `use db`，则该参数是可选的。否则，必须指定它。
* mock：是否启用或禁用脱敏
