...
这是部分输出结果。请注意，我特别关注的是 1 月 6 日和 7 日，因为这是非假期周的星期一和星期二。观察新年第一天可能并不能反映正常工作日早高峰期间的情况。
...
看起来在周一或周二的早高峰时段，大约有 40 起交通事故，而在 17:00 时也大约有相同的事故数量。
...
让我们看看能见度差（0 到 1.0 英里）时的车祸数量。要回答这个问题，请在两个表的 DATETIME 列上使用 JOIN。
...
在能见度低的情况下，单小时内最高的车祸次数是 129 起。有多个因素需要考虑：

- 2014 年 2 月 3 日是星期一
- 早上 8 点是高峰时段
- 当时正在下雨（那个小时的降水量为 0.12 英寸）
- 温度为 32 华氏度（水的冰点）
- 能见度很差，只有 0.25 英里，而纽约市的正常能见度为 10 英里
...
水蒸气在 40 华氏度时可以直接凝华成冰；此查询查看的是 0 到 40 华氏度之间的温度。
...
结冰温度的结果让我有些意外，我没想到在一月寒冷的周日早上，城市里会有这么多的交通。快速查看 [weather.com](https://weather.com/storms/winter/news/northeast-storm-rain-snow-wind) 显示，那天有一场大风暴，导致了许多车祸，正如数据所显示的那样。
