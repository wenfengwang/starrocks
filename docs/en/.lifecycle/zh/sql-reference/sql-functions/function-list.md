...
StarRocks 提供了丰富的函数，以便于数据查询和分析。除了常用函数外，StarRocks 还支持 ARRAY、JSON、MAP 和 STRUCT 等半结构化函数。同时也支持高阶 [Lambda 函数](Lambda_expression.md)。如果这些函数无法满足您的业务需求，您可以使用 [Java UDF](JAVA_UDF.md) 来编译函数。
