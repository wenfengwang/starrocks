...
- `INTERVAL N type`：时间粒度，例如 `interval 5 day`。
  - `N` 表示时间间隔的长度，必须是 INT 类型的值。
  - `type` 表示时间单位，可以是 YEAR、QUARTER、MONTH、WEEK、DAY。如果将 `type` 设置为 HOUR、MINUTE 或 SECOND 且作用于 DATE 值，则会返回错误。
- `boundary`：可选参数，用于指定是返回时间间隔的开始（`FLOOR`）还是结束（`CEIL`）。有效取值：FLOOR、CEIL。如果未指定此参数，则默认为 `FLOOR`。
...
时间间隔的起始时间为公元元年 `0001-01-01 00:00:00`。

## 例子

示例 1：将给定时间转换为 5 年时间间隔的开始时间，不指定 `boundary` 参数。
...
示例 3：将给定的 DATE 值转换为 5 秒时间间隔的开始时间。
...
由于系统无法找到 DATE 值的时间部分，因此返回错误。

示例 4：将给定的 DATE 值转换为 5 天时间间隔的开始时间。
...
示例 5：将给定的 DATE 值转换为 5 天时间间隔的结束时间。
