...
星型架构基准测试（SSB）旨在测试OLAP数据库产品的基本性能指标。SSB使用星型模式测试集，该测试集在学术界和工业界得到广泛应用。更多信息，请参阅论文 [星型架构基准测试](https://www.cs.umb.edu/~poneil/StarSchemaB.PDF)。
ClickHouse将星型模式扁平化为宽扁平表，并将SSB重写为单表基准。更多信息，请参见 [ClickHouse的星型架构基准测试](https://clickhouse.tech/docs/en/getting-started/example-datasets/star-schema/)。
本次测试比较了StarRocks、Apache Druid和ClickHouse在SSB单表数据集上的性能。

## 测试结论

- 在SSB标准数据集上执行的13个查询中，StarRocks的整体查询性能是**ClickHouse的2.1倍，是Apache Druid的8.7倍**。
- 开启StarRocks的位图索引后，性能是关闭该功能时的1.3倍。StarRocks的整体性能是**ClickHouse的2.8倍，是Apache Druid的11.4倍**。
...
| 机器           | 3台云主机                                                |
| ----------------- | ------------------------------------------------------------ |
| CPU               | 16核 Intel (R) Xeon (R) Platinum 8269CY CPU @2.50GHz <br />缓存大小：36608 KB |
| 内存            | 64 GB                                                        |
| 网络带宽 | 5 Gbit/s                                                     |
| 磁盘              | ESSD                                                         |
...
StarRocks、Apache Druid和ClickHouse部署在相同配置的主机上。

- StarRocks：1个FE和3个BE。FE可以单独部署，也可以与BE混合部署。
- ClickHouse：三个节点，分布式表
- Apache Druid：三个节点。一个部署有Master服务器和数据服务器，一个部署有查询服务器和数据服务器，第三个仅部署有数据服务器。

内核版本：Linux 3.10.0-1160.59.1.el7.x86_64

操作系统版本：CentOS Linux release 7.9.2009

软件版本：StarRocks社区版3.0，ClickHouse 23.3，Apache Druid 25.0.0
...
| customer       | 300万    | Customer维度表 |
| part           | 140万  | Parts维度表     |
| supplier       | 20万 | Supplier维度表 |
| dates          | 2,556        | Date维度表     |
| lineorder_flat | 6亿  | Lineorder扁平表     |

### 测试结果

下表显示了13个查询的性能测试结果。查询延迟的单位是毫秒。表头中的`ClickHouse vs StarRocks`表示使用ClickHouse的查询响应时间除以StarRocks的查询响应时间。数值越大表示StarRocks的性能越好。
...
| 合计  | 1236          | 939                 | 2645                | 2.14                    | 10750            | 8.70               |

## 测试流程

有关如何创建ClickHouse表并将数据加载到表中的更多信息，请参见[ClickHouse官方文档](https://clickhouse.tech/docs/en/getting-started/example-datasets/star-schema/)。以下部分描述了StarRocks的数据生成和数据加载。
...
编译完成后，所有相关工具都将安装到`output`目录，并且以下操作都将在该目录下执行。

首先，为SSB标准数据集生成数据，`scale factor=100`。
...
启用位图索引后，StarRocks的性能将得到提升。如果要测试启用位图索引后StarRocks的性能，特别是在Q2.2、Q2.3和Q3.3上，可以为所有STRING列创建位图索引。

1. 创建另一个`lineorder_flat`表，并创建位图索引。
...
数据加载完成后，等待数据版本压缩完成，然后再次执行[4.4查询数据](#query-data)。

您可以通过运行`select CANDIDATES_NUM from information_schema.be_compactions`来查看数据版本压缩的进度。对于三个BE节点，以下结果显示压缩已完成：
...
--Q2.3
选择 sum(lo_revenue), year(lo_orderdate) AS year, p_brand 
从 lineorder_flat 
...
--Q3.2 
选择 c_city, s_city, year(lo_orderdate) AS year, sum(lo_revenue) AS revenue
从 lineorder_flat 
...
--Q4.1 
选择 year(lo_orderdate) AS year, c_nation, sum(lo_revenue - lo_supplycost) AS profit
从 lineorder_flat 
...
--Q4.3 
选择 year(lo_orderdate) AS year, s_city, p_brand, 
...
以下语句与当前群集大小和数据大小（三个 BE，比例因子 = 100）匹配。如果您的集群具有更多的 BE 节点或更大的数据量，您可以调整存储桶数量，重新创建表，然后重新加载数据，以获得更好的测试结果。
...
CREATE TABLE `lineorder_flat` (
  ...
  `LO_REVENUE` int(11) NOT NULL COMMENT "",
  `LO_SUPPLYCOST` int(11) NOT NULL COMMENT "",
  `LO_TAX` tinyint(4) NOT NOT NULL COMMENT "",
  `LO_COMMITDATE` date NOT NULL COMMENT "",
  `LO_SHIPMODE` varchar(100) NOT NULL COMMENT "",
  `C_NAME` varchar(100) NOT NULL COMMENT "",
  `C_ADDRESS` varchar(100) NOT NULL COMMENT "",
...
重复键(`LO_ORDERDATE`, `LO_ORDERKEY`)
注释 "OLAP"
按`LO_ORDERDATE`的年份截断进行分区
按`LO_ORDERKEY`的哈希值进行分发 划分为48个存储桶
