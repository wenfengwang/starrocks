...
TPC-H 是由事务处理性能委员会（TPC）开发的决策支持基准。它包括一套面向业务的即席查询和并发数据修改。
TPC-H 可用于构建基于真实生产环境的模型，以模拟销售系统的数据仓库。此测试使用 8 张表，数据大小从 1 GB 到 3 TB 不等。共测试了 22 个查询，主要性能指标是每个查询的响应时间，即从提交查询到返回结果的持续时间。

## 1. 测试结论

我们对 TPC-H 100 GB 数据集进行了 22 个查询的测试。以下图显示了测试结果。
![比较](../assets/7.2-1.png)

在测试中，StarRocks 从其原生存储和 Hive 外部表中查询数据。StarRocks 和 Trino 从 Hive 外部表中查询相同的数据副本。数据经过 ZLIB 压缩，并以 ORC 格式存储。

StarRocks 从其原生存储查询数据的延迟为 21 秒，从 Hive 外部表查询数据的延迟为 92 秒，Trino 查询 Hive 外部表的延迟**长达 307 秒**。
...
> StarRocks FE 可以单独部署，也可以与 BE 混合部署，不会影响测试结果。

## 3. 测试数据和结果

### 3.1 测试数据

该测试使用了 TPC-H 100 GB 数据集。

| 表    | 记录数     | StarRocks 压缩后的数据大小 |
| -------- | ----------- | ---------------------------------------- |
...
| **查询** | **StarRocks原生 （ms）** | **StarRocks-Hive 外部 （ms）** | **Trino（毫秒）** |
| --------- | ------------------------- | -------------------------------- | -------------- |
| 问题1        | 1291                      | 5349                             | 14350          |
| 问题2        | 257                       | 940                              | 4075           |
| 问题3        | 690                       | 2585                             | 7615           |
...
| 问题11       | 265                       | 691                              | 2090           |
| 问题12       | 387                       | 3827                             | 7635           |
| 问题13       | 2165                      | 3733                             | 14530          |
| 问14       | 184                       | 3824                             | 4355           |
| 问题15       | 354                       | 7321                             | 8975           |
| 问题16       | 240                       | 1029                             | 2265           |
| 问题17       | 753                       | 5011                             | 46405          |
| 问题18       | 2909                      | 7032                             | 30975          |
| 问题19       | 702                       | 3905                             | 7315           |
| 问题20       | 963                       | 4623                             | 12500          |
| 问题21       | 870                       | 10016                            | 58340          |
| 问题22       | 545                       | 991                              | 5605           |
| 和       | 21060                     | 92449                            | 307975         |
...
2. 运行以下脚本以创建表：
    ...
    表已创建，并指定了默认存储桶编号。您可以删除之前的表，并根据集群大小和节点配置重新规划存储桶编号，以获得更好的测试结果。
    ...
    重复键(`s_suppkey`)
    注释 "OLAP"
    哈希分布(`s_suppkey`) 划分 12 桶
    属性 (
        "replication_num" = "1",
        "storage_format" = "DEFAULT"
    );
    如果存在则删除视图revenue0;
    创建视图revenue0 (supplier_no, total_revenue) as
    选择
        l_suppkey,
        总数(l_extendedprice * (1 - l_discount))
    从
        lineitem
    其中
        l_shipdate >= 日期 '1996-01-01'
        和 l_shipdate < 日期 '1996-01-01' + 间隔 '3' 月
    按
        l_suppkey 分组;
    ```
...
此步骤是可选的。我们建议收集完整的统计信息以生成更好的测试结果。连接到 StarRocks 集群后，运行以下命令：
...
1. 在 Hive 中创建 Hive 外部表。表格格式为 CSV。将 CSV 测试数据上传到 HDFS 的数据存储目录下。在本示例中，Hive 外部表的 HDFS 数据存储路径为 /user/tmp/csv/。
...
2. 将每个 Hive 外部表中的 CSV 数据转换为 ORC 格式的 Hive 表。这有助于在 StarRocks 和 Trino 中为后续测试创建 ORC 格式的 Hive 外部表。
