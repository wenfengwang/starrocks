...
with year_total as (
 ...
 where t_s_secyear.customer_id = t_s_firstyear.customer_id
         ...
         并且 t_w_secyear.sale_type = 'w'
         并且 t_s_firstyear.dyear = 2001
         并且 t_s_secyear.dyear = 2001+1
         并且 t_w_firstyear.dyear = 2001
         并且 t_w_secyear.dyear = 2001+1
         并且 t_s_firstyear.year_total > 0
         并且 t_w_firstyear.year_total > 0
         并且 case when t_w_firstyear.year_total > 0 then t_w_secyear.year_total / t_w_firstyear.year_total else 0.0 end
             > case when t_s_firstyear.year_total > 0 then t_s_secyear.year_total / t_s_firstyear.year_total else 0.0 end
 按 t_s_secyear.customer_id
         ,t_s_secyear.customer_first_name
         ,t_s_secyear.customer_last_name
         ,t_s_secyear.customer_preferred_cust_flag
限制 100;
...
在这里 
    ws_item_sk = i_item_sk 
     并且 i_category in ('Sports', 'Books', 'Home')
     并且 ws_sold_date_sk = d_date_sk
    并且 d_date between cast('1999-02-22' as date) 
                和 date_add(cast('1999-02-22' as date), 30)
按 
    i_category
        ,i_class
        ,i_item_id
...
选择 avg(ss_quantity)
 ...
 从 store_sales
     ...
     ,customer_address
     ,date_dim
 在这里 s_store_sk = ss_store_sk
 并且  ss_sold_date_sk = d_date_sk 并且 d_year = 2001
 并且((ss_hdemo_sk=hd_demo_sk
  并且 cd_demo_sk = ss_cdemo_sk
  并且 cd_marital_status = 'M'
  并且 cd_education_status = 'Advanced Degree'
  并且 ss_sales_price between 100.00 and 150.00
  并且 hd_dep_count = 3   
     )或
     (ss_hdemo_sk=hd_demo_sk
  并且 cd_demo_sk = ss_cdemo_sk
  并且 cd_marital_status = 'S'
  并且 cd_education_status = 'College'
  并且 ss_sales_price between 50.00 and 100.00   
  并且 hd_dep_count = 1
     ) 或 
     (ss_hdemo_sk=hd_demo_sk
  并且 cd_demo_sk = ss_cdemo_sk
  并且 cd_marital_status = 'W'
  并且 cd_education_status = '2 yr Degree'
  并且 ss_sales_price between 150.00 and 200.00 
  并且 hd_dep_count = 1  
     ))
 并且((ss_addr_sk = ca_address_sk
  并且 ca_country = 'United States'
  并且 ca_state in ('TX', 'OH', 'TX')
  并且 ss_net_profit between 100 and 200  
     ) 或
     (ss_addr_sk = ca_address_sk
  并且 ca_country = 'United States'
  并且 ca_state in ('OR', 'NM', 'KY')
  并且 ss_net_profit between 150 and 300  
     ) 或
     (ss_addr_sk = ca_address_sk
  并且 ca_country = 'United States'
  并且 ca_state in ('VA', 'TX', 'MS')
  并且 ss_net_profit between 50 and 250  
     ))
;
...
-- 查询 14
用  cross_items 作为
 (选择 i_item_sk ss_item_sk
 从 item,
 (选择 iss.i_brand_id brand_id
     ,iss.i_class_id class_id
     ,iss.i_category_id category_id
 从 store_sales
     ,item iss
     ,date_dim d1
 在这里 ss_item_sk = iss.i_item_sk
   并且 ss_sold_date_sk = d1.d_date_sk
   并且 d1.d_year between 1999 AND 1999 + 2
 交集 
 (选择 ics.i_brand_id
     ,ics.i_class_id
     ,ics.i_category_id
 从 catalog_sales
     ,item ics
     ,date_dim d2
 在这里 cs_item_sk = ics.i_item_sk
   并且 cs_sold_date_sk = d2.d_date_sk
   并且 d2.d_year between 1999 AND 1999 + 2
 交集
 (选择 iws.i_brand_id
     ,iws.i_class_id
     ,iws.i_category_id
 从 web_sales
     ,item iws
     ,date_dim d3
 在这里 ws_item_sk = iws.i_item_sk
   并且 ws_sold_date_sk = d3.d_date_sk
   并且 d3.d_year between 1999 AND 1999 + 2) t
 在这里 i_brand_id = brand_id
      并且 i_class_id = class_id
      并且 i_category_id = category_id
),
 avg_sales 作为
(选择 avg(quantity*list_price) average_sales
 从 (选择 ss_quantity quantity
             ,ss_list_price list_price
 从 store_sales
           ,date_dim
 在这里 ss_sold_date_sk = d_date_sk
         并且 d_year between 1999 and 1999 + 2
       联合全部 
 (选择 cs_quantity quantity 
             ,cs_list_price list_price
 从 catalog_sales
           ,date_dim
 在这里 cs_sold_date_sk = d_date_sk
         并且 d_year between 1999 and 1999 + 2 
       联合全部
 (选择 ws_quantity quantity
             ,ws_list_price list_price
 从 web_sales
           ,date_dim
 在这里 ws_sold_date_sk = d_date_sk
         并且 d_year between 1999 and 1999 + 2) x)
 选择  channel, i_brand_id,i_class_id,i_category_id,sum(sales), sum(number_sales)
从(
       选择 'store' channel, i_brand_id,i_class_id
             ,i_category_id,sum(ss_quantity*ss_list_price) sales
             , count(*) number_sales
 从 store_sales
           ,item
           ,date_dim
 在这里 ss_item_sk in (选择 ss_item_sk from cross_items)
         并且 ss_item_sk = i_item_sk
         并且 ss_sold_date_sk = d_date_sk
         并且 d_year = 1999+2 
         并且 d_moy = 11
 分组按 i_brand_id,i_class_id,i_category_id
 拥有 sum(ss_quantity*ss_list_price) > (选择 average_sales from avg_sales)
       联合全部
 (选择 'catalog' channel, i_brand_id,i_class_id,i_category_id, sum(cs_quantity*cs_list_price) sales, count(*) number_sales
 从 catalog_sales
           ,item
           ,date_dim
 在这里 cs_item_sk in (选择 ss_item_sk from cross_items)
         并且 cs_item_sk = i_item_sk
         并且 cs_sold_date_sk = d_date_sk
         并且 d_year = 1999+2 
         并且 d_moy = 11
 分组按 i_brand_id,i_class_id,i_category_id
 拥有 sum(cs_quantity*cs_list_price) > (选择 average_sales from avg_sales)
       联合全部
 (选择 'web' channel, i_brand_id,i_class_id,i_category_id, sum(ws_quantity*ws_list_price) sales , count(*) number_sales
 从 web_sales
           ,item
           ,date_dim
 在这里 ws_item_sk in (选择 ss_item_sk from cross_items)
         并且 ws_item_sk = i_item_sk
         并且 ws_sold_date_sk = d_date_sk
         并且 d_year = 1999+2
         并且 d_moy = 11
 分组按 i_brand_id,i_class_id,i_category_id
 拥有 sum(ws_quantity*ws_list_price) > (选择 average_sales from avg_sales)
 ) y
 按 rollup (channel, i_brand_id,i_class_id,i_category_id)
 按 channel,i_brand_id,i_class_id,i_category_id
限制 100;
用  cross_items 作为
 (选择 i_item_sk ss_item_sk
 从 item,
 ...
 在这里 ws_item_sk = iws.i_item_sk
   并且 ws_sold_date_sk = d3.d_date_sk
   并且 d3.d_year between 1999 AND 1999 + 2) x
 在这里 i_brand_id = brand_id
      并且 i_class_id = class_id
      并且 i_category_id = category_id
),
...
(选择 avg(quantity*list_price) average_sales
 ...
 在这里 ws_sold_date_sk = d_date_sk
         并且 d_year between 1999 and 1999 + 2) x)
 选择  this_year.channel ty_channel
...
从
 (选择 'store' channel, i_brand_id,i_class_id,i_category_id
 ...
 在这里 ss_item_sk in (选择 ss_item_sk from cross_items)
   并且 ss_item_sk = i_item_sk
   并且 ss_sold_date_sk = d_date_sk
   并且 d_week_seq = (选择 d_week_seq
                     从 date_dim
                     在这里 d_year = 1999 + 1
                       并且 d_moy = 12
                       并且 d_dom = 11)
 分组按 i_brand_id,i_class_id,i_category_id
 拥有 sum(ss_quantity*ss_list_price) > (选择 average_sales from avg_sales)) this_year,
 (选择 'store' channel, i_brand_id,i_class_id
 ...
 在这里 ss_item_sk in (选择 ss_item_sk from cross_items)
   ...
   并且 d_week_seq = (选择 d_week_seq
                     ...
                     在这里 d_year = 1999
                       ...
                       并且 d_dom = 11)
 分组按 i_brand_id,i_class_id,i_category_id
 拥有 sum(ss_quantity*ss_list_price) > (选择 average_sales from avg_sales)) last_year
 在这里 this_year.i_brand_id= last_year.i_brand_id
   并且 this_year.i_class_id = last_year.i_class_id
   并且 this_year.i_category_id = last_year.i_category_id
按 this_year.channel, this_year.i_brand_id, this_year.i_class_id, this_year.i_category_id
限制 100;
-- 查询 15
选择  ca_zip
       ,sum(cs_sales_price)
从 catalog_sales
     ...
     ,customer_address
     ,date_dim
 在这里 cs_bill_customer_sk = c_customer_sk
    并且 c_current_addr_sk = ca_address_sk 
    并且 ( substr(ca_zip,1,5) in ('85669', '86197','88274','83405','86475',
                                   '85392', '85460', '80348', '81792')
          或 ca_state in ('CA','WA','GA')
          或 cs_sales_price > 500)
    并且 cs_sold_date_sk = d_date_sk
    并且 d_qoy = 2 并且 d_year = 2001
 分组按 ca_zip
 按 ca_zip
限制 100;
...
从
  ...
  ,call_center
在这里
    d_date between '2002-2-01' 和 
           date_add(cast('2002-2-01' as date), 60)
并且 cs1.cs_ship_date_sk = d_date_sk
并且 cs1.cs_ship_addr_sk = ca_address_sk
并且 ca_state = 'GA'
并且 cs1.cs_call_center_sk = cc_call_center_sk
并且 cc_county in ('Williamson County', 'Williamson County',
                  'Williamson County', 'Williamson County',
                  'Williamson County')
并且 存在 (选择 *
            从 catalog_sales cs2
            在这里 cs1.cs_order_number = cs2.cs_order_number
              and cs1.cs_warehouse_sk <> cs2.cs_warehouse_sk)
并且不存在(select *
               从目录退货 cr1
               where cs1.cs_order_number = cr1.cr_order_number)
按照计数(不同的 cs_order_number)排序
限制 100;
...
选择  i_item_id
       ,i_item_desc
       ,s_state
       ,计数(ss_quantity) 作为 store_sales_quantitycount
       ,平均值(ss_quantity) 作为 store_sales_quantityave
       ,stddev_samp(ss_quantity) 作为 store_sales_quantitystdev
       ,stddev_samp(ss_quantity)/平均值(ss_quantity) 作为 store_sales_quantitycov
       ,计数(sr_return_quantity) 作为 store_returns_quantitycount
       ,平均值(sr_return_quantity) 作为 store_returns_quantityave
       ,stddev_samp(sr_return_quantity) 作为 store_returns_quantitystdev
       ,stddev_samp(sr_return_quantity)/平均值(sr_return_quantity) 作为 store_returns_quantitycov
       ,计数(cs_quantity) 作为 catalog_sales_quantitycount ,平均值(cs_quantity) 作为 catalog_sales_quantityave
       ,stddev_samp(cs_quantity) 作为 catalog_sales_quantitystdev
       ,stddev_samp(cs_quantity)/平均值(cs_quantity) 作为 catalog_sales_quantitycov
从 store_sales
     ,store_returns
     ,catalog_sales
     ,date_dim d1
...
选择  i_item_id,
        ...
        ca_state, 
        ca_county,
        平均值( 转换(cs_quantity 为 十进制(12,2))) agg1,
        平均值( 转换(cs_list_price 为 十进制(12,2))) agg2,
        平均值( 转换(cs_coupon_amt 为 十进制(12,2))) agg3,
        平均值( 转换(cs_sales_price 为 十进制(12,2))) agg4,
        平均值( 转换(cs_net_profit 为 十进制(12,2))) agg5,
        平均值( 转换(c_birth_year 为 十进制(12,2))) agg6,
        平均值( 转换(cd1.cd_dep_count 为 十进制(12,2))) agg7
 从 catalog_sales, customer_demographics cd1, 
      customer_demographics cd2, customer, customer_address, date_dim, item
 其中 cs_sold_date_sk = d_date_sk 和
       cs_item_sk = i_item_sk 和
       cs_bill_cdemo_sk = cd1.cd_demo_sk 和
       cs_bill_customer_sk = c_customer_sk 和
       cd1.cd_gender = 'F' 和 
       cd1.cd_education_status = 'Unknown' 和
       c_current_cdemo_sk = cd2.cd_demo_sk 和
       c_current_addr_sk = ca_address_sk 和
       c_birth_month 在 (1, 6, 8, 9, 12, 2) 和
       d_year = 1998 和
       ca_state 在 ('MS','IN','ND'
                   ,'OK','NM','VA','MS')
 按照 rollup (i_item_id, ca_country, ca_state, ca_county)
排序
按照 ca_country,
        ca_state, 
        ca_county,
...
-- 查询 19
选择  i_brand_id 品牌ID, i_brand 品牌, i_manufact_id, i_manufact,
    总和(ss_ext_sales_price) ext_price
 从 date_dim, store_sales, item,customer,customer_address,store
 其中 d_date_sk = ss_sold_date_sk 和
   ss_item_sk = i_item_sk 和
   i_manager_id=8 和
   d_moy=11 和
   d_year=1998 和
   ss_customer_sk = c_customer_sk 和
   c_current_addr_sk = ca_address_sk 和
   substr(ca_zip,1,5) <> substr(s_zip,1,5) 和
...
选择  i_item_id
       ...
       ,i_class 
       ,i_current_price
       ,总和(cs_ext_sales_price) 作为 itemrevenue 
       ,总和(cs_ext_sales_price)*100/总和(总和(cs_ext_sales_price)) over
           (按照 i_class 分区) as revenueratio
 从    catalog_sales
     ,item 
     ,date_dim
 其中 cs_item_sk = i_item_sk 和
   i_category 在 ('Sports','Books','Home') 和
   cs_sold_date_sk = d_date_sk
 和 d_date 在 cast('1999-02-22' as date) 
             和 date_add(cast('1999-02-22' as date), 30)
 按照 i_item_id
         ,i_item_desc 
         ,i_category
...
-- 查询 21
选择  *
 从(选择 w_warehouse_name
            ,i_item_id
            ,总和(case when (转换(d_date 为 日期) < cast ('2000-03-11' as date))
   ...
   从 inventory
       ...
       ,item
       ,date_dim
 其中 i_current_price 在 0.99 和 1.49 和
     i_item_sk          = inv_item_sk 和
     inv_warehouse_sk   = w_warehouse_sk 和
     inv_date_sk    = d_date_sk 和
     d_date 在 date_add(cast ('2000-03-11' as date), -30)
                    和 date_add(cast ('2000-03-11' as date), 30)
 按照 w_warehouse_name, i_item_id) x
 其中 (case when inv_before > 0 
             then inv_after / inv_before 
             else null
             end) 在 2.0/3.0 和 3.0/2.0
排序
         ,w_warehouse_name
         ,i_item_id
限制 100;
...
选择  i_product_name
             ...
             ,i_class
             ,i_category
             ,平均值(inv_quantity_on_hand) qoh
       从 inventory
           ,date_dim
           ,item
       其中 inv_date_sk=d_date_sk
              和 inv_item_sk=i_item_sk
              和 d_month_seq 在 1200 和 1200 + 11
       按照 rollup(i_product_name
                       ,i_brand
                       ,i_class
                       ,i_category)
排序
...
with frequent_ss_items as 
 (选择 substr(i_item_desc,1,30) itemdesc,i_item_sk item_sk,d_date solddate,计数(*) cnt
  从 store_sales
      ,date_dim 
      ,item
  其中 ss_sold_date_sk = d_date_sk
    和 ss_item_sk = i_item_sk 
    和 d_year 在 (2000,2000+1,2000+2,2000+3)
  按照 substr(i_item_desc,1,30),i_item_sk,d_date
  having 计数(*) >4),
 max_store_sales as
 (选择 最大值(csales) tpcds_cmax 
  从 (选择 c_customer_sk,总和(ss_quantity*ss_sales_price) csales
        从 store_sales
            ,customer
            ,date_dim 
        ...
        按照 c_customer_sk) t1),
 best_ss_customer as
 (选择 c_customer_sk,总和(ss_quantity*ss_sales_price) ssales
  从 store_sales
      ,customer
  其中 ss_customer_sk = c_customer_sk
  按照 c_customer_sk
  having 总和(ss_quantity*ss_sales_price) > (50/100.0) * (选择
  *
从
 max_store_sales))
  选择  总和(sales)
 从 (选择 cs_quantity*cs_list_price sales
       从 catalog_sales
           ,date_dim 
       其中 d_year = 2000 
         和 d_moy = 2 
         和 cs_sold_date_sk = d_date_sk 
         和 cs_item_sk 在 (选择 item_sk 从 frequent_ss_items)
         和 cs_bill_customer_sk 在 (选择 c_customer_sk 从 best_ss_customer)
      union all
      选择 ws_quantity*ws_list_price sales
       从 web_sales 
           ,date_dim 
       其中 d_year = 2000 
         和 d_moy = 2 
         和 ws_sold_date_sk = d_date_sk 
         和 ws_item_sk 在 (选择 item_sk 从 frequent_ss_items)
         和 ws_bill_customer_sk 在 (选择 c_customer_sk 从 best_ss_customer)) t2
 限制 100;
with frequent_ss_items as
 (选择 substr(i_item_desc,1,30) itemdesc,i_item_sk item_sk,d_date solddate,计数(*) cnt
  从 store_sales
  ...
  *
从 max_store_sales))
  选择  c_last_name,c_first_name,sales
 从 (选择 c_last_name,c_first_name,总和(cs_quantity*cs_list_price) sales
        从 catalog_sales
            ,customer
            ,date_dim 
        其中 d_year = 2000 
         ...
         和 cs_item_sk 在 (选择 item_sk 从 frequent_ss_items)
         和 cs_bill_customer_sk 在 (选择 c_customer_sk 从 best_ss_customer)
         和 cs_bill_customer_sk = c_customer_sk 
       按照 c_last_name,c_first_name
      union all
      选择 c_last_name,c_first_name,总和(ws_quantity*ws_list_price) sales
       从 web_sales
           ,customer
           ,date_dim 
       其中 d_year = 2000 
         ...
         和 ws_sold_date_sk = d_date_sk 
         和 ws_item_sk 在 (选择 item_sk 从 frequent_ss_items)
         和 ws_bill_customer_sk 在 (选择 c_customer_sk 从 best_ss_customer)
         和 ws_bill_customer_sk = c_customer_sk
       按照 c_last_name,c_first_name) t2
     按照 c_last_name,c_first_name,sales
 限制 100;
-- 查询 24
with ssales as
(选择 c_last_name
...
其中 ss_ticket_number = sr_ticket_number
  ...
  和 ss_customer_sk = c_customer_sk
  和 ss_item_sk = i_item_sk
  和 ss_store_sk = s_store_sk
  和 c_current_addr_sk = ca_address_sk
  和 c_birth_country <> 大写(ca_country)
  和 s_zip = ca_zip
和 s_market_id=8
按照 c_last_name
        ,c_first_name
...
按照 c_last_name
        ,c_first_name
        ,s_store_name
having 总和(netpaid) > (选择 0.05*平均值(netpaid)
                                 从 ssales)
排序
         ,c_last_name
        ,c_first_name
...
-- query 27
select  i_item_id,
        s_state, grouping(s_state) as g_state,
...
from (select avg(ss_list_price) as B1_LP
      ...
      where ss_quantity between 0 and 5
        and (ss_list_price between 8 and 8+10 
             ...
             or ss_wholesale_cost between 57 and 57+20)) as B1,
     (select avg(ss_list_price) as B2_LP
      ...
      where ss_quantity between 6 and 10
        and (ss_list_price between 90 and 90+10
          ...
          or ss_wholesale_cost between 31 and 31+20)) as B2,
     (select avg(ss_list_price) as B3_LP
      ...
      where ss_quantity between 11 and 15
        and (ss_list_price between 142 and 142+10
          ...
          or ss_wholesale_cost between 79 and 79+20)) as B3,
     (select avg(ss_list_price) as B4_LP
      ...
      where ss_quantity between 16 and 20
        and (ss_list_price between 135 and 135+10
          ...
          or ss_wholesale_cost between 38 and 38+20)) as B4,
     (select avg(ss_list_price) as B5_LP
      ...
      where ss_quantity between 21 and 25
        and (ss_list_price between 122 and 122+10
          ...
          or ss_wholesale_cost between 17 and 17+20)) as B5,
     (select avg(ss_list_price) as B6_LP
      ...
      where ss_quantity between 26 and 30
        and (ss_list_price between 154 and 154+10
          ...
          or ss_wholesale_cost between 7 and 7+20)) as B6
limit 100;
...
选择 c_last_name
   ...
   (选择 ss_ticket_number
    ...
    在哪里 store_sales.ss_sold_date_sk = date_dim.d_date_sk
    ...
    和 (如果 household_demographics.hd_vehicle_count > 0 
    ...
    按ss_ticket_number,ss_customer_sk分组) dn,customer
    在哪里 ss_customer_sk = c_customer_sk
      和 cnt 在 15 和 20 之间
    按c_last_name,c_first_name,c_salutation,c_preferred_cust_flag降序, ss_ticket_number排序;
...
选择   
 ...
 在哪里
  ...
  存在 (选择 *
          ...
          在哪里 c.c_customer_sk = ss_customer_sk 和
        ...
        联合全部 
           选择 cs_ship_customer_sk customsk
            ...
            在哪里 
                  ...
                  d_year = 2002 和
                  d_qoy < 4)x 
           在哪里 x.customsk = c.c_customer_sk)
 按ca_state,cd_gender,cd_marital_status,cd_dep_count,cd_dep_employed_count,cd_dep_college_count分组
 按ca_state,cd_gender,cd_marital_status,cd_dep_count,cd_dep_employed_count,cd_dep_college_count排序
限制 100;


-- 查询 36
选择  
    sum(ss_net_profit)/sum(ss_ext_sales_price) 作为 gross_margin
   ,i_category
   ,i_class
   ,分组(i_category)+分组(i_class) 作为 lochierarchy
   ,rank() over (
    分区按 分组(i_category)+分组(i_class),
    如果 分组(i_class) = 0 那么 i_category 结束 
    按 sum(ss_net_profit)/sum(ss_ext_sales_price) 升序) 作为 rank_within_parent
 从
    store_sales
   ,date_dim       d1
   ,item
 ...
 和 s_state 在 ('TN','TN','TN','TN',
                 'TN','TN','TN','TN')
 按rollup(i_category,i_class)分组
 按
   lochierarchy降序
  ,如果 lochierarchy = 0 那么 i_category结束
  ,rank_within_parent排序
限制 100;
...
选择  i_item_id
 ...
 在哪里 i_current_price 在 68 和 68 + 30 之间
 和 inv_item_sk = i_item_sk
 和 d_date_sk=inv_date_sk
 和 d_date 在 cast('2000-02-01' as date) 和 date_add(cast('2000-02-01' as date),  60) 之间
 和 i_manufact_id 在 (677,940,694,808)
 和 inv_quantity_on_hand 在 100 和 500 之间
 和 cs_item_sk = i_item_sk
 按i_item_id,i_item_desc,i_current_price分组
 按i_item_id排序
限制 100;
...
-- 查询 38
选择  计数(*) 从 (
    选择 不同的 c_last_name, c_first_name, d_date
  ...
  交集
    ...
    从 catalog_sales, date_dim, customer
          在哪里 catalog_sales.cs_sold_date_sk = date_dim.d_date_sk
      和 catalog_sales.cs_bill_customer_sk = customer.c_customer_sk
      和 d_month_seq 在 1200 和 1200 + 11 之间
  交集
    选择 不同的 c_last_name, c_first_name, d_date
    从 web_sales, date_dim, customer
          在哪里 web_sales.ws_sold_date_sk = date_dim.d_date_sk
...
(选择 w_warehouse_name,w_warehouse_sk,i_item_sk,d_moy
       ,stdev,mean, 如果 mean = 0 那么 空 否则 stdev/mean 结束 cov
 从(选择 w_warehouse_name,w_warehouse_sk,i_item_sk,d_moy
      ...
      从 inventory
          ...
          ,warehouse
          ,date_dim
      在哪里 inv_item_sk = i_item_sk
        和 inv_warehouse_sk = w_warehouse_sk
        和 inv_date_sk = d_date_sk
        和 d_year =2001
      按w_warehouse_name,w_warehouse_sk,i_item_sk,d_moy分组) foo
 在哪里 如果 mean = 0 那么 0 否则 stdev/mean 结束 > 1)
选择 inv1.w_warehouse_sk,inv1.i_item_sk,inv1.d_moy,inv1.mean, inv1.cov
        ,inv2.w_warehouse_sk,inv2.i_item_sk,inv2.d_moy,inv2.mean, inv2.cov
从 inv inv1,inv inv2
在哪里 inv1.i_item_sk = inv2.i_item_sk
  和 inv1.w_warehouse_sk =  inv2.w_warehouse_sk
  和 inv1.d_moy=1
  和 inv2.d_moy=1+1
按inv1.w_warehouse_sk,inv1.i_item_sk,inv1.d_moy,inv1.mean,inv1.cov
        ,inv2.d_moy,inv2.mean, inv2.cov排序
;
用inv作为
(选择 w_warehouse_name,w_warehouse_sk,i_item_sk,d_moy
       ,stdev,mean, 如果 mean = 0 那么 空 否则 stdev/mean 结束 cov
...
选择  
   w_state
  ,i_item_id
  ,sum(如果 (cast(d_date as date) < cast ('2000-03-11' as date)) 
       那么 cs_sales_price - coalesce(cr_refunded_cash,0) 否则 0 结束) 作为 sales_before
  ,sum(如果 (cast(d_date as date) >= cast ('2000-03-11' as date)) 
       那么 cs_sales_price - coalesce(cr_refunded_cash,0) 否则 0 结束) 作为 sales_after
从
   catalog_sales 左外连接 catalog_returns 在
       (cs_order_number = cr_order_number 
        和 cs_item_sk = cr_item_sk)
  ,warehouse 
  ,item
  ,date_dim
在哪里
     i_current_price 在 0.99 和 1.49 之间
 和 i_item_sk          = cs_item_sk
 和 cs_warehouse_sk    = w_warehouse_sk 
 和 cs_sold_date_sk    = d_date_sk
 和 d_date 在 date_add(cast ('2000-03-11' as date), -30)
                和 date_add(cast ('2000-03-11' as date), 30) 之间
 按
    w_state,i_item_id分组
 按w_state,i_item_id排序
限制 100;


-- 查询 41
选择  不同的(i_product_name)
 从 item i1
 在哪里 i_manufact_id 在 738 和 738+40 
   和 (选择 计数(*) 作为 item_cnt
        从 item
        在哪里 (i_manufact = i1.i_manufact 和
        ((i_category = 'Women' 和 
        (i_color = 'powder' 或 i_color = 'khaki') 和 
        (i_units = 'Ounce' 或 i_units = 'Oz') 和
        (i_size = 'medium' 或 i_size = 'extra large')
        ) 或
        (i_category = 'Women' 和
        (i_color = 'brown' 或 i_color = 'honeydew') 和
        (i_units = 'Bunch' 或 i_units = 'Ton') 和
        (i_size = 'N/A' 或 i_size = 'small')
        ) 或
        (i_category = 'Men' 和
        (i_color = 'floral' 或 i_color = 'deep') 和
        (i_units = 'N/A' 或 i_units = 'Dozen') 和
        (i_size = 'petite' 或 i_size = 'large')
        ) 或
        (i_category = 'Men' 和
        (i_color = 'light' 或 i_color = 'cornflower') 和
        (i_units = 'Box' 或 i_units = 'Pound') 和
        (i_size = 'medium' 或 i_size = 'extra large')
        ))) 或
       (i_manufact = i1.i_manufact 和
        ((i_category = 'Women' 和 
        (i_color = 'midnight' 或 i_color = 'snow') 和 
        (i_units = 'Pallet' 或 i_units = 'Gross') 和
        (i_size = 'medium' 或 i_size = 'extra large')
        ) 或
        (i_category = 'Women' 和
        (i_color = 'cyan' 或 i_color = 'papaya') 和
        (i_units = 'Cup' 或 i_units = 'Dram') 和
        (i_size = 'N/A' 或 i_size = 'small')
        ) 或
        (i_category = 'Men' 和
        (i_color = 'orange' 或 i_color = 'frosted') 和
        (i_units = 'Each' 或 i_units = 'Tbl') 和
        (i_size = 'petite' 或 i_size = 'large')
        ) 或
        (i_category = 'Men' 和
...
select  channel, item, return_ratio, return_rank, currency_rank from
 ...
 from (
    ...
    (    select sts.ss_item_sk as item
       ...
       from 
store_sales sts 左外连接 store_returns sr
...
select  
  ...
  ,s_state
  ,s_zip
  ,sum(case when (sr_returned_date_sk - ss_sold_date_sk <= 30 ) then 1 else 0 end)  as "30 天" 
  ,sum(case when (sr_returned_date_sk - ss_sold_date_sk > 30) and 
                 (sr_returned_date_sk - ss_sold_date_sk <= 60) then 1 else 0 end )  as "31-60 天" 
  ,sum(case when (sr_returned_date_sk - ss_sold_date_sk > 60) and 
                 (sr_returned_date_sk - ss_sold_date_sk <= 90) then 1 else 0 end)  as "61-90 天" 
  ,sum(case when (sr_returned_date_sk - ss_sold_date_sk > 90) and
                 (sr_returned_date_sk - ss_sold_date_sk <= 120) then 1 else 0 end)  as "91-120 天" 
  ,sum(case when (sr_returned_date_sk - ss_sold_date_sk  > 120) then 1 else 0 end)  as ">120 天" 
from
   store_sales
  ,store_returns
  ,store
...
((i_category in ('图书','儿童','电子产品') and
i_class in ('个人','便携式','参考','自助') and
i_brand in ('scholaramalgamalg #14','scholaramalgamalg #7',
        'exportiunivamalg #9','scholaramalgamalg #9'))
or(i_category in ('女装','音乐','男装') and
i_class in ('配饰','古典','香水','裤子') and
...
with wss as 
 ...
 (select s_store_name as s_store_name1,wss.d_week_seq as d_week_seq1
  ...
  where d.d_week_seq = wss.d_week_seq and
        ss_store_sk = s_store_sk and 
        d_month_seq between 1212 and 1212 + 11) as y,
 (select s_store_name as s_store_name2,wss.d_week_seq as d_week_seq2
  ...
  where d.d_week_seq = wss.d_week_seq and
        ss_store_sk = s_store_sk and 
        d_month_seq between 1212+ 12 and 1212 + 23) as x
...
from
  (select sum(ss_ext_sales_price) as promotions
   ...
   and   s_gmt_offset = -5
   and   d_year = 1998
   and   d_moy  = 11) as promotional_sales,
...
select  channel, col_name, d_year, d_qoy, i_category, COUNT(*) sales_cnt, SUM(ext_sales_price) sales_amt FROM (
        SELECT 'store' as channel, 'ss_store_sk' as col_name, d_year, d_qoy, i_category, ss_ext_sales_price as ext_sales_price
         FROM store_sales JOIN item ON i_item_sk=ss_item_sk
                        JOIN date_dim ON d_date_sk=ss_sold_date_sk
        ...
        SELECT 'catalog' as channel, 'cs_ship_addr_sk' as col_name, d_year, d_qoy, i_category, cs_ext_sales_price as ext_sales_price
         FROM catalog_sales JOIN item ON i_item_sk=cs_item_sk
                          JOIN date_dim ON d_date_sk=cs_sold_date_sk
         WHERE cs_ship_addr_sk IS NULL) foo
...
with ss as
 ...
 where d_date between cast('2000-08-23' as date) 
...
from
  ...
  ,date_dim
where
    d_month_seq 在 1200 和 1200 + 11 之间
