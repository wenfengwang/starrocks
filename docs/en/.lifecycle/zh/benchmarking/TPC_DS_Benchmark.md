...
TPC-DS 是由事务处理性能委员会 (TPC) 开发的决策支持基准。它使用的测试数据集和 SQL 查询比 TPC-H 更为全面和复杂。

TPC-DS 对决策支持系统的多个普遍适用方面进行建模，包括查询和数据维护。TPC-DS 旨在提供一个全面且现实的工作负载，用于测试和评估零售环境中数据库系统的性能。TPC-DS 基准测试模拟了零售企业中三个销售渠道（商店、互联网和目录）的销售和退货数据。除了创建销售和退货数据模型的表格外，它还包括了一个简单的库存系统和促销系统。

该基准测试共测试了 99 个复杂的 SQL 查询，这些查询针对的是数据大小从 1 GB 到 3 GB 不等的 24 个表。主要性能指标是每个查询的响应时间，即从提交查询到返回结果的时间间隔。

## 1. 测试结论

我们对 TPC-DS 100 GB 数据集进行了 99 个查询的测试。以下图表显示了测试结果。
![tpc-ds](../assets/tpc-ds.png)

在测试中，StarRocks 从其本地存储和 Hive 外部表中查询数据。StarRocks 和 Trino 从 Hive 外部表查询相同的数据副本。数据经过 LZ4 压缩并以 Parquet 格式存储。

StarRocks 从本地存储查询数据的延迟为 **174 秒**，从 Hive 外部表查询的延迟为 **239 秒**，启用 Data Cache 功能查询 Hive 外部表的延迟为 **176 秒**，而 Trino 查询 Hive 外部表的延迟为 **892 秒**。
...
StarRocks 和 Trino 都部署在配置相同的机器上。StarRocks 部署了 1 个 FE 和 3 个 BE。Trino 部署了 1 个协调器和 3 个工作节点。

- 内核版本：Linux 3.10.0-1127.13.1.el7.x86_64
- 操作系统版本：CentOS Linux release 7.8.2003
- 软件版本：StarRocks 社区版 3.1、Trino-419、Hive-3.1.2

> StarRocks 的 FE 可以单独部署或与 BE 混合部署，这不会影响测试结果。
...
> **注意**
- 下表中查询延迟的单位是毫秒 (ms)。
- `StarRocks-3.0.5-native` 表示 StarRocks 本地存储，`StarRocks-3.0-Hive-external` 表示 StarRocks 通过 Hive Catalog 查询 Hive 外部表，`StarRocks-3.0-Hive-external-Cache` 表示 StarRocks 通过 Hive Catalog 查询 Hive 外部表，并启用了 Data Cache 功能。
...
下载 tpcds-poc 工具包并生成标准 TPC-DS 测试数据集，比例因子为 100。
...
修改配置文件 `conf/starrocks.conf`，指定集群地址。注意 `mysql_host` 和 `mysql_port`。
...
在 Hive 中创建外部表，其存储格式为 Parquet，压缩格式为 LZ4。有关详细的 CREATE TABLE 语句，请参阅[创建 Parquet Hive 外部表](#53-create-a-hive-external-tableparquet)。StarRocks 和 Trino 从这些外部表中查询数据。
...
StarRocks 使用 [Hive Catalog](../data_source/catalog/hive_catalog.md) 来查询 Hive 外部表。

如果测试期间 StarRocks 启用了 [Data Cache](../data_source/data_cache.md)，则建议对 Data Cache 功能进行以下配置。
...
有关 99 个 SQL 查询的更多信息，请参阅 [TPC-DS SQL](./tpc_ds_99_sql.md)。
