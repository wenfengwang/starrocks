...
TPC-Hは、Transaction Processing Performance Council（TPC）によって開発された意思決定支援ベンチマークです。ビジネス指向のアドホッククエリと並行データ変更のスイートで構成されています。
TPC-Hは、販売システムのデータウェアハウスをシミュレートするための実際のプロダクション環境に基づいたモデルを構築するために使用できます。このテストでは、データサイズが1GBから3TBまでの8つのテーブルを使用します。合計22のクエリがテストされ、主なパフォーマンスメトリックは各クエリの応答時間です。応答時間は、クエリが送信されて結果が返されるまでの時間のことです。

## 1. テスト結論

TPC-H 100GBのデータセットに対して22のクエリをテストしました。以下の図はテスト結果を示しています。
![comparison](../assets/7.2-1.png)

テストでは、StarRocksはネイティブストレージとHive外部テーブルからデータをクエリします。StarRocksとTrinoは、Hive外部テーブルから同じデータのコピーをクエリします。データはZLIB圧縮され、ORC形式で保存されています。

StarRocksがネイティブストレージからデータをクエリするためのレイテンシは21秒であり、StarRocksがHive外部テーブルからデータをクエリするためのレイテンシは92秒であり、TrinoがHive外部テーブルからデータをクエリするためのレイテンシは**307秒**です。
...
> StarRocks FEは、独立してデプロイすることも、BEとのハイブリッドデプロイメントを行うこともできます。これはテスト結果に影響しません。

## 3. テストデータと結果

### 3.1 テストデータ

テストにはTPC-H 100GBのデータセットを使用しました。
...
| **クエリ** | **StarRocksネイティブ（ミリ秒）** | **StarRocks-Hive外部（ミリ秒）** | **Trino（ミリ秒）** |
| --------- | ------------------------- | -------------------------------- | -------------- |
| q1        | 1291                      | 5349                             | 14350          |
| q2        | 257                       | 940                              | 4075           |
| q3        | 690                       | 2585                             | 7615           |
...
1. tpch-pocツールキットをダウンロードし、コンパイルします。
    ...
    関連するツールは`bin`ディレクトリに保存されており、ホームディレクトリで操作を開始できます。

2. データを生成します。

    ```Bash
    # 100GBのデータを`data_100`ディレクトリに生成します。
    ./bin/gen_data/gen-tpch.sh 100 data_100
    ```

#### 4.1.2 テーブルスキーマの作成

1. 設定ファイル`conf/starrocks.conf`を編集し、クラスターアドレスを指定します。
    ...
    # parallel_fragment_exec_instance_numは並列度を指定します。
    # 推奨される並列度はクラスターのCPUコア数の半分です。以下の例では8を使用しています。
    parallel_num: 8
    ```
...
2. 以下のスクリプトを実行してテーブルを作成します。
    ...
    テーブルが作成され、デフォルトのバケット数が指定されます。以前のテーブルを削除し、クラスターサイズとノードの設定に基づいてバケット数を再計画し、より良いテスト結果を得ることができます。
