...
GROUP BY `GROUPING SETS` ｜ `CUBE` ｜ `ROLLUP` は、GROUP BY 句の拡張です。これにより、GROUP BY 句内の複数のセットのグループの集計を実現することができます。結果は、複数の対応する GROUP BY 句の UNION 操作と等価です。

GROUP BY 句は、GROUPING SETS を含む GROUP BY GROUPING SETS の特殊なケースです。例えば、GROUPING SETS ステートメント:
