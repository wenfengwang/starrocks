...
本文は、StarRocksがサポートするすべてのSQLコマンドを列挙し、機能別に分類しています。

- [すべてのコマンド](#すべてのコマンド)
  - [ユーザーアカウント管理](#ユーザーアカウント管理)
  - [クラスター管理](#クラスター管理)
    - [FE、BE、CN、ブローカー、プロセス](#febecnbrokerprocess)
    - [リソースグループ](#リソースグループ)
    - [ストレージボリューム](#ストレージボリューム)
    - [テーブル、タブレット、レプリカ](#テーブルタブレットレプリカ)
    - [ファイル、インデックス、変数](#ファイルインデックス変数)
    - [SQLブラックリスト](#sqlブラックリスト)
    - [プラグイン](#プラグイン)
  - [インポート、エクスポート](#インポートエクスポート)
    ...
    - [エクスポート](#エクスポート)
    - [ETLタスク](#etlタスク)
  - [データカタログ (Catalog)、データベース、リソース](#データカタログ-catalogデータベースリソース)
    - [カタログ](#カタログ)
    - [データベース](#データベース)
    - [リソース](#リソース)
  - [テーブル作成、パーティション](#テーブル作成パーティション)
  - [ビュー、マテリアライズドビュー](#ビューマテリアライズドビュー)
    - [ビュー](#ビュー)
    - [マテリアライズドビュー](#マテリアライズドビュー)
  - [関数、SELECT](#関数select)
  - [CBO統計情報](#cbo統計情報)
  - [バックアップとリカバリ](#バックアップとリカバリ)
  - [ツール補助ステートメント](#ツール補助ステートメント)
...
クラスターを管理し、FE、BE、Compute Node (CN)、リソースグループ (Resource Group)、ストレージボリューム(Storage Volume)、テーブル、タブレット、レプリカ (Replica)、ファイル、インデックス(Index)、変数(Variable)、プラグイン(Plugin)などを含みます。
