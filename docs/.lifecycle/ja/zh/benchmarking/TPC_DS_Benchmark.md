...
TPC-DS は、アメリカの Transaction Processing Performance Council (TPC) が策定した、意思決定支援システムを模擬するためのベンチマークテストセットです。TPC-H と比較して、TPC-DS はより複雑で包括的なベンチマークテストセットです。

TPC-DS は意思決定支援システムのベンチマークテストであり、意思決定支援システムのいくつかの一般的な側面をモデル化しています。これには、クエリとデータメンテナンスが含まれ、大規模データ製品の分析性能を測定するために使用されます。TPC-DS は、小売業の3つの販売チャネル（実店舗、インターネット、カタログ）の販売および返品業務をシミュレートし、関連する販売および返品モデルのテーブルを構築するだけでなく、シンプルな在庫管理システムとプロモーションシステムも含まれています。このテストには24のテーブルが含まれ、データ量は1GBから3TBまで設定可能です。ベンチマークテストには99の複雑なクエリが含まれ、主要な評価指標は各クエリの応答時間、つまりクエリを送信してから結果が返されるまでの時間です。

## 1. テスト結果

TPC-DS 100GB のデータセットでの比較テストを実施し、合計99のクエリについて以下の結果が得られました:

![TPC-DS](./../assets/tpc-ds.png)

StarRocks は、ローカルストレージクエリと Hive 外部テーブルクエリの2つの方法でテストを実施しました。StarRocks の Hive 外部テーブルクエリと Trino のクエリは同じデータセットを使用し、データは Parquet 形式で保存され、LZ4 形式で圧縮されています。

最終的に、StarRocks のローカルストレージクエリの総実行時間は 174 秒、StarRocks の Hive 外部テーブルクエリの総実行時間は 239 秒、StarRocks の Hive 外部テーブルクエリで Data Cache を有効にした場合の総実行時間は 176 秒、**Trino のクエリの総実行時間は 892 秒**でした。

**StarRocks のローカルストレージクエリの性能は Trino の 5.1 倍、StarRocks の Hive 外部テーブルクエリは Trino の 3.7 倍、StarRocks の Hive 外部テーブルクエリで Data Cache を有効にした場合の性能は Trino の 5 倍です。**
...
StarRocks と Trino は同じ構成のマシン上でデプロイされ、テストが実施されました。StarRocks は1つの FE と3つの BE で構成され、Trino は1つの Coordinator と3つの Worker で構成されています。

- カーネルバージョン: Linux 3.10.0-1127.13.1.el7.x86_64

- OS バージョン: CentOS Linux release 7.8.2003

- ソフトウェアバージョン: StarRocks コミュニティ版 3.0、Trino-419、Hive-3.1.2
...
>
> - クエリ結果の単位はミリ秒です。
> - `StarRocks-3.0.5-native` は StarRocks のローカルストレージを使用しています。`StarRocks-3.0-Hive external` は Catalog を通じて Hive 外部テーブルをクエリし、`StarRocks-3.0-Hive external-Cache` は Catalog を通じて Hive 外部テーブルをクエリし、StarRocks の Data Cache 機能を有効にしています。
> - StarRocks は集約プッシュダウン最適化を有効にしています `SET global cbo_push_down_aggregate_mode = 0`。

| **クエリ** | **StarRocks-3.0.5-native** | **StarRocks-3.0-Hive 外部** | **StarRocks-3.0-Hive 外部-Cache** | **Trino-419** |
| --------- | -------------------------- | ------------------------------- | ------------------------------------- | ------------- |
...
### 4.1 StarRocks ローカルテーブルテストプロセス

#### 4.1.1 データ生成

tpcds-poc ツールキットをダウンロードして、TPC-DS 標準テストセット `scale factor=100` のデータを生成します。
...
`conf/starrocks.conf` 設定ファイルを変更して、スクリプトが操作するクラスタのアドレスを指定します。`mysql_host` と `mysql_port` に注目し、次にテーブル作成を実行します。
...
### 4.2 StarRocks Hive 外部テーブルテストプロセス

#### 4.2.1 テーブル構造の作成

Hive で外部テーブルを作成します。外部テーブルのストレージフォーマットは Parquet、圧縮フォーマットは LZ4 です。詳細なテーブル作成ステートメントは [5.3 Hive 外部テーブルの作成（Parquet 形式）](#53-hive-外部表建表parquet-格式) を参照してください。この Hive 外部テーブルは StarRocks と Trino の比較テストクエリのテーブルです。

#### 4.2.2 データのインポート

[4.1.1 StarRocks ローカルテーブルテスト](#411-生成数据) で生成した TPC-DS CSV 生データを HDFS の指定パスにアップロードします。この記事では `/user/tmp/csv/` を例としています。次に、Hive で CSV 形式の外部テーブルを作成し、ストレージパスを `/user/tmp/csv/` に設定します。詳細なテーブル作成ステートメントは [5.4 Hive 外部テーブルの作成（CSV 形式）](#54-hive-外部表建表-csv-格式) を参照してください。

次に、INSERT INTO コマンドを使用して CSV 形式の外部テーブルのデータを Parquet 形式の外部テーブルにインポートします。これにより、ストレージフォーマットが Parquet、圧縮フォーマットが LZ4 のデータが得られます。インポートコマンドは以下の通りです：
...
StarRocks 3.0 バージョンは外部テーブルの統計情報の収集をサポートしていません。Hive を使用して列レベルの統計情報を収集します。
...
StarRocks が [Data Cache](../data_source/data_cache.md) 機能を開始するテストシナリオでは、現在のマシン構成とクラスタサイズに基づいて、以下の設定を推奨します。
...
## 5. クエリステートメントとテーブル作成ステートメント

### 5.1 TPC-DS クエリ SQL

99 の詳細なクエリ SQL については、[TPC-DS SQL](./tpc_ds_99_sql.md) を参照してください。

### 5.2 StarRocks Native テーブル作成
