...
where
    d_date between '2002-02-01' and 
           date_add(cast('2002-02-01' as date), 60)
...
with frequent_ss_items as 
 ...
 (select c_customer_sk, sum(ss_quantity*ss_sales_price) ssales
  ...
  having sum(ss_quantity*ss_sales_price) > (50/100.0) * (select * from max_store_sales))
  select  sum(sales)
...
where
 ...
 and ss2.ca_county = ss3.ca_county
    ...
    and ws3.d_qoy = 3
    and ws3.d_year = 2000
    and (ws1.web_sales > 0 ? ws2.web_sales / ws1.web_sales : null) 
       > (ss1.store_sales > 0 ? ss2.store_sales / ss1.store_sales : null)
    and (ws2.web_sales > 0 ? ws3.web_sales / ws2.web_sales : null)
...
and d_date between '2000-01-27' and 
        date_add(cast('2000-01-27' as date), interval 90 day)
 ...
 where i_manufact_id between 738 and 738+40 
   and (select count(*) as item_cnt
       ...
       (i_manufact = i1.i_manufact and
        ...
        (i_color = 'forest' または i_color = 'ghost') かつ
        (i_units = 'Lb' または i_units = 'Bundle') かつ
        (i_size = 'medium' または i_size = 'extra large')
        )))) > 0
...
select  
  ...
  ,s_state
  ,s_zip
  ,sum(case when (sr_returned_date_sk - ss_sold_date_sk <= 30 ) then 1 else 0 end)  as "30日" 
  ,sum(case when (sr_returned_date_sk - ss_sold_date_sk > 30) and 
                 (sr_returned_date_sk - ss_sold_date_sk <= 60) then 1 else 0 end )  as "31-60日" 
  ,sum(case when (sr_returned_date_sk - ss_sold_date_sk > 60) and 
                 (sr_returned_date_sk - ss_sold_date_sk <= 90) then 1 else 0 end)  as "61-90日" 
  ,sum(case when (sr_returned_date_sk - ss_sold_date_sk > 90) and
                 (sr_returned_date_sk - ss_sold_date_sk <= 120) then 1 else 0 end)  as "91-120日" 
  ,sum(case when (sr_returned_date_sk - ss_sold_date_sk  > 120) then 1 else 0 end)  as "120日以上" 
from
   store_sales
  ,store_returns
  ,store
...
select  
  ...
  ,sm_type
  ,web_name
  ,sum(case when (ws_ship_date_sk - ws_sold_date_sk <= 30 ) then 1 else 0 end)  as "30日" 
  ,sum(case when (ws_ship_date_sk - ws_sold_date_sk > 30) and 
                 (ws_ship_date_sk - ws_sold_date_sk <= 60) then 1 else 0 end )  as "31-60日" 
  ,sum(case when (ws_ship_date_sk - ws_sold_date_sk > 60) and 
                 (ws_ship_date_sk - ws_sold_date_sk <= 90) then 1 else 0 end)  as "61-90日" 
  ,sum(case when (ws_ship_date_sk - ws_sold_date_sk > 90) and
                 (ws_ship_date_sk - ws_sold_date_sk <= 120) then 1 else 0 end)  as "91-120日" 
  ,sum(case when (ws_ship_date_sk - ws_sold_date_sk  > 120) then 1 else 0 end)  as ">120日" 
from
   web_sales
  ,warehouse
  ,ship_mode
...
select   
    ...
    ,sum(apr_sales/w_arehouse_sq_ft) as apr_sales_per_sq_foot
    ,sum(may_sales/w_arehouse_sq_ft) as may_sales_per_sq_foot
    ,sum(jun_sales/w_arehouse_sq_ft) as jun_sales_per_sq_foot
    ,sum(jul_sales/w_arehouse_sq_ft) as jul_sales_per_sq_foot
    ,sum(aug_sales/w_arehouse_sq_ft) as aug_sales_per_sq_foot
    ,sum(sep_sales/w_arehouse_sq_ft) as sep_sales_per_sq_foot
    ,sum(oct_sales/w_arehouse_sq_ft) as oct_sales_per_sq_foot
    ,sum(nov_sales/w_arehouse_sq_ft) as nov_sales_per_sq_foot
    ,sum(dec_sales/w_arehouse_sq_ft) as dec_sales_per_sq_foot
...
select  
  ...
  count(*) as cnt3
 from
  customer as c, customer_address as ca, customer_demographics
 where
  c.c_current_addr_sk = ca.ca_address_sk and
...
SELECT channel, col_name, d_year, d_qoy, i_category, COUNT(*) AS sales_cnt, SUM(ext_sales_price) AS sales_amt FROM (
        ...
        SELECT 'catalog' AS channel, 'cs_ship_addr_sk' AS col_name, d_year, d_qoy, i_category, cs_ext_sales_price AS ext_sales_price
         ...
         WHERE cs_ship_addr_sk IS NULL
           AND cs_sold_date_sk=d_date_sk
           AND cs_item_sk=i_item_sk) AS foo
...
FROM
   (SELECT ss_ticket_number
    ...
    GROUP BY ss_ticket_number,ss_customer_sk,store.s_city) AS ms,customer
WHERE ss_customer_sk = c_customer_sk
...
where ss_item_sk = i_item_sk and
      ...
      or (i_category in ('Men','Jewelry','Women') and
          i_class in ('shirts','bridal','dresses') 
        ))
...
-- query 91
select  
        cc_call_center_id コールセンター,
        cc_name コールセンター名,
        cc_manager マネージャー,
        sum(cr_net_loss) 返品損失
from
        call_center,
        catalog_returns,
        date_dim,
...
-- query 92
select  
   sum(ws_ext_discount_amt) as "過剰割引額"
from 
    web_sales 
   ,item 
   ,date_dim
...
-- query 94
select  
   count(distinct ws_order_number) as "注文数"
  ,sum(ws_ext_ship_cost) as "総配送料"
  ,sum(ws_net_profit) as "総純利益"
from
   web_sales ws1
  ,date_dim
  ,customer_address
...
select  
  ...
  ,sm_type
  ,cc_name
  ,sum(case when (cs_ship_date_sk - cs_sold_date_sk <= 30 ) then 1 else 0 end) as "30日以内" 
  ,sum(case when (cs_ship_date_sk - cs_sold_date_sk > 30) and 
                 (cs_ship_date_sk - cs_sold_date_sk <= 60) then 1 else 0 end ) as "31-60日" 

  ,sum(case when (cs_ship_date_sk - cs_sold_date_sk > 60) and 
                 (cs_ship_date_sk - cs_sold_date_sk <= 90) then 1 else 0 end)  as "61-90日" 
  ,sum(case when (cs_ship_date_sk - cs_sold_date_sk > 90) and
                 (cs_ship_date_sk - cs_sold_date_sk <= 120) then 1 else 0 end)  as "91-120日" 
  ,sum(case when (cs_ship_date_sk - cs_sold_date_sk  > 120) then 1 else 0 end)  as ">120日" 
from
   catalog_sales
  ,warehouse
  ,ship_mode
