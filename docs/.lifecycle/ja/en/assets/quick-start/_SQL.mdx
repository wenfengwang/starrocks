...
#### ニューヨークでは1時間あたりにどれくらいの交通事故がありますか？
...
こちらは出力の一部です。1月6日と7日は祝日ではない週の月曜日と火曜日ですので、特に注目しています。元日のデータを見ても、通常のラッシュアワーの朝の状況を反映しているとは言えないでしょう。
...
月曜日や火曜日の朝のラッシュアワーでは約40件の事故が発生しており、17:00頃にも同程度の事故が発生しているようです。
...
これは2014年のデータですが、最近のニューヨークはこれほど寒くはありません。
...
#### 視界が悪い時にニューヨークで運転するのは安全ですか？

視界が悪い（0〜1.0マイル）時の交通事故の件数を見てみましょう。この質問に答えるためには、DATETIMEカラムを基にして2つのテーブルをJOINします。
...
視界が悪い時の1時間あたりの最大事故数は129件です。いくつかの点を考慮する必要があります：

- 2014年2月3日は月曜日でした
- 午前8時はラッシュアワーです
- その時間には雨が降っていました（降水量は0.12インチ）
- 気温は華氏32度（水の凝固点）
- 視界は0.25マイルで悪いですが、ニューヨークの通常の視界は10マイルです
...
水蒸気は華氏40度で氷に昇華することがあります。このクエリでは、華氏0度から40度の温度を見ています。
...
氷点下の気温での結果は少し意外でした。1月の寒い日曜日の朝に市内で交通量が多いとは思いませんでした。[weather.com](https://weather.com/storms/winter/news/northeast-storm-rain-snow-wind)を簡単に見ると、その日は大きな嵐があり、データに見られるように多くの事故が発生していたことが分かります。
