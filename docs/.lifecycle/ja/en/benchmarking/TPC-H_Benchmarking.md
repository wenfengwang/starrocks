...
TPC-Hは、Transaction Processing Performance Council（TPC）によって開発された意思決定支援ベンチマークです。ビジネス指向のアドホッククエリと同時データ変更のスイートで構成されています。
TPC-Hは、実際の本番環境に基づいたモデルを構築し、販売システムのデータウェアハウスをシミュレートするために使用できます。このテストでは、1 GBから3 TBのデータサイズを持つ8つのテーブルを使用します。合計22のクエリがテストされ、主要なパフォーマンス指標は各クエリの応答時間です。これは、クエリが送信されてから結果が返されるまでの期間です。

## 1. テスト結果

TPC-H 100 GBデータセットに対して22のクエリを実行したテストを行いました。以下の図はテスト結果を示しています。
![comparison](../assets/7.2-1.png)

テストでは、StarRocksはネイティブストレージとHive外部テーブルの両方からデータをクエリしました。StarRocksとTrinoはHive外部テーブルから同じデータコピーをクエリしました。データはZLIBで圧縮され、ORC形式で保存されています。

StarRocksがネイティブストレージからデータをクエリする際のレイテンシは21秒、Hive外部テーブルからは92秒、TrinoがHive外部テーブルをクエリする際のレイテンシは**最大で307秒**です。
...
> StarRocks FEは、BEと別々にデプロイすることも、BEと組み合わせてデプロイすることもでき、テスト結果には影響しません。

## 3. テストデータと結果

### 3.1 テストデータ

テストではTPC-H 100 GBデータセットを使用します。

| テーブル    | レコード数     | StarRocksでの圧縮後のデータサイズ |
| -------- | ----------- | ---------------------------------------- |
| customer | 1500万件  | 2.4 GB                                   |
| lineitem | 6億件 | 75 GB                                    |
| nation   | 25件          | 2.2 KB                                   |
| orders   | 1億5000万件 | 16 GB                                    |
| part     | 2億件 | 2.4 GB                                   |
| partsupp | 8億件 | 11.6 GB                                  |
| region   | 5件           | 389 B                                    |
| supplier | 100万件   | 0.14 GB                                  |
...
from (select c_custkey, count(o_orderkey) as c_count
    from customer left outer join orders on c_custkey = o_custkey and o_comment not like '%特別な%リクエスト%'
    group by c_custkey
) as c_orders
group by c_count
...
where
  ...
  and ps_suppkey not in (
    select s_suppkey from supplier where s_comment like '%顧客%苦情%'
  )
...
order by
  cntrycode;
    ...
    if exists revenue0のビューを削除します。
    revenue0 (supplier_no, total_revenue)というビューを作成します。
    select
        l_suppkey,
...
このステップはオプションです。より良いテスト結果を生成するために、完全な統計情報を収集することを推奨します。StarRocksクラスタに接続した後、以下のコマンドを実行してください：
...
1. Hiveで外部テーブルを作成します。テーブル形式はCSVです。CSVテストデータをHDFSのデータストレージディレクトリにアップロードします。この例では、Hive外部テーブルのHDFSデータストレージパスは/user/tmp/csv/です。
...
2. 各Hive外部テーブルのCSVデータをORC形式のHiveテーブルに転送します。これにより、後続のテストのためにStarRocksおよびTrinoでORC形式のHive外部テーブルを作成することができます。
