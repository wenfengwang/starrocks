...
StarRocks提供了丰富的函数集合，以便于数据查询和分析。除了常用函数之外，StarRocks还支持半结构化函数，如ARRAY、JSON、MAP和STRUCT函数。它还支持高阶[Lambda函数](Lambda_expression.md)。如果这些函数无法满足您的业务需求，您可以使用[Java UDF](JAVA_UDF.md)编译函数。
