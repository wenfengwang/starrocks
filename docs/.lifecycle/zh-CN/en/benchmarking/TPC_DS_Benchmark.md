...
TPC-DS 是由事务处理性能委员会（TPC）开发的决策支持基准测试。它使用比 TPC-H 更全面的测试数据集和复杂的SQL查询。

TPC-DS 模拟决策支持系统的几个普适方面，包括查询和数据维护。TPC-DS 旨在为测试和评估零售环境中数据库系统的性能提供全面且真实的工作负载。TPC-DS 基准测试模拟了零售企业三个销售渠道（商店、互联网和目录）的销售和退货数据。除了为销售和退货数据模型创建表之外，它还包括一个简单的库存系统和促销系统。

该基准测试对24张表执行了99个复杂的SQL查询，这些表的数据量从1GB到3GB不等。主要的性能指标是每个查询的响应时间，即从查询提交到返回结果所用的时间。

## 1. 测试结论

我们对一个 TPC-DS 100GB 数据集执行了99个查询的测试。下图显示了测试结果。
![tpc-ds](../assets/tpc-ds.png)

在测试中，StarRocks 从其本地存储和Hive外部表中查询数据。StarRocks 和 Trino 从 Hive 外部表中查询相同的数据副本。数据经过LZ4压缩并以Parquet格式存储。

StarRocks 从其本地存储查询数据的延迟为 **174秒**，从Hive外部表查询数据的延迟为 **239秒**，启用数据缓存功能后，StarRocks 从Hive外部表查询数据的延迟为 **176秒**，Trino 从Hive外部表查询数据的延迟为 **892秒**。
...
StarRocks 和 Trino 部署在具有相同配置的机器上。StarRocks 有1个 FE 和3个 BE 部署。Trino 有1个协调器和3个工作节点部署。

- 内核版本：Linux 3.10.0-1127.13.1.el7.x86_64
- 操作系统版本：CentOS Linux 发行版 7.8.2003
- 软件版本：StarRocks 社区版 3.1，Trino-419，Hive-3.1.2

> StarRocks FE 可以单独部署或与 BE 混合部署，这不会影响测试结果。
...
>
> - 以下表中查询延迟的单位为毫秒。
> - `StarRocks-3.0.5-native` 表示 StarRocks 本地存储，`StarRocks-3.0-Hive external` 表示 StarRocks 通过 Hive Catalog 查询 Hive 外部表，`StarRocks-3.0-Hive external-Cache` 表示启用数据缓存后，StarRocks 通过 Hive Catalog 查询 Hive 外部表。
> - StarRocks 启用了聚合下推（`SET global cbo_push_down_aggregate_mode = 0`）。

| **查询** | **StarRocks-3.0.5-native** | **StarRocks-3.0-Hive external** | **StarRocks-3.0-Hive external-Cache** | **Trino-419** |
| --------- | -------------------------- | ------------------------------- | ------------------------------------- | ------------- |
...
下载tpcds-poc工具包并生成标准的TPC-DS测试数据集`规模因子=100`。
...
在Hive中创建外部表，其存储格式为Parquet，压缩格式为LZ4。有关详细的CREATE TABLE语句，请参见[创建Parquet Hive外部表](#53-create-a-hive-external-tableparquet)。StarRocks和Trino从这些外部表中查询数据。

#### 4.2.2 加载数据

将[4.1.1](#411-generate-data)生成的CSV数据加载到Hive的指定HDFS路径。本示例使用`/user/tmp/csv/`作为HDFS路径。在Hive中创建CSV格式的外部表，并将其存储在`/user/tmp/csv/`。有关详细的CREATE TABLE语句，请参见[创建CSV Hive外部表](#54-create-a-hive-external-tablecsv)。

使用INSERT INTO将数据从CSV外部表加载到Parquet外部表。这会生成经过LZ4压缩的Parquet数据。
...
StarRocks v3.0不支持从外部表收集统计信息。因此，我们使用Hive来收集列级统计信息。
...
StarRocks使用[Hive catalog](../data_source/catalog/hive_catalog.md)来查询Hive外部表。

如果在测试期间为StarRocks启用了[数据缓存](../data_source/data_cache.md)，建议为数据缓存功能使用以下配置。
...
## 5. 查询语句和CREATE TABLE语句

### 5.1 TPC-DS SQL查询

有关99个SQL查询的更多信息，请参见[TPC-DS SQL](./tpc_ds_99_sql.md)。

### 5.2 创建一个StarRocks本地表
