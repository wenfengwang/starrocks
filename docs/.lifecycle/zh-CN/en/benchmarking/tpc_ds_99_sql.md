...
选择   
 ...
 和 d2.d_year              = 1999
 和 sr_customer_sk         = cs_bill_customer_sk
...
with my_customers as (
 ...
 , segments as
(选择将(revenue/50)强制转换为int) as segment
  来自my_revenue
 )
  选择 segment, 计算(*)作为num_customers, segment*50 as segment_base
 来自segments
 分组按segment
 排序按segment, num_customers
 限制100;


- 查询55
选择i_brand_id作为brand_id, i_brand作为brand,
    求和(ss_ext_sales_price)作为ext_price
 来自date_dim, store_sales, item
 其中d_date_sk = ss_sold_date_sk
    并且ss_item_sk = i_item_sk
    并且i_manager_id=28
    并且d_moy=11
    并且d_year=1999
 分组按i_brand, i_brand_id
 排序按ext_price降序, i_brand_id
限制 100 ;


- 查询56
作为ss
 选择i_item_id,求和(ss_ext_sales_price)作为total_sales
 从
    store_sales,
    date_dim,
         customer_address,
 ...
 其中i_item_id在(选择
     i_item_id
从item
其中i_color在('slate', 'blanched', 'burnished'))
 并且ss_item_sk              = i_item_sk
 并且ss_sold_date_sk         = d_date_sk
 并且d_year                  = 2001
 并且d_moy                   = 2
 并且ss_addr_sk              = ca_address_sk
 并且ca_gmt_offset           = -5 
 分组按i_item_id),
 作为cs
 选择i_item_id,求和(cs_ext_sales_price)作为total_sales
 从
    catalog_sales,
    date_dim,
         customer_address,
...
从item
其中i_color在('slate', 'blanched', 'burnished'))
 并且cs_item_sk              = i_item_sk
 并且cs_sold_date_sk         = d_date_sk
 并且d_year                  = 2001
 并且d_moy                   = 2
 并且cs_bill_addr_sk         = ca_address_sk
 并且ca_gmt_offset           = -5 
 分组按i_item_id),
 作为ws
 选择i_item_id,求和(ws_ext_sales_price)作为total_sales
 从
    web_sales,
    date_dim,
         customer_address,
...
从item
其中i_color在('slate', 'blanched', 'burnished'))
 并且ws_item_sk              = i_item_sk
 并且ws_sold_date_sk         = d_date_sk
 并且d_year                  = 2001
 并且d_moy                   = 2
 并且ws_bill_addr_sk         = ca_address_sk
 并且ca_gmt_offset           = -5
 分组按i_item_id)
 选择i_item_id,求和(total_sales)作为total_sales
 从  (选择 * 从ss 
        求并集全部
        选择 * 从cs 
        求并集全部
...
- 查询57
作为v1
 选择i_category, i_brand,
        cc_name,
        d_year, d_moy,
        求和(cs_sales_price)作为sum_sales,
        avg(求和(cs_sales_price)) over
          (根据i_category, i_brand,
                     cc_name, d_year分区)
          avg_monthly_sales,
        rank() over
          (根据i_category, i_brand,
                     cc_name排序
           按d_year, d_moy) rn
 从item, catalog_sales, date_dim, call_center
 其中cs_item_sk = i_item_sk 并且
       cs_sold_date_sk = d_date_sk 并且
       cc_call_center_sk= cs_call_center_sk 并且
       (
         d_year = 1999 或
         ( d_year = 1999-1 并且 d_moy =12) 或
         ( d_year = 1999+1 并且 d_moy =1)
       )
 分组按i_category, i_brand,
          cc_name , d_year, d_moy),
 作为v2
 选择v1.i_category, v1.i_brand, v1.cc_name
        ,v1.d_year, v1.d_moy
        ,v1.avg_monthly_sales
        ,v1.sum_sales, v1_lag.sum_sales作为psum, v1_lead.sum_sales作为nsum
 从v1, v1 v1_lag, v1 v1_lead
 其中v1.i_category = v1_lag.i_category 并且
       v1.i_category = v1_lead.i_category 并且
       v1.i_brand = v1_lag.i_brand 并且
       v1.i_brand = v1_lead.i_brand 并且
       v1. cc_name = v1_lag. cc_name 并且
       v1. cc_name = v1_lead. cc_name 并且
       v1.rn = v1_lag.rn + 1 并且
       v1.rn = v1_lead.rn - 1
 选择* 
 从v2
  其中d_year = 1999 并且
        avg_monthly_sales > 0 并且
        case when avg_monthly_sales > 0 then abs(sum_sales - avg_monthly_sales) / avg_monthly_sales else null end > 0.1
 按sum_sales - avg_monthly_sales, cc_name排序
 限制100;


- 查询58
作为ss_items
 (选择i_item_id作为item_id
        ,求和(ss_ext_sales_price)作为ss_item_rev 
 从store_sales
     ,item
     ,date_dim
 其中ss_item_sk = i_item_sk 并且
 d_date在(选择d_date
                  从date_dim
                  其中d_week_seq = (选择d_week_seq 
                                      从date_dim
                                      其中d_date = '2000-01-03'))
 并且ss_sold_date_sk   = d_date_sk
 分组按i_item_id),
 作为cs_items
 (选择i_item_id作为item_id
        ,求和(cs_ext_sales_price)作为cs_item_rev
 从catalog_sales
      ,item
      ,date_dim
 其中cs_item_sk = i_item_sk
  并且d_date在(选择d_date
                  从date_dim
                  其中d_week_seq = (选择d_week_seq 
                                      从date_dim
 ...
 作为ws_items
 (选择i_item_id作为item_id
        ,求和(ws_ext_sales_price)作为ws_item_rev
 从web_sales
      ,item
      ,date_dim
 其中ws_item_sk = i_item_sk
  并且d_date在(选择d_date
                  从date_dim
                  其中d_week_seq =(选择d_week_seq 
                                     从date_dim
 ...
 选择ss_items.item_id
       ,ss_item_rev
       ,ss_item_rev/((ss_item_rev+cs_item_rev+ws_item_rev)/3) * 100作为ss_dev
       ,cs_item_rev
       ,cs_item_rev/((ss_item_rev+cs_item_rev+ws_item_rev)/3) * 100作为cs_dev
       ,ws_item_rev
       ,ws_item_rev/((ss_item_rev+cs_item_rev+ws_item_rev)/3) * 100作为ws_dev
       ,(ss_item_rev+cs_item_rev+ws_item_rev)/3作为average
 从ss_items,cs_items,ws_items
 其中ss_items.item_id=cs_items.item_id
   并且ss_items.item_id=ws_items.item_id 
   并且ss_item_rev在0.9 * cs_item_rev 和 1.1 * cs_item_rev
   并且ss_item_rev在0.9 * ws_item_rev 和 1.1 * ws_item_rev
   并且cs_item_rev在0.9 * ss_item_rev 和 1.1 * ss_item_rev
   并且cs_item_rev在0.9 * ws_item_rev 和 1.1 * ws_item_rev
   并且ws_item_rev在0.9 * ss_item_rev 和 1.1 * ss_item_rev
   并且ws_item_rev在0.9 * cs_item_rev 和 1.1 * cs_item_rev
 按item_id
         ,ss_item_rev排序
 限制100;
...
作为wss
 (选择d_week_seq,
        ss_store_sk,
        求和(case when (d_day_name='Sunday') then ss_sales_price else null end)作为sun_sales,
        求和(case when (d_day_name='Monday') then ss_sales_price else null end)作为mon_sales,
        求和(case when (d_day_name='Tuesday') then ss_sales_price else  null end)作为tue_sales,
        求和(case when (d_day_name='Wednesday') then ss_sales_price else null end)作为wed_sales,
        求和(case when (d_day_name='Thursday') then ss_sales_price else null end)作为thu_sales,
        求和(case when (d_day_name='Friday') then ss_sales_price else null end)作为fri_sales,
        求和(case when (d_day_name='Saturday') then ss_sales_price else null end)作为sat_sales
 从store_sales,date_dim
 其中d_date_sk = ss_sold_date_sk
 分组按d_week_seq,ss_store_sk
 )
 选择s_store_name1,s_store_id1,d_week_seq1
...
from (选择 ss_ticket_number
      ...
      从 store_sales
           ...
           ,household_demographics
           ,customer_address 
      在 store_sales.ss_sold_date_sk = date_dim.d_date_sk
        和 store_sales.ss_store_sk = store.s_store_sk  
        和 store_sales.ss_hdemo_sk = household_demographics.hd_demo_sk
        和 store_sales.ss_addr_sk = customer_address.ca_address_sk
        和 date_dim.d_dom 在 1 和 2 之间 
        和 (household_demographics.hd_dep_count = 4 或
             household_demographics.hd_vehicle_count= 3)
        和 date_dim.d_year 在 (1999,1999+1,1999+2)
        和 store.s_city 在 ('Fairview','Midway')
       按 ss_ticket_number
               ,ss_customer_sk
               ,ss_addr_sk,ca_city) dn
      ,customer
      ,customer_address current_addr
 在 ss_customer_sk = c_customer_sk
...
选择  
 ...
 在
  ...
  存在 (选择 *
   ...
   (不存在 (选择 *
            从 web_sales,date_dim
            其中 c.c_customer_sk = ws_bill_customer_sk 和
                  ws_sold_date_sk = d_date_sk 和
                  d_year = 2001 和
                  d_moy 在 4 和 4+2) 和
    不存在 (选择 * 
            从 catalog_sales,date_dim
            其中 c.c_customer_sk = cs_ship_customer_sk 和
                  cs_sold_date_sk = d_date_sk 和
                  d_year = 2001 和
                  d_moy 在 4 和 4+2))
 按 cd_gender,
...
选择  
   ...
   ,rank() over (
    分区 按 grouping(s_state)+grouping(s_county),
    case when grouping(s_county) = 0 then s_state end 
    按 sum(ss_net_profit) 降序) as rank_within_parent
 从
    store_sales
   ,date_dim       d1
   ,store
 ...
 和 s_state 在
             ( 选择 s_state
               从  (选择 s_state as s_state,
                rank() over ( 分区 按 s_state 按 sum(ss_net_profit) 降序) as ranking
                      从   store_sales, store, date_dim
                      其中  d_month_seq 在 1200 和 1200+11 之间
                和 d_date_sk = ss_sold_date_sk
                和 s_store_sk  = ss_store_sk
                      按 s_state
                     ) tmp1 
               其中 ranking <= 5
             )
 按 rollup(s_state,s_county)
 按
...
-- 查询 71
选择 i_brand_id brand_id, i_brand brand,t_hour,t_minute,
    sum(ext_price) ext_price
 从 item, (选择 ws_ext_sales_price as ext_price, 
                        ws_sold_date_sk as sold_date_sk,
                        ws_item_sk as sold_item_sk,
                        ws_sold_time_sk as time_sk  
                 从 web_sales,date_dim
                 其中 d_date_sk = ws_sold_date_sk
                   和 d_moy=11
                   和 d_year=1999
                 联合全部
                 (选择 cs_ext_sales_price as ext_price,
                        cs_sold_date_sk as sold_date_sk,
                        cs_item_sk as sold_item_sk,
                        cs_sold_time_sk as time_sk
                 从 catalog_sales,date_dim
                 其中 d_date_sk = cs_sold_date_sk
                   和 d_moy=11
                   和 d_year=1999
                 联合全部
                 (选择 ss_ext_sales_price as ext_price,
 ...
 在
   ...
   和 time_sk = t_time_sk
   和 (t_meal_time = 'breakfast' 或 t_meal_time = 'dinner')
按 i_brand, i_brand_id,t_hour,t_minute
...
选择  i_item_desc
      ...
      ,计数(*) total_cnt
从 catalog_sales
加入 inventory on (cs_item_sk = inv_item_sk)
加入 warehouse on (w_warehouse_sk=inv_warehouse_sk)
加入 item on (i_item_sk = cs_item_sk)
加入 customer_demographics on (cs_bill_cdemo_sk = cd_demo_sk)
加入 household_demographics on (cs_bill_hdemo_sk = hd_demo_sk)
加入 date_dim d1 on (cs_sold_date_sk = d1.d_date_sk)
加入 date_dim d2 on (inv_date_sk = d2.d_date_sk)
加入 date_dim d3 on (cs_ship_date_sk = d3.d_date_sk)
左外连接 promotion on (cs_promo_sk=p_promo_sk)
左外连接 catalog_returns on (cr_item_sk = cs_item_sk 和 cr_order_number = cs_order_number)
在 d1.d_week_seq = d2.d_week_seq
  和 inv_quantity_on_hand < cs_quantity 
  和 d3.d_date > d1.d_date + 5


  和 hd_buy_potential = '>10000'
  和 d1.d_year = 1999
  和 cd_marital_status = 'D'
按 i_item_desc,w_warehouse_name,d1.d_week_seq
按 total_cnt 降序, i_item_desc, w_warehouse_name, d_week_seq
limit 100;
...
选择 c_last_name
   ...
   (选择 ss_ticket_number
    ...
    和 (household_demographics.hd_buy_potential = '>10000' 或
         household_demographics.hd_buy_potential = 'Unknown')
    和 household_demographics.hd_vehicle_count > 0
    和 case when household_demographics.hd_vehicle_count > 0 then 


             household_demographics.hd_dep_count/ household_demographics.hd_vehicle_count else null end > 1
使用年度总计作为 (
 选择 c_customer_id 客户id
      ,c_first_name 客户名
      ,c_last_name 客户姓
      ,d_year 年份
      ,sum(ss_net_paid) 年度总计
      ,'s' 销售类型
 从 客户
    ,店铺销售
    ,日期维度
 其中 c_customer_sk = ss_customer_sk
  和 ss_sold_date_sk = d_date_sk
  和 d_year 在 (2001,2001+1)
 组别 以 c_customer_id
       ,c_first_name
       ,c_last_name
       ,d_year
 ...
 选择 c_customer_id 客户id
      ...
      ,c_last_name 客户姓
      ,d_year 年份
      ,sum(ws_net_paid) 年度总计
      ,'w' 销售类型
 从 客户
    ,网上销售
    ,日期维度
 其中 c_customer_sk = ws_bill_customer_sk
  和 ws_sold_date_sk = d_date_sk
  和 d_year 在 (2001,2001+1)
 组别 以 c_customer_id
       ,c_first_name
 ...
 从 年度总计 t_s_firstyear
    ,年度总计 t_s_secyear
    ,年度总计 t_w_firstyear
    ,年度总计 t_w_secyear
 其中 t_s_secyear.customer_id = t_s_firstyear.customer_id
         和 t_s_firstyear.customer_id = t_w_secyear.customer_id
         和 t_s_firstyear.customer_id = t_w_firstyear.customer_id
         和 t_s_firstyear.sale_type = 's'
         和 t_w_firstyear.sale_type = 'w'
         和 t_s_secyear.sale_type = 's'
         和 t_w_secyear.sale_type = 'w'
         和 t_s_firstyear.year = 2001
         和 t_s_secyear.year = 2001+1
         和 t_w_firstyear.year = 2001
         和 t_w_secyear.year = 2001+1
         和 t_s_firstyear.year_total > 0
         和 t_w_firstyear.year_total > 0
         和 case 当 t_w_firstyear.year_total > 0 否则 t_w_secyear.year_total / t_w_firstyear.year_total 结束
           > case 当 t_s_firstyear.year_total > 0 否则 t_s_secyear.year_total / t_s_firstyear.year_total 结束
 按 1, 1, 1
限制 100;
...
使用 全部销售 作为 (
 选择 d_year
       ...
       ,i_category_id
       ,i_manufact_id
       ,SUM(sales_cnt) 作为 销售数量
       ,SUM(sales_amt) 作为 销售金额
 从 (选择 d_year
             ,i_brand_id
             ,i_class_id
             ,i_category_id
             ,i_manufact_id
             ,cs_quantity - coalesce(cr_return_quantity,0) 作为 sales_cnt
             ,cs_ext_sales_price - coalesce(cr_return_amount,0.0) 作为 sales_amt
       从 目录销售 加入 商品 ON i_item_sk=cs_item_sk
                          加入 日期维度 ON d_date_sk=cs_sold_date_sk
                          左连接 目录退货 ON (cs_order_number=cr_order_number 
       ...
       选择 d_year
             ...
             ,i_category_id
             ,i_manufact_id
             ,ss_quantity - coalesce(sr_return_quantity,0) 作为 sales_cnt
             ,ss_ext_sales_price - coalesce(sr_return_amt,0.0) 作为 sales_amt
       从 店铺销售 加入 商品 ON i_item_sk=ss_item_sk
                        加入 日期维度 ON d_date_sk=ss_sold_date_sk
                        左连接 店铺退货 ON (ss_ticket_number=sr_ticket_number 
                                                和 ss_item_sk=sr_item_sk)
       其中 i_category='Books'
       联合
       选择 d_year
             ...
             ,i_category_id
             ,i_manufact_id
             ,ws_quantity - coalesce(wr_return_quantity,0) 作为 sales_cnt
             ,ws_ext_sales_price - coalesce(wr_return_amt,0.0) 作为 sales_amt
       从 网上销售 加入 商品 ON i_item_sk=ws_item_sk
                      加入 日期维度 ON d_date_sk=ws_sold_date_sk
                      左连接 网上退货 ON (ws_order_number=wr_order_number 
                                            和 ws_item_sk=wr_item_sk)
       其中 i_category='Books') sales_detail
 组别 以 d_year, i_brand_id, i_class_id, i_category_id, i_manufact_id)
 选择  prev_yr.d_year 作为 上一年
                          ,curr_yr.d_year 作为 年份
                          ,curr_yr.i_brand_id
                          ,curr_yr.i_class_id
                          ,curr_yr.i_category_id
                          ,curr_yr.i_manufact_id
                          ,prev_yr.sales_cnt 作为 上一年销售数量
                          ,curr_yr.sales_cnt 作为 当年销售数量
                          ,curr_yr.sales_cnt-prev_yr.sales_cnt 作为 销售数量差额
                          ,curr_yr.sales_amt-prev_yr.sales_amt 作为 销售金额差额
 从 全部销售 curr_yr, 全部销售 prev_yr
 其中 curr_yr.i_brand_id=prev_yr.i_brand_id
   和 curr_yr.i_class_id=prev_yr.i_class_id
   和 curr_yr.i_category_id=prev_yr.i_category_id
   和 curr_yr.i_manufact_id=prev_yr.i_manufact_id
   和 curr_yr.d_year=2002
   和 prev_yr.d_year=2002-1
   和 将(curr_yr.sales_cnt AS DECIMAL(17,2))/将(prev_yr.sales_cnt AS DECIMAL(17,2))<0.9
 按 销售数量差额,销售金额差额
限制 100;


-- 查询 76
选择  渠道, 列名称, d_year, d_qoy, i_category, 计数(*) 销售数量, 总和(ext_sales_price) 销售金额 从 (
        选择 'store' 作为 渠道, 'ss_store_sk' 作为 列名称, d_year, d_qoy, i_category, ss_ext_sales_price 作为 销售金额
         从 店铺销售, 商品, 日期维度
         其中 ss_store_sk 为空
           和 ss_sold_date_sk=d_date_sk
           和 ss_item_sk=i_item_sk
        联合全部
        选择 'web' 作为 渠道, 'ws_ship_customer_sk' 作为 列名称, d_year, d_qoy, i_category, ws_ext_sales_price 作为 销售金额
         从 网上销售, 商品, 日期维度
         其中 ws_ship_customer_sk 为空
           和 ws_sold_date_sk=d_date_sk
           和 ws_item_sk=i_item_sk
        联合全部
        选择 'catalog' 作为 渠道, 'cs_ship_addr_sk' 作为 列名称, d_year, d_qoy, i_category, cs_ext_sales_price 作为 销售金额
         从 目录销售, 商品, 日期维度
         其中 cs_ship_addr_sk 为空
           和 cs_sold_date_sk=d_date_sk
           和 cs_item_sk=i_item_sk) foo
组别 以 渠道, 列名称, d_year, d_qoy, i_category
按 渠道, 列名称, d_year, d_qoy, i_category
限制 100;
...
用 ss 作为 (
 选择 s_store_sk,
         sum(ss_ext_sales_price) 作为 销售总额,
         sum(ss_net_profit) 作为 利润
 从 店铺销售, 日期维度, 店铺
 其中 ss_sold_date_sk = d_date_sk
       和 d_date 在 将('2000-08-23' 作为 日期) 
                  和 加日期(cast('2000-08-23' 作为 日期),  30)
       和 ss_store_sk = s_store_sk
 组别 以 s_store_sk)
 ,
 sr 作为
 (选择 s_store_sk,
         sum(sr_return_amt) 作为 退货金额,
         sum(sr_net_loss) 作为 利润损失
 从 店铺退货, 日期维度, 店
 其中 sr_returned_date_sk = d_date_sk
       和 d_date 在 将('2000-08-23' 作为 日期)
                  和 加日期(cast('2000-08-23' 作为 日期),  30)
       和 sr_store_sk = s_store_sk
 组别 以 s_store_sk), 
 cs 作为
 (选择 cs_call_center_sk,
        sum(cs_ext_sales_price) 作为 销售总额,
        sum(cs_net_profit) 作为 利润
 从 目录销售, 日期维度
 其中 cs_sold_date_sk = d_date_sk
       和 d_date 在 将('2000-08-23' 作为 日期)
                  和 加日期(cast('2000-08-23' 作为 日期),  30)
 组别 以 cs_call_center_sk 
 ), 
 cr 作为
 (选择 cr_call_center_sk,
         sum(cr_return_amount) 作为 退货金额,
         sum(cr_net_loss) 作为 利润损失
 从 目录退货, 日期维度
 其中 cr_returned_date_sk = d_date_sk
       和 d_date 在 将('2000-08-23' 作为 日期)
                  和 加日期(cast('2000-08-23' 作为 日期),  30)
 组别 以 cr_call_center_sk
 ), 
 ws 作为
 ( 选择 wp_web_page_sk,
        sum(ws_ext_sales_price) 作为 销售金额,
...
-- 查询 91
选择  
        cc_call_center_id 呼叫中心,
        cc_name 呼叫中心名称,
        cc_manager 经理,
        sum(cr_net_loss) 退货损失
从
        呼叫中心,
        目录退货,
        日期维度,
...
-- 查询 92
选择  
   sum(ws_ext_discount_amt) 作为 "超额折扣金额" 
从 
    web_sales 
   ,商品 
   ,日期维度
...
和 d_date 在 '2000-01-27' 和 
        date_add(cast('2000-01-27' as date), 90) 之间
...
从 (选择 ss_item_sk
                  ,ss_ticket_number
                  ,ss_customer_sk
                  ,case when sr_return_quantity 不是 null 则 (ss_quantity-sr_return_quantity)*ss_sales_price
                                                            否则 (ss_quantity*ss_sales_price) end act_sales
            从 store_sales 左外连接 store_returns on (sr_item_sk = ss_item_sk
                                                               和 sr_ticket_number = ss_ticket_number)
                ,reason
...
-- 查询 94
选择  
   count(distinct ws_order_number) as "订单数量"
  ,sum(ws_ext_ship_cost) as "总运费成本"
  ,sum(ws_net_profit) as "总净利润"
从
   web_sales ws1
  ,日期维度
  ,customer_address
  ,网站
其中
    d_date 在 '1999-2-01' 和 
           date_add(cast('1999-2-01' as date), 60) 之间
...
和 存在 (选择 *
            ...
            其中 ws1.ws_order_number = ws2.ws_order_number
              和 ws1.ws_warehouse_sk 不等于 ws2.ws_warehouse_sk)
和 不存在(选择 *
               从 web_returns wr1
               其中 ws1.ws_order_number = wr1.wr_order_number)
按 count(distinct ws_order_number) 降序排列
限制 100;
...
csci 作为(
 选择 cs_bill_customer_sk customer_sk
      ,cs_item_sk item_sk
从 catalog_sales,日期维度
其中 cs_sold_date_sk = d_date_sk
  和 d_month_seq 在 1200 和 1200 + 11 之间
按 cs_bill_customer_sk
        ,cs_item_sk 分组)
选择  sum(case when ssci.customer_sk 不是 null 和 csci.customer_sk 是 null 则 1 否则 0) 仅商店
      ,sum(case when ssci.customer_sk 是 null 和 csci.customer_sk 不是 null 则 1 否则 0) 仅目录
      ,sum(case when ssci.customer_sk 不是 null 和 csci.customer_sk 不是 null 则 1 否则 0) 商店和目录
从 ssci 全外连接 csci on (ssci.customer_sk=csci.customer_sk
...
选择 i_item_id
      ...
      ,sum(ss_ext_sales_price)*100/sum(sum(ss_ext_sales_price)) 在
          (按 i_class 分区) 之内作为 revenueratio
从
        store_sales
        ,商品 
        ,日期维度
其中 
    ...
    和 i_category 在 ('Sports', 'Books', 'Home') 内
    和 ss_sold_date_sk = d_date_sk
    和 d_date 在 cast('1999-02-22' as date) 
            和 date_add(cast('1999-02-22' as date), 30) 之间
按 
    i_item_id
        ,i_item_desc 
        ,i_category
...
选择  
  ...
  ,sm_type
  ,cc_name
  ,sum(case when (cs_ship_date_sk - cs_sold_date_sk <= 30 ) 则 1 否则 0) 作为 "30 天"
```
