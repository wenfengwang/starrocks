...
共位 Join 功能是分布式系统实现 Join 数据分布的策略之一，能够减少数据多节点分布时 Join 操作引起的数据移动和网络传输，从而提高查询性能。

在 StarRocks 中使用共位 Join 功能，您需要在建表时为其指定一个共位组（CG），同一 CG 内的表需遵循相同的共位组模式（CGS），即表对应的分桶副本具有一致的分桶键、副本数量和副本放置方式。如此可以保证同一 CG 内，所有表的数据分布在相同一组 BE 节点上。当 Join 列为分桶键时，计算节点只需做本地 Join，从而减少数据在节点间的传输耗时，提高查询性能。因此，共位 Join，相对于其他 Join，例如 Shuffle Join 和 Broadcast Join，可以有效避免数据网络传输开销，提高查询性能。
...
在建表时，您需要在 PROPERTIES 中指定属性 `"colocate_with" = "group_name"` 以创建一个共位 Join 表，并且指定其归属于特定的共位组。

> 说明
>
> 自 2.5.4 起，支持了对不同 Database 中的表执行共位 Join，您只需要在建表时指定相同的 `colocate_with` 属性即可。
...
如果指定的 CG 不存在，StarRocks 会自动创建一个只包含当前表的 CG，并指定当前表为该 CG 的主表。如果 CG 已存在，StarRocks 会检查当前表是否满足 CGS。如果满足，StarRocks 会创建该表，并将该表加入组。同时，StarRocks 会根据已存在的组中的数据分布规则为当前表创建分片和副本。

一个组归属于一个数据库。组名在一个数据库内唯一，在内部存储中，组的全名为 `dbId_groupName`，但您只感知 `groupName`。
> 说明
>
> 如果您为不同数据库的表指定了相同的共位组，以保证这些表互相共位，则该共位组都会存在每个数据库中，您可以通过执行 `show proc "/colocation_group"` 查看每个数据库包含的共位组信息。
...
* CG 中所有表的 Bucket Seq 和 BE 节点的映射关系和主表一致。
* 主表中所有分区的 Bucket Seq 和 BE 节点的映射关系和第一个分区一致。
* 主表第一个分区的 Bucket Seq 和 BE 节点的映射关系利用原生的 Round Robin 算法决定。

CG 内表的一致的数据分布定义和子表副本映射，能够保证分桶键取值相同的数据行一定在相同 BE 节点上，因此当分桶键做 Join 列时，只需本地 Join 即可。

### 删除共位组

当组中最后一张表彻底删除后（彻底删除是指从回收站中删除。通常，一张表通过 `DROP TABLE` 命令被删除后，会在回收站默认停留一天的时间后，再被彻底删除），该组也会被自动删除。

### 查看组信息

您可以通过以下命令查看集群内已存在的组信息。只有拥有 `root` 角色的用户才可以查看，不支持普通用户查看。
...
|列名|描述|
|----|----|
|GroupId|一个组的全集群唯一标识。前半部分为数据库 ID，后半部分为 **组 ID**。|
|GroupName|组的全名。|
|TabletIds|该组包含的表的 ID 列表。|
|BucketsNum|分桶数。|
|ReplicationNum|副本数。|
|DistCols|分布列，即分桶列类型。|
|IsStable|该组是否[稳定](#colocation-副本均衡和修复)。|

<br/>

您可以通过以下命令进一步查看特定组的数据分布情况。
...
以上示例中，HASH JOIN 节点显示了 Colocate Join 未生效以及相应原因：`局部性置换：false，原因：组不稳定`。同时 StarRocks 生成了一个 EXCHANGE 节点。
...
`disable_colocate_join`：是否在会话粒度关闭 Colocate Join 功能。默认为 `false`，即不关闭。
...
StarRocks 提供了多个与 Colocate Join 相关的 HTTP Restful API，用于查看和修改 Colocation 组。

该 API 在 FE 端实现，您可以使用 `fe_host:fe_http_port` 访问。访问需要 `cluster_admin` 角色对应的权限。
...
3. 设置 Group 的数据分布。
    ...
    该接口可以强制设置某个 Group 的数据分布。

    > 注意：
    > 使用该命令时，需要将 FE 的配置 `tablet_sched_disable_colocate_balance` 设置为 `true`，即关闭系统自动的 Colocation 副本修复和均衡。否则在修改数据分布设置之后可能会被系统自动重置。
    ...
    其中请求正文是以嵌套数组表示的 Bucket 序列以及每个 bucket 中片段所在的 BE 的 ID。