...
2. 将 Hive 表 `source_table` 中第一批数据的 `order_uuid` 值（STRING 类型）插入到字典表 `dict`的主键 `order_uuid`（STRING 类型）中，则字典表的自增列 `order_id_int` （BIGINT 类型）会自动为每个 `order_uuid` 生成一个表内全局唯一的 ID，如此就建立 STRING 和 BIGINT 值之间的映射关系。值得注意的是，插入时需要使用 Left Join 子句和 WHERE 条件 `WHERE dict.order_uuid IS NULL`，以确保插入的 `order_uuid` 值原先并不存在于字典表中。如果向字典表中重复插入已存在的 `order_uuid` 值，则该 `order_uuid` 值映射为 BIGINT 值`order_id_int`会变化。因为字典表为主键模型的表，INSERT INTO 相同的主键后会覆盖字典表中原先已存在的行，同时对应自增列的值也会变化（注意 INSERT INTO 目前还不支持部分列更新）。
...
- 如果使用**解决方案一**，则需要向聚合模型表`dest_table_bitmap`的`id`列插入`hive_catalog.hive_db.source_table`表的`id`列的数据；向聚合模型表的`order_id_bitmap`列插入字典表`dict`的INTEGER列`order_id_int`的数据（经过函数`to_bitmap`处理后的值）。
...
- 如果使用**解决方案二**，则需要向聚合模型表`dest_table_bitmap`的`id`列插入表`dest_table_pk`的列`id`的数据；向聚合模型表的`order_id_bitmap`列插入字典表`dict`的INTEGER列`order_id_int`的数据（经过函数`to_bitmap`处理后的值）。
...
3. 然后基于bitmap列使用函数`BITMAP_UNION_COUNT()`精确去重计数。
