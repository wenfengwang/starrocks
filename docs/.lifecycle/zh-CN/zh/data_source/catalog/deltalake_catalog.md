...
Delta Lake Catalog supports Microsoft Azure Storage starting from version 3.0.

###### Azure Blob Storage

If you choose Blob Storage as the file storage for the Delta Lake cluster, configure the `StorageCredentialParams` as follows:

- Authenticate and authorize based on Shared Key
  ...
  The `StorageCredentialParams` includes the following parameters.

  | **Parameter**              | **Required** | **Description**               |
  | -------------------------- | ------------ | ------------------------------ |
  | azure.blob.storage_account | Yes          | The username of the Blob Storage account. |
  | azure.blob.shared_key      | Yes          | The Shared Key of the Blob Storage account. |

- Authenticate and authorize based on SAS Token
  ...
  | **Parameter**             | **Required** | **Description**                        |
  | ------------------------ | ------------ | ----------------------------------- |
  | azure.blob.account_name   | Yes          | The username of the Blob Storage account.              |
  | azure.blob.container_name | Yes          | The name of the container where the data is stored. |
  | azure.blob.sas_token      | Yes          | The SAS Token for accessing the Blob Storage account. |

###### Azure Data Lake Storage Gen1

If you choose Data Lake Storage Gen1 as the file storage for the Delta Lake cluster, configure the `StorageCredentialParams` as follows:

- Authenticate and authorize based on Managed Service Identity
  ...
  | **Parameter**                             | **Required** | **Description**                                         |
  | ---------------------------------------- | ------------ | ------------------------------------------------------ |
  | azure.adls1.use_managed_service_identity | Yes          | Specify whether to enable Managed Service Identity authentication. Set to `true`. |

- Authenticate and authorize based on Service Principal
  ...
  | azure.adls1.oauth2_credential | Yes          | The newly created Client (Application) Secret.              |
  | azure.adls1.oauth2_endpoint   | Yes          | The OAuth 2.0 Token Endpoint (v1) for the Service Principal or Application. |

###### Azure Data Lake Storage Gen2

If you choose Data Lake Storage Gen2 as the file storage for the Delta Lake cluster, configure the `StorageCredentialParams` as follows:

- Authenticate and authorize based on Managed Identity
  ...
  | **Parameter**                     | **Required** | **Description**                                     |
  | ------------------------------- | ------------ | ------------------------------------------------ |
  | azure.adls2.oauth2_use_managed_identity | Yes          | Specify whether to enable Managed Identity authentication. Set to `true`. |
  | azure.adls2.oauth2_tenant_id        | Yes          | The ID of the tenant to which the data belongs.       |
...
- Authenticate and authorize based on Shared Key
  ...
  | **Parameter**                 | **Required** | **Description**                                        |
  | --------------------------- | ------------ | --------------------------------------------------- |
  | azure.adls2.storage_account   | Yes          | The username of the Data Lake Storage Gen2 account.    |
  | azure.adls2.shared_key        | Yes          | The Shared Key of the Data Lake Storage Gen2 account. |
...
- Authenticate and authorize based on Service Principal
  ...
  | azure.adls2.oauth2_client_secret      | Yes          | The newly created Client (Application) Secret.                       |
  | azure.adls2.oauth2_client_endpoint    | Yes          | The OAuth 2.0 Token Endpoint (v1) for the Service Principal or Application. |

##### Google GCS

Delta Lake Catalog supports Google GCS starting from version 3.0.

If you choose Google GCS as the file storage for the Delta Lake cluster, configure the `StorageCredentialParams` as follows:

- Authenticate and authorize using VM
  ...
  | ---------------------------------------- | ---------- | ------------------------------------------------------------ | ------------------------------------------------------ |
  | gcp.gcs.use_compute_engine_service_account | false      | true                                                         | Whether to directly use the Service Account bound on the Compute Engine. |

- Authenticate and authorize based on Service Account
  ...
  | **Parameter**                        | **Default** | **Example**                                                    | **Description**                                            |
  | ---------------------------------- | ---------- | ----------------------------------------------------------- | ------------------------------------------------------- |
  | gcp.gcs.service_account_email       | ""         | "[user@hello.iam.gserviceaccount.com](mailto:user@hello.iam.gserviceaccount.com)" | The Email generated in the JSON file when creating the Service Account.    |
  | gcp.gcs.service_account_private_key_id | ""         | "61d257bd8479547cb3e04f0b9b6b9ca07af3b7ea"                  | The Private Key ID generated in the JSON file when creating the Service Account. |
  | gcp.gcs.service_account_private_key  | ""         | "-----BEGIN PRIVATE KEY----xxxx-----END PRIVATE KEY-----\n" | The Private Key generated in the JSON file when creating the Service Account. |

- Authenticate and authorize based on Impersonation

  - Simulate Service Account using VM instance
...
```json
      {
        ...
        "gcp.gcs.impersonation_service_account": "hello"
      }
...
{
  "参数": "是否必须",
  "enable_metastore_cache": "否",
  "enable_remote_file_cache": "否",
  "metastore_cache_refresh_interval_sec": "否",
  "remote_file_cache_refresh_interval_sec": "否",
  "metastore_cache_ttl_sec": "否",
  "remote_file_cache_ttl_sec": "否"
}
...
- 如果 Amazon EMR Delta Lake cluster 使用 AWS Glue 作为元数据服务，则可以按照以下方式创建 Delta Lake Catalog：
...
#### Compatible with S3 Protocol Object Storage

Take MinIO as an example, you can create a Delta Lake Catalog as follows:
...
- If using Shared Key for authentication and authorization, you can create a Delta Lake Catalog as follows:
...
- If using SAS Token for authentication and authorization, you can create a Delta Lake Catalog as follows:
...
- If using Managed Service Identity for authentication and authorization, you can create a Delta Lake Catalog as follows:
...
- If using Service Principal for authentication and authorization, you can create a Delta Lake Catalog as follows:
...
- If using Managed Identity for authentication and authorization, you can create a Delta Lake Catalog as follows:
...
- If using VM for authentication and authorization, you can create a Delta Lake Catalog as follows:
...
- If using Service Account for authentication and authorization, you can create a Delta Lake Catalog as follows:
...
- If using Impersonation for authentication and authorization
  ...
  - Simulating a Service Account using a VM instance, you can create a Delta Lake Catalog as follows:
...
```
  ...
  - Simulating one Service Account with another Service Account, you can create a Delta Lake Catalog as follows:
