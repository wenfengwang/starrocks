...
如果要将 Data Lake Storage Gen2 作为 Paimon 集群的文件存储选项，则请按以下配置设置 `StorageCredentialParams`：
...
- 基于托管身份进行认证和授权
  ...
  | **参数**                                | **是否必须** | **说明**                                                |
  | --------------------------------------- | ------------ | ------------------------------------------------------- |
  | azure.adls2.oauth2_use_managed_identity | 是           | 指定是否开启托管身份鉴权方式。设置为 `true`。          |
  | azure.adls2.oauth2_tenant_id            | 是           | 数据所属租户的ID。                                      |
  | azure.adls2.oauth2_client_id            | 是           | 托管身份的客户（应用程序）ID。                           |
...
- 基于共享密钥进行认证和鉴权
  ...
  | **参数**                    | **是否必须** | **说明**                                    |
  | --------------------------- | ------------ | ------------------------------------------- |
  | azure.adls2.storage_account | 是           | Data Lake Storage Gen2账户的用户名。         |
  | azure.adls2.shared_key      | 是           | Data Lake Storage Gen2账户的共享密钥。      |
...
- 基于服务主体进行认证和鉴权
  ...
  | azure.adls2.oauth2_client_secret   | 是           | 新建的客户（应用程序）密钥。                               |
  | azure.adls2.oauth2_client_endpoint | 是           | 服务主体或应用程序的OAuth 2.0令牌终结点（v1）。            |

##### Google GCS

如果要将 Google GCS 作为 Paimon 集群的文件存储选项，则请按照以下配置设置 `StorageCredentialParams`：
...
- 基于VM进行认证和鉴权
  ...
  | **参数**                                   | **默认值** | **取值范例** | **说明**                                             |
  | ------------------------------------------ | ---------- | ------------ | ---------------------------------------------------- |
  | gcp.gcs.use_compute_engine_service_account | false      | true         | 是否直接使用Compute Engine上绑定的服务账户。         |
...
- 基于服务账户进行认证和鉴权
  ...
  | **参数**                               | **默认值** | **取值范例**                                                | **说明**                                                 |
  | -------------------------------------- | ---------- | ----------------------------------------------------------- | -------------------------------------------------------- |
  | gcp.gcs.service_account_email          | ""         | "[user@hello.iam.gserviceaccount.com](mailto:user@hello.iam.gserviceaccount.com)" | 创建服务账户时生成的JSON文件中的电子邮件。               |
 | gcp.gcs.service_account_private_key_id | ""         | "61d257bd8479547cb3e04f0b9b6b9ca07af3b7ea"              | 创建服务账户时生成的JSON文件中的私钥ID。                 |
  | gcp.gcs.service_account_private_key    | ""         | "-----BEGIN PRIVATE KEY----xxxx-----END PRIVATE KEY-----\n" | 创建服务账户时生成的JSON文件中的私钥。                   |
...
- 基于模拟进行认证和鉴权
  ...
  - 使用VM实例模拟服务账户
    ...
    | ------------------------------------------ | ---------- | ------------ | --------------------------------------------------------- |
    | gcp.gcs.use_compute_engine_service_account | false      | true         | 是否直接使用Compute Engine上绑定的服务账户。            |
    | gcp.gcs.impersonation_service_account      | ""         | "hello"       | 需要模拟的目标服务账户。                                  |

  - 使用一个服务账户（临时命名为"元服务账户"）模拟另一个服务账户（临时命名为"数据服务账户"）
    ...
    | **参数**                               | **默认值** | **取值范例**                                                | **说明**                                                   |
    | -------------------------------------- | ---------- | ----------------------------------------------------------- | ---------------------------------------------------------- |
    | gcp.gcs.service_account_email          | ""         | "[user@hello.iam.gserviceaccount.com](mailto:user@hello.iam.gserviceaccount.com)" | 创建元服务账户时生成的JSON文件中的电子邮件。             |
    | gcp.gcs.service_account_private_key_id | ""         | "61d257bd8479547cb3e04f0b9b6b9ca07af3b7ea"              | 创建元服务账户时生成的JSON文件中的私钥ID。               |
    | gcp.gcs.service_account_private_key    | ""         | "-----BEGIN PRIVATE KEY----xxxx-----END PRIVATE KEY-----\n" | 创建元服务账户时生成的JSON文件中的私钥。                 |
    | gcp.gcs.impersonation_service_account  | ""         | "hello"                                                     | 需要模拟的目标数据服务账户。                              |

### 示例

以下示例创建了名为`paimon_catalog_fs`的Paimon Catalog，其元数据类型`paimon.catalog.type`为`filesystem`，用于查询Paimon集群中的数据。
...
```SQL
显示 CREATE CATALOG paimon_catalog_fs;
```
