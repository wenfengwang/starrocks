...
| aliyun.oss.secret_key           | 是      | 指定阿里云账号或 RAM用户的AccessKey Secret，获取方式，请参见 [获取 AccessKey](https://help.aliyun.com/document_detail/53045.html)。      |
...
- 基于冒名顶替进行认证和授权
  ...
  - 通过 VM 实例模拟 Service Account

    ```SQL
    "gcp.gcs.use_compute_engine_service_account" = "true",
    "gcp.gcs.impersonation_service_account" = "<假定的谷歌服务帐号电子邮件>"
    ```

    `StorageCredentialParams` 包括以下参数。

    | **参数**                                   | **默认值** | **示例值** | **说明**                                                     |
    | ------------------------------------------ | ---------- | ------------ | ------------------------------------------------------------ |
    | gcp.gcs.use_compute_engine_service_account | false      | true         | 指定是否直接使用 Compute Engine 上绑定的 Service Account。     |
  ...
  - 使用一个 Service Account（暂时命名为“Meta Service Account”）模拟另一个 Service Account（暂时命名为“Data Service Account”）
    ...
    ```SQL
    "gcp.gcs.service_account_email" = "<谷歌服务帐号电子邮件>",
...
指定一组控制元数据更新策略的参数。此组参数是可选的。StarRocks 依据该策略来更新缓存的 Hive、Hudi 和 Delta Lake 元数据。有关 Hive、Hudi 和 Delta Lake 元数据缓存更新的详细信息，请参见[Hive catalog](../../data_source/catalog/hive_catalog.md)、[Hudi catalog](../../data_source/catalog/hudi_catalog.md) 和 [Delta Lake catalog](../../data_source/catalog/deltalake_catalog.md)。

StarRocks 默认采用自动异步更新策略，也即开箱即用。因此，通常无需调整 `MetadataUpdateParams` 中的策略参数。

若 Hive、Hudi 或 Delta Lake 数据频繁更新，您可以调整这些参数以优化自动异步更新策略的性能。

| 参数                                   | 是否必须 | 说明                                                         |
| -------------------------------------- | -------- | ------------------------------------------------------------ |
| enable_metastore_cache                 | 否       | 指定 StarRocks 是否缓存 Hive、Hudi 或 Delta Lake 表的元数据。取值范围：`true` 和 `false`。默认值：`true`。`true` 表示开启缓存，`false` 表示关闭缓存。 |
| enable_remote_file_cache               | 否       | 指定 StarRocks 是否缓存 Hive、Hudi 或 Delta Lake 表或分区的数据文件的元数据。取值范围：`true` 和 `false`。默认值：`true`。`true` 表示开启缓存，`false` 表示关闭缓存。 |
| metastore_cache_refresh_interval_sec   | 否       | StarRocks 异步更新的 Hive、Hudi 或 Delta Lake 表或分区元数据的时间间隔。单位：秒。默认值：`7200`，即 2 小时。 |
| remote_file_cache_refresh_interval_sec | 否       | StarRocks 异步更新的 Hive、Hudi 或 Delta Lake 表或分区数据文件元数据的时间间隔。单位：秒。默认值：`60`。 |
| metastore_cache_ttl_sec                | 否       | StarRocks 自动淘汰的 Hive、Hudi 或 Delta Lake 表或分区元数据的时间间隔。单位：秒。默认值：`86400`，即 24 小时。 |
| remote_file_cache_ttl_sec              | 否       | StarRocks 自动淘汰的 Hive、Hudi 或 Delta Lake 表或分区数据文件元数据的时间间隔。单位：秒。默认值：`129600`，即 36 小时。 |

### 例子

下例创建了一个名为 `unified_catalog_hms` 或 `unified_catalog_glue` 的 Unified Catalog，用于查询融合数据源中的数据。

#### HDFS

选择 HDFS 作为存储时，可按以下方式创建 Unified Catalog：
...
- 若使用 HMS 作为元数据服务，可按以下方式创建 Unified Catalog：
...
- 若 Amazon EMR 使用 AWS Glue 作为元数据服务，可按以下方式创建 Unified Catalog：
...
##### If authenticated and authenticated based on IAM User

- If using HMS as the metadata service, the Unified Catalog can be created as follows:
...
- If Amazon EMR uses AWS Glue as the metadata service, the Unified Catalog can be created as follows:
...
#### S3 Protocol-Compatible Object Storage

Taking MinIO as an example, the Unified Catalog can be created as follows:
...
- If authenticated and authenticated based on Shared Key, the Unified Catalog can be created as follows:
...
- If authenticated and authenticated based on SAS Token, the Unified Catalog can be created as follows:
...
- If authenticated and authenticated based on Managed Service Identity, the Unified Catalog can be created as follows:
...
- If authenticated and authenticated based on Service Principal, the Unified Catalog can be created as follows:
...
- If authenticated and authenticated based on Managed Identity, the Unified Catalog can be created as follows:
...
- If authenticated and authenticated based on VM, the Unified Catalog can be created as follows:
...
- If authenticated and authenticated based on Service Account, the Unified Catalog can be created as follows:
...
- If authenticated and authenticated based on Impersonation

  - Simulate Service Account using VM instance, the Unified Catalog can be created as follows:
...
- 以下是用一个 Service Account 模拟另一个 Service Account 来创建 Unified Catalog 的示例：
...
您可以通过 [SHOW CATALOGS](../../sql-reference/sql-statements/data-manipulation/SHOW_CATALOGS.md) 查询当前 StarRocks 集群中所有 Catalog：

```SQL
SHOW CATALOGS;
```

您也可以使用 [SHOW CREATE CATALOG](../../sql-reference/sql-statements/data-manipulation/SHOW_CREATE_CATALOG.md) 来查询某个 External Catalog 的创建语句。例如，使用以下命令查询 Unified Catalog `unified_catalog_glue` 的创建语句：
...
您可以使用以下方法切换到目标 Unified Catalog 和数据库：

- 首先使用 [SET CATALOG](../../sql-reference/sql-statements/data-definition/SET_CATALOG.md) 指定当前会话生效的 Unified Catalog，然后通过 [USE](../../sql-reference/sql-statements/data-definition/USE.md) 指定数据库：
...
- 直接使用 [USE](../../sql-reference/sql-statements/data-definition/USE.md) 将会话切换到目标 Unified Catalog 下的指定数据库：
...
您可以使用 [DROP CATALOG](../../sql-reference/sql-statements/data-definition/DROP_CATALOG.md) 删除某个 External Catalog。

例如，使用以下命令删除 Unified Catalog `unified_catalog_glue`：
...
您可以通过以下操作查询 Unified Catalog 内的数据：

1. 使用 [SHOW DATABASES](../../sql-reference/sql-statements/data-manipulation/SHOW_DATABASES.md) 查看指定 Unified Catalog 所属的数据源中的数据库：
...
3. 使用 [SELECT](../../sql-reference/sql-statements/data-manipulation/SELECT.md) 查询目标数据库中的目标表：
...
您可以使用 [INSERT INTO](../../sql-reference/sql-statements/data-manipulation/INSERT.md) 将 Hive、Iceberg、Hudi 或 Delta Lake 表中的数据导入 StarRocks 中 Unified Catalog 下的表。

例如，使用以下命令将 Hive 表 `hive_table` 的数据导入到 StarRocks 中 Unified Catalog `unified_catalog` 下的数据库`test_database` 中的表 `test_table`：
...
与 StarRocks 内部数据目录 (Internal Catalog) 类似，如果您拥有 Unified Catalog 的 [CREATE DATABASE](../../administration/privilege_item.md#数据目录权限-catalog) 权限，您可以使用 [CREATE DATABASE](../../sql-reference/sql-statements/data-definition/CREATE_DATABASE.md) 在该 Unified Catalog 内创建数据库。

> **注意**
>
> 您可以使用 [GRANT](../../sql-reference/sql-statements/account-management/GRANT.md) 和 [REVOKE](../../sql-reference/sql-statements/account-management/REVOKE.md) 对用户和角色进行权限的授予和收回。

请注意，目前仅支持创建 Hive 数据库和 Iceberg 数据库。

[切换至目标 Unified Catalog](#切换-unified-catalog-和数据库)，然后使用以下语句创建数据库：
...
- 选择 HMS 作为元数据服务时，如果在创建数据库时未指定 `location`，系统会使用 HMS 默认的 `<warehouse_location>/<database_name.db>` 作为文件路径。
- 选择 AWS Glue 作为元数据服务时，`location` 参数没有默认值，因此在创建数据库时必须指定该参数。
...
| HDFS                                   | `hdfs`                                                       |
| Google GCS                             | `gs`                                                         |
| Azure Blob Storage                     | <ul><li>如果存储账号支持通过 HTTP 协议访问，`prefix` 为 `wasb`。</li><li>如果存储账号支持通过 HTTPS 协议访问，`prefix` 为 `wasbs`。</li></ul> |
| Azure Data Lake Storage Gen1           | `adl`                                                        |
| Azure Data Lake Storage Gen2           | <ul><li>如果存储账号支持通过 HTTP 协议访问，`prefix` 为 `abfs`。</li><li>如果存储账号支持通过 HTTPS 协议访问，`prefix` 为 `abfss`。</li></ul> |
| 阿里云 OSS                             | `oss`                                                        |
| 腾讯云 COS                             | `cosn`                                                       |
| 华为云 OBS                             | `obs`                                                        |
...
与 StarRocks 内部数据库一致，如果您拥有 Unified Catalog 内数据库的 [DROP](../../administration/privilege_item.md#数据库权限-database) 权限，您可以使用 [DROP DATABASE](../../sql-reference/sql-statements/data-definition/DROP_DATABASE.md) 来删除该数据库。仅支持删除空数据库。

> **注意**
>
> 您可以使用 [GRANT](../../sql-reference/sql-statements/account-management/GRANT.md) 和 [REVOKE](../../sql-reference/sql-statements/account-management/REVOKE.md) 对用户和角色进行权限的授予和收回。

请注意，目前仅支持删除 Hive 数据库和 Iceberg 数据库。

删除数据库操作并不会将 HDFS 或对象存储上对应的文件路径删除。

[切换至目标 Unified Catalog](#切换-unified-catalog-和数据库)，然后使用以下语句删除数据库：

```SQL
DROP DATABASE <database_name>
```
> 通过 [GRANT](../../sql-reference/sql-statements/account-management/GRANT.md) 和 [REVOKE](../../sql-reference/sql-statements/account-management/REVOKE.md) 操作可以对用户和角色进行权限的赋予和收回。

注意目前仅支持创建 Hive 表和 Iceberg 表。

[切换至目标 Unified Catalog 和数据库](#切换-unified-catalog-和数据库)，之后使用 [CREATE TABLE](../../sql-reference/sql-statements/data-definition/CREATE_TABLE.md) 来创建 Hive 表或 Iceberg 表：
...
例如，使用以下语句创建一张 Hive 表 `hive_table`：
...
与 StarRocks 内表一致，如果您有 Unified Catalog 内表的 [INSERT](../../administration/privilege_item.md#表权限-table) 权限，那么您可以使用 [INSERT](../../sql-reference/sql-statements/data-manipulation/INSERT.md) 将 StarRocks 表数据写入到该表（目前仅支持写入到 Parquet 格式的 Unified Catalog 表）。

> **说明**
>
> 您可以通过 [GRANT](../../sql-reference/sql-statements/account-management/GRANT.md) 和 [REVOKE](../../sql-reference/sql-statements/account-management/REVOKE.md) 操作可以对用户和角色进行权限的赋予和收回。

注意目前仅支持向 Hive 表和 Iceberg 表中插入数据。

[切换至目标 Unified Catalog 和数据库](#切换-unified-catalog-和数据库)，之后使用 [INSERT INTO](../../sql-reference/sql-statements/data-manipulation/INSERT.md) 向 Hive 表或 Iceberg 表中插入数据：

```SQL
INSERT {INTO | OVERWRITE} <table_name>
(列名 [, ...])
{ VALUES ( { 表达式 | DEFAULT } [, ...] ) [, ...] | 查询 }
...
例如，使用以下语句将数据插入到 Hive 表 `hive_table` 中：
...
与 StarRocks 内表一致，如果您有 Unified Catalog 内表的 [DROP](../../administration/privilege_item.md#表权限-table) 权限，那么您可以使用 [DROP TABLE](../../sql-reference/sql-statements/data-definition/DROP_TABLE.md) 来删除该表。

> **说明**
>
> 您可以通过 [GRANT](../../sql-reference/sql-statements/account-management/GRANT.md) 和 [REVOKE](../../sql-reference/sql-statements/account-management/REVOKE.md) 操作可以对用户和角色进行权限的赋予和收回。

注意目前仅支持删除 Hive 表和 Iceberg 表。

[切换至目标 Unified Catalog 和数据库](#切换-unified-catalog-和数据库)，之后使用 [DROP TABLE](../../sql-reference/sql-statements/data-definition/DROP_TABLE.md) 来删除 Hive 表或 Iceberg 表。
