...
StarRocks 从 3.1 版本开始支持 Elasticsearch 目录。

StarRocks 和 Elasticsearch 目前都是流行的分析系统。StarRocks 擅长大规模分布式计算，并支持通过外部表查询 Elasticsearch。Elasticsearch 擅长全文检索。结合两者提供了更完善的 OLAP 解决方案。使用 Elasticsearch 目录，您可以直接通过 StarRocks 使用 SQL 分析 Elasticsearch 集群内所有的索引数据，无需进行数据迁移。

与其他数据源的目录不同，创建 Elasticsearch 目录后，下面只有一个名为 `default` 的数据库 (Database)，每个 Elasticsearch 索引 (Index) 自动映射一张数据表 (Table)，并且都会自动挂载在 `default` 数据库下面。
...
| type                 | 是           | 无         | 数据源的类型。创建 Elasticsearch 目录时，必须设置为 `es`。 |
| user                 | 否           | 空         | 开启 HTTP Basic 认证的 Elasticsearch 集群的用户名，需要确保该用户有访问 `/cluster/state/ nodes/http` 等路径权限和对索引的读取权限。 |
| password             | 否           | 空         | 对应用户的密码信息。                                         |
| es.type              | 否           | `_doc`     | 指定索引的类型。如果要查询的数据是在 Elasticsearch 8 及以上版本，那在 StarRocks 中创建外部表时就不需要配置该参数，因为 Elasticsearch 8 及以上版本已经移除了 mapping types。 |
...
| enable_docvalue_scan | 否           | `true`     | 是否从 Elasticsearch 列式存储获取查询字段的值。大多数情况下，从列式存储中读取数据的性能要优于从行式存储中读取数据的性能。 |
