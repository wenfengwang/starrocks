...
本文档介绍如何手动部署 StarRocks 存储计算一体的集群（BE 节点同时负责数据存储和处理）。其他安装方法，请参阅[部署总览](../deployment/deployment_overview.md)。

如果您需要部署存储计算分离的集群，请参考[部署使用 StarRocks 存储计算分离集群](./shared_data/s3.md)。

## 第一步：启动 Leader FE 节点

以下步骤在 FE 实例上执行。

1. 创建元数据存储路径。建议将元数据存储于 FE 部署文件的不同路径。确保该路径存在且您有写入权限。

   ```Bash
   # 用您想创建的元数据目录来替换 <meta_dir>。
   mkdir -p <meta_dir>
   ```

2. 进入之前准备的[StarRocks FE 部署文件](../deployment/prepare_deployment_files.md)路径，修改 FE 配置文件 **fe/conf/fe.conf**。

   a. 在 `meta_dir` 配置项指定元数据路径。

      ```YAML
      # 用您已创建的元数据目录来替换 <meta_dir>。
      meta_dir = <meta_dir>
      ```

   b. 如果 FE 端口被占用（如在 [环境配置清单](../deployment/environment_configurations.md) 中提到），需要在 FE 配置文件中为其分配其他可用端口。
      ...
      > **注意：**
      >
      > 如果要在集群中部署多个 FE 节点，所有 FE 节点的 `http_port` 必须相同。

   c. 如需启用 IP 地址访问集群，需在配置文件中添加 `priority_networks`，并为 FE 节点分配一个独有的 IP 地址（CIDR 格式）。若要启用 FQDN 访问，则可忽略此项。
      ...
      > **说明：**
      >
      > 您可以在终端运行 `ifconfig` 命令查看实例当前的 IP 地址。

   d. 如果实例安装了多个 JDK，需要使用的 JDK 与 `JAVA_HOME` 环境变量中指定的 JDK 不一致，需在配置文件中添加 `JAVA_HOME` 配置项，并指定所用 JDK 的安装路径。

      ```YAML
      # 用选定的 JDK 安装路径替换 <path_to_JDK>。
      JAVA_HOME = <path_to_JDK>
      ```

   e. 更多高级配置选项，请参阅 [参数配置 - FE 配置项](../administration/Configuration.md#fe-配置项)。

3. 启动 FE 节点。

   - 如需使用 IP 地址访问集群，请执行以下命令启动 FE 节点：

     ```Bash
     ./fe/bin/start_fe.sh --daemon
     ```

   - 若需要使用 FQDN 访问，执行以下命令启动 FE 节点：

     ```Bash
     ./fe/bin/start_fe.sh --host_type FQDN --daemon
     ```

     只在第一次启动节点时需要指定 `--host_type` 参数。

     > **注意：**
     >
     > 启动 FE 节点前，请确保已在 **/etc/hosts** 中为所有实例设置了主机名。详情参阅[环境配置清单 - 主机名](../deployment/environment_configurations.md#主机名)。

4. 查看 FE 日志，确认 FE 节点是否成功启动。

   ```Bash
   cat fe/log/fe.log | grep thrift
   ```

   如果日志中显示如下信息，则FE节点已成功启动：
...
1. 创建数据存储路径。建议将数据存储于 BE 部署文件的不同路径。确保该路径存在且您有写入权限。

   ```Bash
   # 用您想创建的数据存储路径来替换 <storage_root_path>。
   mkdir -p <storage_root_path>
   ```

2. 进入之前准备的[StarRocks BE 部署文件](../deployment/prepare_deployment_files.md)路径，修改 BE 配置文件 **be/conf/be.conf**。

   a. 在 `storage_root_path` 配置项指定数据存储路径。

      ```YAML
      # 用您创建的数据存储路径来替换 <storage_root_path>。
      storage_root_path = <storage_root_path>
      ```

   b. 如果 BE 端口被占用（如在 [环境配置清单](../deployment/environment_configurations.md) 中提到），需要在 BE 配置文件中为其分配其他可用端口。
   ...
   c. 如需启用 IP 地址访问集群，需在配置文件中添加 `priority_networks` 并为 BE 节点分配一个独有的 IP 地址（CIDR格式）。若要启用 FQDN 访问，则可忽略此项。
   ...
   e. 更多高级配置选项，请参阅 [参数配置 - BE 配置项](../administration/Configuration.md#be-配置项)。
...
3. 启动 BE 节点。
   ...
   > **注意：**
   >
   > - 启动 BE 节点前，请确保已在 **/etc/hosts** 中为所有实例设置了主机名。详情参阅[环境配置清单 - 主机名](../deployment/environment_configurations.md#主机名)。
   > - 启动 BE 节点时不需要指定 `--host_type` 参数。

4. 查看 BE 日志，确认 BE 节点是否成功启动。

   ```Bash
   cat be/log/be.INFO | grep heartbeat
   ```

   如果日志中显示如下信息，则 BE 节点已成功启动：
...
计算节点（CN）是一种无状态的计算服务，本身不存储数据。您可以通过添加 CN 节点为查询提供额外的计算资源。您可以使用 BE 部署文件部署 CN 节点。CN 节点自 v2.4 版本起支持。
...
1. 进入先前准备好的 [StarRocks BE 部署文件](../deployment/prepare_deployment_files.md)所在路径，修改 CN 配置文件 **be/conf/cn.conf**。
   ...
   b. 如需为集群启用 IP 地址访问，您必须在配置文件中添加配置项 `priority_networks`，为 CN 节点分配一个专有的 IP 地址（CIDR格式）。如需为集群启用 FQDN 访问，则可以忽略该配置项。.
      ...
      > **说明**
      >
      > 您可以在终端中运行 `ifconfig` 查看当前实例拥有的 IP 地址。

   c. 如果您的实例安装了多个 JDK，并且你要使用 JDK 与环境变量 `JAVA_HOME` 中指定的不同，则必须在配置文件中添加配置项 `JAVA_HOME` 来指定所选该 JDK 的安装路径。
...
高可用的 FE 集群需要在 StarRocks 集群中部署至少三个 Follower FE 节点。若需部署高可用的 FE 集群，您需要额外再启动两个新的 FE 节点。
