...
来自'../../assets/commonMarkdown/sharedDataIntro.md'的共享数据介绍
...
StarRocks 存算分离集群的部署方式与存算一体集群的部署方式类似，但存算分离集群需要部署 CN 节点而非 BE 节点。本小节仅列出部署 StarRocks 存算分离集群时需要添加到 FE 和 CN 配置文件 **fe.conf** 和 **cn.conf** 中的额外配置项。有关部署 StarRocks 集群的详细说明，请参阅 [手动部署](../deploy_manually.md)。
...
不同的认证方式需要配置不同的 FE 参数。请根据业务需求参考以下各小节的示例。
...
来自'../../assets/commonMarkdown/sharedDataCNconf.md'的部署 CN 配置

## 使用 StarRocks 存算分离集群

来自'../../assets/commonMarkdown/sharedDataUseIntro.md'的使用StarRocks存算分离集群介绍
