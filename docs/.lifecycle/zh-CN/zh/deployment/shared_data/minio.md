...
部署 StarRocks 存算分离集群的方式与存算一体集群的方式类似，但存算分离集群需要部署 CN 节点而非 BE 节点。本小节仅列出部署 StarRocks 存算分离集群时需要添加到 FE 和 CN 配置文件 **fe.conf** 和 **cn.conf** 中的额外配置项。有关部署 StarRocks 集群的详细说明，请参阅 [部署 StarRocks](../deploy_manually.md)。
...
由于 StarRocks 通过 [AWS Signature Version 4 protocol](https://docs.aws.amazon.com/AmazonS3/latest/API/sig-v4-authenticating-requests.html) 访问 MinIO，您需要在所有 FE 节点的配置文件 **fe.conf** 中添加以下配置项，以 `aws_s3` 为前缀：
...
用于存储数据的 MinIO 存储空间路径，由 MinIO 存储桶的名称及其下的子路径（如有）组成，如 `testbucket/subpath`。

#### aws_s3_endpoint

访问 MinIO 存储空间的连接地址，如 `https://s3.us-west-2.amazonaws.com`。

#### aws_s3_region

需访问的 MinIO 存储空间的地区，如 `us-west-2`。

#### aws_s3_access_key

访问 MinIO 存储空间的 Access Key。

#### aws_s3_secret_key

访问 MinIO 存储空间的 Secret Key。
...
以下示例使用 Access Key 以及 Secret Key 认证为 MinIO 存储空间 `defaultbucket` 创建存储卷 `def_volume`，激活并将其设置为默认存储卷：
