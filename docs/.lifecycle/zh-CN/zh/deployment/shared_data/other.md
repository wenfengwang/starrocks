...
> - If you specify this item as `AZBLOB`, you must add a configuration item with the prefix `azure_blob`.
> - If you specify this item as `HDFS`, you only need to specify `cloud_native_hdfs_url`.

#### aws_s3_path

The storage path for storing data, composed of the name of the bucket and its subpath (if any), such as `testbucket/subpath`.

#### aws_s3_endpoint

The connection address for accessing the storage space.

#### aws_s3_region

The region of the storage space to be accessed, such as `us-west-2`.

#### aws_s3_access_key

The Access Key for accessing the storage space.

#### aws_s3_secret_key

The Secret Key for accessing the storage space.

> **Note**
>
> After successfully creating a storage and computing separated cluster, you can only modify the configuration items related to security credentials. If you change the configuration items related to the original storage path, the databases and tables created before this will become read-only, and you cannot import data into them.

## Storage and Computing Separation Deployment CN Configuration

<SharedDataCNconf />

## Using StarRocks Storage and Computing Separated Cluster

<SharedDataUseIntro />

The following example uses the Access Key and Secret Key for OSS storage space `defaultbucket` to create a storage volume `def_volume`, activate it, and set it as the default storage volume:
