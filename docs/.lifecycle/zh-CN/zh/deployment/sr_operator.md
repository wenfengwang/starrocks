...
1. 查看 FE Service 内部虚拟 IP `CLUSTER-IP` 和端口 `PORT(S)`。
    ...
    be-domain-search                     集群IP      无               <无>          9050/TCP                              23分钟
    fe-domain-search                     集群IP      无               <无>          9030/TCP                              25分钟
    starrockscluster-sample-fe-service   集群IP      10.100.162.xxx   <无>          8030/TCP,9020/TCP,9030/TCP,9010/TCP   25分钟
    ```
...
2. 查询 FE Service 向外部暴露的 IP 地址 `EXTERNAL-IP` 和端口 `PORT(S)`。
    ...
    $ kubectl -n starrocks get svc
    NAME                                 TYPE           CLUSTER-IP       EXTERNAL-IP                                                              PORT(S)                                                       AGE
    be-domain-search                     集群IP         无               <无>                                                                   9050/TCP                                                      127分钟
    fe-domain-search                     集群IP         无               <无>                                                                   9030/TCP                                                      129分钟
    starrockscluster-sample-fe-service   LoadBalancer   10.100.162.xxx   a7509284bf3784983a596c6eec7fc212-618xxxxxx.us-west-2.elb.amazonaws.com   8030:30629/TCP,9020:32544/TCP,9030:32244/TCP,9010:32024/TCP   129分钟               集群IP         无                <无>                                                                   9030/TCP                                                      23小时
    ```
    ...
    autoScalingPolicy: # CN 集群的自动扩缩容策略。
      ...
      hpaPolicy:
        metrics: # 资源指标
          - type: Resource
            resource:
              name: memory  # CN 的平均内存使用率被指定为资源指标。
              target:
                # 弹性伸缩阈值为 60%。
                当 CN 的平均内存利用率超过 60% 时，CN 数量增加以实现扩展。
                当 CN 的平均内存利用率低于 60% 时，CN 数量减少以实现收缩。
                averageUtilization: 60
                type: Utilization
          - type: Resource
            resource:
              name: cpu # CN 的平均 CPU 使用率被指定为资源指标。
              target:
                # 弹性伸缩阈值为 60%。
                当 CN 的平均 CPU 利用率超过 60% 时，CN 数量增加以实现扩展。
                当 CN 的平均 CPU 利用率低于 60% 时，CN 数量减少以实现收缩。
                averageUtilization: 60
                type: Utilization
