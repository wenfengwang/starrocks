...
1. 您可以从 [下载 StarRocks](https://www.starrocks.io/download/community) 页面直接下载 StarRocks 二进制包，或在终端中运行以下命令获取：
...
您需要在计算机上安装 [Docker 引擎](https://docs.docker.com/engine/install/) (17.06.0 以上)。
...
2. 运行以下命令将 StarRocks 部署文件从 Docker 镜像复制到您的主机：
   ...
   | **路径**             | **说明**                                                     |
   | -------------------- | ------------------------------------------------------------ |
   | **be_artifacts**     | 该路径下包含 BE 或 CN 节点的部署路径 **be**、StarRocks 许可文件 **LICENSE.txt** 以及 StarRocks 通知文件 **NOTICE.txt。** |
   | **broker_artifacts** | 该路径下包含 Broker 节点的部署路径 **apache_hdfs_broker**。  |
   | **fe_artifacts**     | 该路径下包含 FE 节点的部署路径 **fe**、StarRocks 许可文件 **LICENSE.txt** 以及 StarRocks 通知文件 **NOTICE.txt。** |
