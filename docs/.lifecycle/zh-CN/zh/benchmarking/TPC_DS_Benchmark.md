...
TPC-DS是一套决策支持系统的基准测试，它对决策支持系统的几个通用方面进行建模，包括查询和数据维护，用于衡量大数据产品的分析性能。TPC-DS模拟了零售企业三种销售渠道（实体店、互联网、目录）的销售和退货业务，除了建立相关销售和退货模型的表格，它还包括一个简单的库存系统和一个促销系统。该测试共包含24张表，数据量可设定从1GB到3TB不等。基准测试共包含99个复杂查询，主要的评价指标为各个查询的响应时间，即从提交查询到结果返回所需时间。

## 1. 测试结论

在TPC-DS 100GB规模的数据集上进行对比测试，共99个查询，测试结果如下：

![tpc-ds](./../assets/tpc-ds.png)

StarRocks测试了使用本地存储查询和Hive外表查询两种方式，其中StarRocks Hive外表查询和Trino查询的是同一份数据，数据采用Parquet格式存储，LZ4格式压缩。

最终，StarRocks本地存储查询总耗时为174s，StarRocks Hive外表查询总耗时为239s，StarRocks Hive外表查询开启Data Cache后查询总耗时为176s，Trino查询总耗时为892s。

StarRocks本地存储查询性能是Trino的5.1倍，StarRocks Hive外表查询是Trino的3.7倍，StarRocks Hive外表查询开启Data Cache后查询性能是Trino的5倍。
...
StarRocks和Trino部署在相同配置的机器上进行测试，StarRocks部署1个FE和3个BE，Trino部署1个Coordinator和3个Worker。

- 内核版本：Linux 3.10.0-1127.13.1.el7.x86_64

- 操作系统版本：CentOS Linux release 7.8.2003

- 软件版本：StarRocks社区版3.0，Trino-419，Hive-3.1.2
...
>
> - 查询结果的单位是ms。
> - `StarRocks-3.0.5-native`表示使用StarRocks本地存储，`StarRocks-3.0-Hive external`表示通过Catalog查询Hive外表，`StarRocks-3.0-Hive external-Cache`表示通过Catalog查询Hive外表并开启StarRocks的Data Cache特性。
> - StarRocks开启聚合下推优化 `SET global cbo_push_down_aggregate_mode = 0`。
...
StarRocks 3.0版本不支持收集外部表的统计信息，使用Hive收集列级别统计信息。
...
StarRocks使用Catalog功能查询Hive外部表数据，详细操作请参见[Hive catalog](../data_source/catalog/hive_catalog.md)。

StarRocks开启[Data Cache](../data_source/data_cache.md)的测试场景，在当前机器配置和集群规模下，推荐使用如下配置
...
共99条详细的查询SQL，请参见[TPC-DS SQL](./tpc_ds_99_sql.md)。

### 5.2 StarRocks Native建表
...
(dup key (cr_item_sk, cr_order_number)
dist by hash(cr_item_sk, cr_order_number) buckets 16
props(
    "repl_num" = "1"
);
...
(dup key (cs_item_sk, cs_order_number)
dist by hash(cs_item_sk) buckets 192
props(
    "repl_num" = "1"
);
...
(dup key(ca_address_sk)
dist by hash(ca_address_sk) buckets 10
props(
    "repl_num" = "1"
);
...
(dup key (cd_demo_sk)
dist by hash(cd_demo_sk) buckets 10
props(
    "repl_num" = "1"
);
...
(dup key (c_customer_sk)
dist by hash(c_customer_sk) buckets 10
props(
    "repl_num" = "1"
);
...
(dup key (d_date_sk)
dist by hash(d_date_sk) buckets 5
props(
    "repl_num" = "1"
);
...
(dup key (hd_demo_sk)
dist by hash(hd_demo_sk) buckets 1
props(
    "repl_num" = "1"
);
...
(dup key (ib_income_band_sk)
dist by hash(ib_income_band_sk) buckets 1
props(
    "repl_num" = "1"
);
...
(dup key (inv_item_sk, inv_date_sk, inv_warehouse_sk)
dist by hash(inv_item_sk) buckets 32
props(
    "repl_num" = "1"
);
...
(dup key (i_item_sk)
dist by hash(i_item_sk) buckets 10
props(
    "repl_num" = "1"
);
...
### 5.4 Hive External Table Creation (CSV Format)
