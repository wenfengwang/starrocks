...
StarRocks 1.18 版本开始起，Decimal 类型支持更高精度的快速十进制数 (也称 十进制数 V3)。

主要优化有：

1. 内部采用多种宽度的整数来表示 十进制数。比如使用 64-bit 整数来表示 P &le; 18 的 十进制 数值。相比于 十进制数 V2 统一采用 128-bit 整数，算数运算和转换运算在 64-bit 的处理器上使用更少的指令，因此性能有大幅提升。

2. 和 十进制数 V2 相比，快速十进制数 的算法做了极致的优化，尤其是乘法运算，性能提升有 4 倍左右。

快速十进制数 功能由 FE 动态参数 `enable_decimal_v3` 控制，默认值为 `true`，表示开启。

从 3.1 版本开始，[ARRAY](Array.md)、[MAP](Map.md)、[STRUCT](STRUCT.md) 支持快速十进制数。

### 使用限制

读取外部 Hive 数据时，暂不支持读取 ORC 和 Parquet 文件中的 十进制数 数据，会有精度丢失。
