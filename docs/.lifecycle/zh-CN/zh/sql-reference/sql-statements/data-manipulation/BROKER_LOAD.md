...
经纪人装载是一种基于 MySQL 协议的异步导入方式。您提交导入作业以后，StarRocks 会异步地执行导入作业。您可以使用 `SELECT * FROM information_schema.loads` 来查看经纪人装载作业的结果，该功能自 3.1 版本起支持。有关经纪人装载的背景信息、基本原理、支持的数据文件格式、如何执行单表导入 (Single-Table Load) 和多表导入 (Multi-Table Load) 操作、以及如何查看导入作业的结果等信息，请参见[从 HDFS 导入](../../../loading/hdfs_load.md)和[从云存储导入](../../../loading/cloud_storage_load.md)。

> **注意**
>
> 经纪人装载操作需要目标表的 INSERT 权限。如果您的用户账号没有 INSERT 权限，请参考 [GRANT](../account-management/GRANT.md) 给用户赋权。
...
经纪人装载支持一次导入多个数据文件。在一个导入作业中，您可以使用多个 `data_desc` 来声明导入多个数据文件，也可以使用一个 `data_desc` 来声明导入一个路径下的所有数据文件。经纪人装载还支持保证单次导入事务的原子性，即单次导入的多个数据文件都成功或者都失败，而不会出现部分导入成功、部分导入失败的情况。
...
- `file_path`
  ...
  > **注意**
  >
  > - 经纪人装载支持通过 S3 或 S3A 协议访问 AWS S3，因此从 AWS S3 导入数据时，您在文件路径中传入的目标文件的 S3 URI 可以使用 `s3://` 或 `s3a://` 作为前缀。
  > - 由于 经纪人装载 只支持通过 gs 协议访问 Google GCS，因此当从 Google GCS 导入数据时，必须确保文件路径传入的目标文件的 GCS URI 使用 `gs://` 为前缀。
...
> **Explanation**
>
> This parameter is only applicable when the target StarRocks table uses an aggregate model and all Value columns' aggregate functions are `sum`.

- `PARTITION`

  Specify which partitions the data should be imported into. If this parameter is not specified, the data will be imported into all partitions of the StarRocks table by default.

- `TEMPORARY_PARTITION`

  Specify which [temporary partitions](../../../table_design/Temporary_partition.md) the data should be imported into.

- `COLUMNS TERMINATED BY`

  Used to specify the column delimiter in the source data file. If this parameter is not specified, the default column delimiter is `\t`, which represents a Tab. It is crucial to ensure that the specified column delimiter here matches the column delimiter in the source data file; otherwise, the import job will fail due to data quality errors, and the job status (`State`) will display as `CANCELLED`.

  It is important to note that Broker Load submits import requests through the MySQL protocol. In addition to StarRocks handling escape processing, the MySQL protocol also handles escape processing. Therefore, if the column delimiter is a non-visible character like Tab, an additional backslash (\\) needs to be added before the column delimiter here. For example, if the column delimiter is `\t`, it must be input as `\\t`; if the column delimiter is `\n`, it must be input as `\\n`. The column delimiter for Apache Hive™ files is `\x01`, so if the source data file is a Hive file, `\\x01` must be specified here.

  > **Explanation**
  >
  > - StarRocks supports setting a maximum length of 50 bytes of UTF-8 encoded strings as the column delimiter, including common delimiters such as comma (,), Tab, and Pipe (|).
  > - The null value is represented as `\N`. For example, if the data file has three columns and a certain row of data has the first and third columns as `a` and `b` respectively, with no data in the second column, the second column should be represented as `\N` to indicate a null value, written as `a,\N,b`, rather than `a,,b`. `a,,b` would imply that the second column is an empty string.

- `ROWS TERMINATED BY`

  Used to specify the row delimiter in the source data file. If this parameter is not specified, the default row delimiter is `\n`, representing a newline. It is essential to ensure that the specified row delimiter here matches the row delimiter in the source data file; otherwise, the import job will fail due to data quality errors, and the job status (`State`) will display as `CANCELLED`. This parameter is supported starting from version 2.5.4.

  Other considerations and usage conditions are the same as specifying the column delimiter through `COLUMNS TERMINATED BY` mentioned above.

- `FORMAT AS`

  Used to specify the format of the source data file. Possible values include `CSV`, `Parquet`, and `ORC`. If this parameter is not specified, the file format is determined by the file extension specified in the `file_path` parameter (**.csv**, **.parquet**, and **.orc**).

- `format_type_options`

  When `FORMAT AS` is set to `CSV`, it is used to specify CSV format options. The syntax is as follows:
  ...
  > **Explanation**
  >
  > `format_type_options` is supported in version 3.0 and later.

  The option details are provided in the table below.

  | **Parameter** | **Explanation**                                               |
  | ------------- | ------------------------------------------------------------ |
  | skip_header   | Used to specify the number of lines to skip at the beginning of the CSV file. Data type: INTEGER. Default value: `0`.<br />In some CSV files, the first few lines of data are used to define column names, column types, and other metadata information. By setting this parameter, StarRocks can ignore the first few lines of the CSV file when importing data. For example, if this parameter is set to `1`, StarRocks will ignore the first line of the CSV file when importing the data.<br />The delimiter used for these lines must match the row delimiter you set in the import statement. |
  | trim_space    | Used to specify whether to remove leading and trailing spaces from the column delimiter in the CSV file. Data type: BOOLEAN. Default value: `false`.<br />Some databases add spaces before and after the column delimiter when exporting data to a CSV file. Depending on the position, these spaces can be referred to as "leading spaces" or "trailing spaces". By setting this parameter, StarRocks can remove these unnecessary spaces when importing the data.<br />It is important to note that StarRocks does not remove spaces inside a field enclosed by the `enclose` specified character, including leading and trailing spaces. For example, if the column delimiter is a vertical bar (|), and the `enclose` character is double quotes (""):<br />`| "Love StarRocks" |`<br />`| " Love StarRocks " |`<br />`| "Love StarRocks" |`<br />If `trim_space` is set to `true`, the resulting data processed by StarRocks would be as follows:<br />`| "Love StarRocks" |`<br />`| " Love StarRocks " |`<br />`| "Love StarRocks" |` |
  | enclose       | According to [RFC4180](https://www.rfc-editor.org/rfc/rfc4180), it is used to specify the character used to enclose fields in the CSV file. Data type: single-byte character. Default value: `NONE`. The most commonly used `enclose` characters are single quotation marks (') or double quotation marks ("').<br />All special characters inside a field enclosed by the specified `enclose` character are treated as normal symbols. Going beyond the RFC4180 standard, the `enclose` attribute provided by StarRocks supports setting any single-byte character.<br />If a field contains the specified `enclose` character, the same character can be used to escape the `enclose` character. For example, when `enclose` is set to double quotes ("), the field value `a "quoted" c` should be written as `"a ""quoted"" c"` in the CSV file. |
### escape
...
在 v2.3 及以前版本，您需要在导入语句中通过 `WITH BROKER "<broker名>"` 来指定使用哪个 Broker。自 v2.5 起，您不再需要指定 `broker名`，但继续保留 `WITH BROKER` 关键字。
...
如果存储系统为 Blob 存储，请按如下配置 `StorageCredentialParams`：
...
- 基于 SAS Token 进行认证和授权
  ...
  | **参数**                  | **是否必须** | **说明**                                 |
  | ------------------------- | ------------ | ---------------------------------------- |
  | azure.blob.account_name   | 是           | Blob 存储账户的用户名。              |
  | azure.blob.container_name | 是           | 用于存储数据的 Blob 容器的名称。         |
  | azure.blob.sas_token      | 是           | 用于访问 Blob 存储账户的 SAS Token。 |
...
- 基于托管服务标识进行认证和授权
  ...
  | **参数**                                 | **是否必须** | **说明**                                                     |
  | ---------------------------------------- | ------------ | ------------------------------------------------------------ |
  | azure.adls1.use_managed_service_identity | 是           | 指定是否开启托管服务标识授权方式。设置为 `true`。 |
...
- 基于服务主体进行认证和授权
  ...
  | **参数**                      | **是否必须** | **说明**                                                     |
  | ----------------------------- | ------------ | ------------------------------------------------------------ |
  | azure.adls1.oauth2_client_id  | 是           | 服务主体的客户（应用程序）标识。               |
  | azure.adls1.oauth2_credential | 是           | 新建客户（应用程序）秘密。                           |
...
- 基于托管标识进行认证和授权
  ...
  | **参数**                                | **是否必须** | **说明**                                                |
  | --------------------------------------- | ------------ | ------------------------------------------------------- |
  | azure.adls2.oauth2_use_managed_identity | 是           | 指定是否开启托管标识授权方式。设置为 `true`。 |
  | azure.adls2.oauth2_tenant_id            | 是           | 数据所属的租户的 ID。                               |
  | azure.adls2.oauth2_client_id            | 是           | 托管标识的客户（应用程序）标识。           |
...
- 基于共享密钥进行认证和授权
  ...
  | **参数**                    | **是否必须** | **说明**                                   |
  | --------------------------- | ------------ | ------------------------------------------ |
  | azure.adls2.storage_account | 是           | Data Lake Storage Gen2 账户的用户名。      |
...
- 基于服务主体进行认证和授权
  ...
  | **参数**                           | **是否必须** | **说明**                                                     |
  | ---------------------------------- | ------------ | ------------------------------------------------------------ |
  | azure.adls2.oauth2_client_id       | 是           | 服务主体的客户（应用程序）标识。               |
  | azure.adls2.oauth2_client_secret   | 是           | 新建客户（应用程序）密码。                           |
...
+ {R}
  ...
  **超时时间 {T} (源数据文件的总大小 x 源数据文件及相关物化视图的个数)/平均导入速度**

    > **说明**
    >
    > “平均导入速度” {T} 目前 StarRocks 集群的平均导入速度。由于每个 StarRocks 集群的机器环境不同、且集群允许的并发查询任务数也不同，因此，StarRocks 集群的平均导入速度 {T} 根据历史导入速度进行推测。

   例如，要导入一个 1 GB 的数据文件，该数据文件包含 2 个物化视图，当前 StarRocks 集群的平均导入速度 {T} 10 MB/s。在这种情况下，根据公式计算出来时长为 102 秒：

   **{T} (1 x 1024 x 3)/10 = 307.2（秒）
