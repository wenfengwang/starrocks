...
StarRocks 从 3.0 版本开始支持 `AUTO_INCREMENT` 列属性，以简化数据管理。本文将介绍 `AUTO_INCREMENT` 列属性的应用场景、用法和特性。

## 功能介绍

当插入新记录时，StarRocks 会自动为该记录的自增列分配一个表内全局唯一的整数值作为自增 ID，并且后续值会自动增加。自增列可以简化数据管理，同时也可加速一些查询场景。以下是一些自增列的应用场景：

- 主键：自增列可用于生成主键，确保每条记录都有一个唯一的标识符，便于查询和管理数据。
- 关联表：在多个表之间进行关联时，可以使用自增列作为 Join Key，相比使用如 UUID 等字符串类型的列能够提高查询速度。
- 高基数列的精确去重计数：将自增列的 ID 值作为字典唯一值列，相比用字符串直接精确去重计数，查询速度能够提升数倍甚至十数倍。
...
假设 StarRocks 集群具有一个 FE 节点和两个 BE 节点。创建表 `test_tbl3` 并插入五行数据，如下所示：
