...
StarRocks 从 3.1 版本开始支持生成列（Generated Column）。此功能支持预先计算并存储表达式的结果，从而加速包含复杂表达式的查询，并支持[查询改写](#查询改写)，因此大大提高了查询性能。

您可以定义一个或多个生成列来存储表达式的结果。当执行包含相同表达式的查询时，优化器会进行查询改写，用生成列替换表达式。或者您也可以直接查询生成列的数据。

由于在导入阶段计算表达式会占用一定的时间，因此建议您**提前评估生成列对导入性能的影响**。此外，由于建表后新增或修改生成列比较耗时且开销较大，因此**建议您[在建表时创建生成列](#建表时创建生成列推荐)，而不是建表后新增生成列**。
...
1. 创建表 `test_tbl3`，包含五列，其中列 `newcol1` 和 `newcol2` 是生成列，分别是引用普通列 `data_array` 和 `data_json` 通过计算表达式后生成的列。建表成功后插入一行数据。
      ...
      -- 插入一行数据
      INSERT INTO test_tbl3 VALUES (1, [1,2], parse_json('{"a" : 1, "b" : 2}'));
      ```

2. 修改生成列的数据类型和表达式。

      ```SQL
      -- 修改生成列 newcol1 的数据类型和表达式
      ALTER TABLE test_tbl3 MODIFY COLUMN newcol1 String AS where_clause(data_array, x -> x > 1);
      
      -- 修改生成列 newcol2 的数据类型和表达式
      ALTER TABLE test_tbl3 MODIFY COLUMN newcol2 BIGINT AS json_size(data_json);
      ```

    **注意事项：**

    - 不支持修改生成列的名称。
    - 如果生成列被查询，则生成列的修改会在查询开始之前生效。
    - 如果生成列正在被查询或索引使用，则修改生成列时需要排他锁，会导致表的读写操作被阻塞。
...
3. 查询表的数据。
    ...
    返回结果显示，表中已经修改生成列 `newcol1` 和 `newcol2` 的数据类型和表达式，并且 StarRocks 计算表达式自动得出该生成列的值。
