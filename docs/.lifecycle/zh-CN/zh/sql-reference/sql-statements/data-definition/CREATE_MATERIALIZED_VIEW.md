...
- `storage_cooldown_time`: 当存储介质设置为 SSD 时，在此时间点后将此分区从 SSD 降温至 HDD。设置的时间必须大于当前时间。如不指定该属性，默认不自动降温。取值格式为："yyyy-MM-dd HH:mm:ss"。
- `partition_ttl`: 物化视图分区的生存时间 (TTL)。在指定时间范围内的分区数据将被保留，过期的分区将被自动删除。单位：`YEAR`、`MONTH`、`DAY`、`HOUR` 和 `MINUTE`。例如，您可以将此属性设置为 `2 MONTH`（2个月）。建议使用此属性，不推荐使用 `partition_ttl_number`。该属性自 v3.1.5 起支持。
- `partition_ttl_number`：需保留的最近的物化视图分区数量。对于分区开始时间小于当前时间的分区，当数量超过该值之后，多余的分区将被删除。StarRocks 将根据 FE 配置项 `dynamic_partition_check_interval_seconds` 中的时间间隔定期检查物化视图分区，并自动删除过期分区。在[动态分区](../../../table_design/dynamic_partitioning.md)场景下，提前创建的未来分区将不会被纳入 TTL 考虑。默认值：`-1`。当值为 `-1` 时，将保留物化视图所有分区。
- `partition_refresh_number`：单次刷新中，最多刷新的分区数量。如果需要刷新的分区数量超过该值，StarRocks 将拆分此刷新任务，并分批完成。仅当前一批分区刷新成功时，StarRocks 将继续刷新下一批分区，直至所有分区完成刷新。如果其中有分区刷新失败，将不会产生后续的刷新任务。默认值：`-1`。当值为 `-1` 时，将不拆分刷新任务。
- `excluded_trigger_tables`：在此属性中列出的基表，其数据产生变化时不会触发对应物化视图的自动刷新。此参数仅针对导入触发式刷新，通常需要与属性 `auto_refresh_partitions_limit` 搭配使用。格式：`[db_name.]table_name`。默认值为空字符串。当值为空字符串时，任意的基表数据变化都将触发对应物化视图刷新。
- `auto_refresh_partitions_limit`：当触发物化视图刷新时，需要刷新的最近的物化视图分区数量。您可以通过此属性限制刷新的范围，降低刷新代价，但因为仅有部分分区刷新，有可能导致物化视图数据与基表不一致。默认值：`-1`。当参数值为 `-1` 时，StarRocks 将刷新所有分区。当参数值为正整数 N 时，StarRocks 将已存在的分区按时间先后排序，并刷新当前分区和 N-1 个历史分区。如果分区数不足 N，则刷新所有已存在的分区。如果物化视图存在提前创建的未来分区，将刷新所有提前创建的分区。
- `mv_rewrite_staleness_second`：如果当前物化视图的上一次刷新在此属性指定的时间间隔内，则此物化视图可直接用于查询改写，无论基表数据是否更新。如果上一次刷新时间早于此属性指定的时间间隔，StarRocks 将通过检查基表数据是否变更决定该物化视图是否可用于查询改写。单位：秒。该属性自 v3.0 起支持。
...
- `query_rewrite_consistency`：指定当前异步物化视图的查询改写规则。该属性自 v3.2 起支持。有效值：
  - `disable`：禁用基于该异步物化视图的自动查询改写。
  - `checked`（默认值）：在物化视图满足时效性要求时启用自动查询改写，即：
    - 如果未指定 `mv_rewrite_staleness_second`，则只有当物化视图的数据与所有基表中的数据一致时，才可用于查询改写。
    - 如果指定了 `mv_rewrite_staleness_second`，则只有在其最后刷新在 staleness 时间间隔内时，才可用于查询改写。
...
异步物化视图本质上是一张实体表。您可以将其作为普通表进行任何**除直接导入数据**以外的操作。
...
1. 创建一个仅包含原始表 （k1，k2）列的物化视图。
    ...
    ```sql
    +-----------------+-------+--------+------+------+---------+-------+
    | 索引名称        | 字段  | 类型   | 可空 | 键   | 默认值  | 额外  |
    +-----------------+-------+--------+------+------+---------+-------+
    | k1_k2           | k1    | INT    | 是   | 是   | N/A     |       |
    |                 | k2    | INT    | 是   | 是   | N/A     |       |
    +-----------------+-------+--------+------+------+---------+-------+
    ```
...
3. 创建一个以 k1，k2 分组，k3 列为 SUM 聚合的物化视图。
    ...
    | 索引名称        | 字段  | 类型   | 可空 | 键     | 默认值  | 额外  |
    +-----------------+-------+--------+------+-------+---------+-------+
    | k1_k2_sumk3     | k1    | INT    | 是   | 是    | N/A     |       |
    |                 | k2    | INT    | 是   | 是    | N/A     |       |
    |                 | k3    | BIGINT | 是   | 否    | N/A     | SUM   |
    +-----------------+-------+--------+------+-------+---------+-------+
    ```
