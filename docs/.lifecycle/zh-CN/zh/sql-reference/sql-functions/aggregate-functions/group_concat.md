...
> **注意**
>
> 从 v3.0.6 和 v3.1.3 版本起，分隔符必须使用 `SEPARATOR` 关键字来声明。 例如，`select group_concat(name SEPARATOR '-') as res from ss;`。
...
2. 使用 group_concat 对值进行拼接。
   ...
   示例四：将相同 ID 的 `name` 和 `subject` 组合按照 `score` 的升序进行连接。例如返回示例中的第二行数据 `TomMath,TomEnglish` 是将 ID 为 1 的 `TomMath` 和 `TomEnglish` 组合按照 `score` 进行升序排序。
