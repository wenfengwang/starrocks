...
```SQL
函数名(参数) OVER([partition_by_clause] [order_by_clause] [order_by_clause window_clause])
partition_by_clause ::= PARTITION BY 表达式 [, 表达式 ...]
order_by_clause ::= ORDER BY 表达式 [ASC | DESC] [, 表达式 [ASC | DESC] ...]
```
...
```SQL
COUNT(表达式) [OVER (analytic_clause)]
```
...
## 使用 DENSE_RANK() 窗口函数
```SQL
DENSE_RANK() 函数用来为特定窗口中的数据排名。当函数中出现相同排名时，下一行的排名为相同排名数加 1。因此，DENSE_RANK() 返回的序号是连续的数字。而 RANK() 返回的序号有可能是不连续的数字。
...
* IGNORE NULLS：从 3.0 版本开始，LAG() 支持 IGNORE NULLS，即是否在计算结果中忽略 NULL 值。如果不指定 IGNORE NULLS，默认返回结果会包含 NULL 值。比如，如果指定的当前行之前的第 offset 行的值为 NULL，则返回 NULL，参考示例一。如果指定了 IGNORE NULLS，向前遍历 offset 行时会忽略取值为 NULL 的行，继续向前遍历非 NULL 值。如果指定了 IGNORE NULLS，但是在当前行之前并不存在 offset 个非 NULL 值，则返回 NULL或default (如果指定)，参考示例二。
...
可以看到对于第 1 行数据 NULL，往后遪历两行对应的数据是 NULL，因为未指定 IGNORE NULLS，允许返回结果包含 NULL，所以返回 NULL。
...
对于第 1 行数据 NULL，往后遪历两行对应的值是 NULL，因为指定了 IGNORE NULLS，会忽略这一行，继续往前遪历，因此返回第 4 行的 2。
...
从2.4版本开始，该函数支持设置 `rows between n preceding and n following`，即支持计算当前行前n行及后 `n` 行中的最大值。比如要计算当前行前3行和后2行中的最大值，语句可写为：
...
QUALIFY 子句用于过滤窗口函数的结果。在 SELECT 语句中，可以使用 QUALIFY 来设置过滤条件，从多条记录中筛选符合条件的记录。QUALIFY 与聚合函数中的 HAVING 子句功能类似。该函数从2.5版本开始支持。
...
FROM (SELECT DATE,
             PROVINCE_CODE,
```SQL
SELECT TOTAL_SCORE, ROW_NUMBER() OVER(PARTITION BY PROVINCE_CODE ORDER BY TOTAL_SCORE) AS SCORE_ROWNUMBER
FROM example_table) T1
WHERE T1.SCORE_ROWNUMBER = 1;
```

通过使用 QUALIFY，可以简化语句如下：
