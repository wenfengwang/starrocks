...
外部表查询时每个 Scan 算子能同时下发的 I/O 任务的最大数量。取值为整数，默认值 16。目前外部表查询时会使用自适应算法来调整并发 I/O 任务的数量，通过 `enable_connector_adaptive_io_tasks` 开关来控制，默认打开。
...
外部表查询时是否使用自适应策略来调整 I/O 任务的并发数。默认打开。如果未开启自适应策略，可以通过 `connector_io_tasks_per_scan_operator` 变量来手动设置外部表查询时的 I/O 任务并发数。
...
是否开启查询缓存。取值范围：true 和 false。true 表示开启，false 表示关闭（默认值）。开启该功能后，只有当查询满足[查询缓存](../using_starrocks/query_cache.md#应用场景) 所述条件时，才会启用查询缓存。
...
全局运行时筛选开关。运行时筛选（简称 RF）在数据上运行时进行筛选，筛选通常发生在 Join 阶段。当多表进行 Join 时，往往伴随着谓词下推等优化手段进行数据筛选，以减少 Join 表的数据扫描以及 shuffle 等阶段产生的 IO，从而提升查询性能。StarRocks 中有两种 RF，分别是 Local RF 和 Global RF。Local RF 应用于 Broadcast Hash Join 场景。Global RF 应用于 Shuffle Join 场景。

默认值 `true`，表示打开全局运行时筛选开关。关闭该开关后, 不生成 Global RF, 但是依然会生成 Local RF。

### enable_multicolumn_global_runtime_filter

多列全局运行时筛选开关。默认值为 false，表示关闭该开关。
...
* `default`（默认）：启用异步物化视图的自动查询改写，并允许优化器根据 Cost 决定是否可以使用物化视图改写查询。如果查询无法改写，则直接查询基表中的数据。
...
用于兼容 JDBC 连接池 C3P0。该变量值决定了服务端发送给客户端或客户端发送给服务端的最大 packet 大小，单位为字节，默认值为 32 MB。当客户端报 `PacketTooBigException` 异常时，可以考虑调大该值。
...
可用于查询改写的嵌套物化视图的最大层数。类型：整数。取值范围：[1, +∞)。默认值：`3`。取值为 `1` 表示只可使用基于基表创建的物化视图用于查询改写。
...
缓存条目大小的字节数上限，触发穿越模式的阈值。取值范围：0 ~ 9223372036854775807。默认值：4194304。当一个 Tablet 上产生的计算结果的字节数或者行数超过 `query_cache_entry_max_bytes` 或 `query_cache_entry_max_rows` 指定的阈值时，则查询采用穿越模式执行。当 `query_cache_entry_max_bytes` 或 `query_cache_entry_max_rows` 取值为 0 时, 即便 Tablet 产生结果为空，也采用穿越模式。

### query_cache_entry_max_rows (2.5 及以后)

缓存条目的行数上限，见 `query_cache_entry_max_bytes` 描述。默认值：409600。
