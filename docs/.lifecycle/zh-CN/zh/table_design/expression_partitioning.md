...
| `partition_live_number` | 否       | 保留最近多少数量的分区。最近是指分区按时间的先后顺序进行排序，以**当前时间**为基准，然后从后往前数指定个数的分区进行保留，其余（更早的）分区会被删除。后台会定期调度任务来管理分区数量，调度间隔可以通过 FE 动态参数 `dynamic_partition_check_interval_seconds` 配置，默认为 600 秒，即 10 分钟。假设当前为 2023 年 4 月 4 日，`partition_live_number` 设置为 `2`，分区包含 `p20230401`、`p20230402`、`p20230403`和`p20230404`，则分区 `p20230403`和 `p20230404` 会保留，其他分区会删除。如果导入了脏数据，比如未来时间 4 月 5 日和 6 日的数据，导致分区包含 `p20230401`、`p20230402`、`p20230403`、`p20230404`、`p20230405`和`p20230406`，则分区 `p20230403`、`p20230404`、`p20230405`和`p20230406` 会保留，其他分区会删除。|
...
导入如下两行数据，则 StarRocks 会根据导入数据的日期范围自动创建两个分区  `p20230226`和 `p20230227`，范围分别为 [2023-02-26 00:00:00,2023-02-27 00:00:00) 和 [2023-02-27 00:00:00,2023-02-28 00:00:00)。如果后续导入数据的日期属于这两个范围，则都会自动划分至对应分区。
...
+-------------+---------------+----------------+---------------------+--------------------+--------+--------------+------------------------------------------------------------------------------------------------------+--------------------+---------+----------------+---------------+---------------------+--------------------------+----------+------------+----------+
| 17138       | p20230226     | 2              | 2023-07-19 17:53:59 | 0                  | NORMAL | event_day    | [类型: [日期时间]; 键值: [2023-02-26 00:00:00]; ..类型: [日期时间]; 键值: [2023-02-27 00:00:00]; ) | event_day, site_id | 6       | 3              | HDD           | 9999-12-31 23:59:59 | NULL                     | 0B       | 假         | 0        |
| 17113       | p20230227     | 2              | 2023-07-19 17:53:59 | 0                  | NORMAL | event_day    | [类型: [日期时间]; 键值: [2023-02-27 00:00:00]; ..类型: [日期时间]; 键值: [2023-02-28 00:00:00]; ) | event_day, site_id | 6       | 3              | HDD           | 9999-12-31 23:59:59 | NULL                     | 0B       | 假         | 0        |
+-------------+---------------+----------------+---------------------+--------------------+--------+--------------+------------------------------------------------------------------------------------------------------+--------------------+---------+----------------+---------------+---------------------+--------------------------+----------+------------+----------+
2 行 在集合中 (0.00 秒)
...
DISTRIBUTED BY HASH(`id`) 
PROPERTIES(
    "partition_live_number" = "3" -- 仅保留最近 3 个分区。
);
```
...
当导入数据时，StarRocks 将会根据数据和分区表达式定义的规则自动创建分区。

需要注意的是，如果您在建表时采用了表达式分区，并希望使用 [INSERT OVERWRITE](../loading/InsertInto.md#通过-insert-overwrite-select-语句覆盖写入数据) 来覆盖写入**特定分区**的数据，无论该分区是否已创建，您都需要在 `PARTITION()` 中明确提供分区范围。这与 Range 分区或 List 分区不同，后者只需要在 `PARTITION (partition_name)` 中提供分区名称即可。

如果您在建表时采用了时间函数表达式分区，那么在覆盖写入特定分区数据时，您需要提供该分区的开始范围（分区粒度需与建表时配置的分区粒度相同）。如果该分区尚不存在，那么在导入数据时系统将自动创建它。
...
如果您在建表时采用了列表达式分区，那么在覆盖写入特定分区数据时，您需要提供该分区包含的分区列值。如果该分区尚不存在，那么在导入数据时系统将自动创建它。
...
要查看自动创建的分区的详细信息，您需要使用 `SHOW PARTITIONS FROM <table_name>` 语句。而 `SHOW CREATE TABLE <table_name>` 语句返回的结果则仅包含建表时所配置的表达式分区语法。
...
- 从 v3.1.0 版本起，StarRocks [存算分离模式](../deployment/shared_data/s3.md)支持[时间函数表达式分区](#时间函数表达式分区)。从 v3.1.1 版本起，StarRocks 存算分离模式支持[列表达式分区](#列表达式分区自-v31)。
- 使用 CTAS 创建表时，暂不支持表达式分区。
- 暂不支持使用 Spark Load 向表达式分区的表中导入数据。
- 使用 `ALTER TABLE <table_name> DROP PARTITION <partition_name>` 语句删除列表达式分区时，分区会直接被删除且无法恢复。
- 列表达式分区暂不支持[备份与恢复](../administration/Backup_and_restore.md)功能。
- 如果使用了表达式分区，仅支持回滚至 2.5.4 或更高版本。