```yaml
---
displayed_sidebar: "Chinese"   # {T}
---

# 聚合模型  # {T}

建表时，支持定义排序键和指标列，并为指标列指定聚合函数。当多条数据具有相同的排序键时，指标列会进行聚合。在分析统计和汇总数据时，聚合模型能够减少查询时所需要处理的数据，提升查询效率。   # {T}
...
例如，导入如下数据至聚合模型中，排序键为 Date、Country：   # {T}
...
> - 自 2.5.7 版本起，StarRocks 支持在建表和新增分区时自动设置分桶数量 (BUCKETS)，您无需手动设置分桶数量。更多信息，请参见 [确定分桶数量](../Data_distribution.md#确定分桶数量)。  # {T}

## 使用说明  # {T}

- 排序键的相关说明：  # {T}
  - 在建表语句中，**排序键必须定义在其他列之前**。  # {T}
  - 排序键可以通过 `AGGREGATE KEY` 显式定义。

    > - 如果 `AGGREGATE KEY` 未包含全部维度列（除指标列之外的列），则建表会失败。  # {T}
    > - 如果不通过 `AGGREGATE KEY` 显示定义排序键，则默认除指标列之外的列均为排序键。  # {T}
...
> 导入时，仅支持全字段导入，即导入任务需要涵盖表的所有列，例如示例中的 `site_id`、`date`、`city_code` 和 `pv` 四个列。  # {T}
```