...
如果您将 StarRocks 部署在 GCP 上的 VM 实例的同时，使用该 VM 实例模拟一个 Service Account，从而使 StarRocks继承该 Service Account 访问 GCS 的权限，请按如下配置 `StorageCredentialParams`：
...
```
```
| gcp.gcs.service_account_private_key    | ""         | "-----BEGIN PRIVATE KEY----xxxx-----END PRIVATE KEY-----\n" | 创建 Meta 服务帐户时生成的JSON文件中的私钥。 |
| gcp.gcs.impersonation_service_account  | ""         | "hello"                                                     | 需要模拟的目标数据服务帐户。                        |
```