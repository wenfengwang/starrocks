...
|  | 内部表 | 外部表 | 大小查询隔离 | 短查询资源组 | INSERT INTO、Broker Load 计算资源隔离 | 定期加载、流加载、Schema 变更资源隔离 |
|---|---|---|---|---|---|---|
| 2.2 | √ | × | × | × | × | × |
| 2.3 | √ | √ | √ |√ | × | × |
| 2.4 | √ | √ | √ |√ | × | × |
...
- `mem_limit`：该资源组在当前 BE 节点可使用于查询的内存（query_pool）占总内存的百分比（%）。取值范围为 (0,1)。
  > 说明：查询 query_pool 的方法，参见 [内存管理](Memory_management.md)。
- `concurrency_limit`：资源组中并发查询数的上限，用以防止并发查询提交过多而导致的过载。只有在大于 0 的情况下才生效，默认为 0。
- `max_cpu_cores`：资源组在单个 BE 节点中使用的 CPU 核数上限。只有在设置为大于 0 的情况下才生效。取值范围：(0, `avg_be_cpu_cores`]，其中 `avg_be_cpu_cores` 表示所有 BE 的 CPU 核数的平均值。默认为 0。

在以上资源限制的基础上，您可以通过以下大查询限制进一步对资源组进行如下的配置：

- `big_query_cpu_second_limit`：大查询任务在每个 BE 上可以使用 CPU 的时间上限，其中的并行任务将累加 CPU 实际使用时间。单位为秒。只有在大于 0 的情况下才生效，默认为 0。
- `big_query_scan_rows_limit`：大查询任务在每个 BE 上可以扫描的行数上限。只有在大于 0 的情况下才生效，默认为 0。
- `big_query_mem_limit`：大查询任务在每个 BE 上可以使用的内存上限。单位为字节（Byte）。只有在大于 0 的情况下才生效，默认为 0。

> **说明**
>
> 当资源组中运行的查询超过以上大查询限制时，查询将会终止，并返回错误信息。您还可以在 FE 节点的 **fe.audit.log** `ErrorCode` 列中查看错误信息。

资源组的类型 `type` 支持 `short_query` 与 `normal`。

- 默认为 `normal` 类型的资源组，无需通过 `type` 参数指定。
- 当 `short_query` 类型的资源组有查询正在运行时，当前 BE 节点会为其预留 `short_query.cpu_core_limit` 的 CPU 资源，即所有 `normal` 类型的资源组的 CPU 核数使用上限会被硬限制为 BE 节点核数减去 `short_query.cpu_core_limit`。
- 当 `short_query` 资源组没有查询正在运行时，所有 `normal` 类型的资源组的 CPU 核数没有硬性限制。

> **注意**
>
> - 您最多只能创建一个 `short_query` 类型的资源组。
> - StarRocks 不会硬性限制 `short_query` 资源组的 CPU 资源。
...
- `plan_mem_cost_range`：系统预估的查询内存消耗范围。格式为`(DOUBLE, DOUBLE]`。默认为 NULL，表示无此限制。fe.audit.log 中的`PlanMemCost`列表示系统预估的该查询的内存消耗。自 v3.1.4 版本开始，StarRocks 支持此参数。

系统在为查询任务匹配分类器时，查询任务的信息与分类器的条件必须完全一致，才被认为是匹配。如果存在多个分类器的条件与查询任务完全匹配，则需要计算各个分类器的匹配度。其中只有匹配度最高的分类器会生效。

> **说明**
>
> 您可以在 FE 节点**fe.audit.log**的`ResourceGroup`列或`EXPLAIN VERBOSE <query>`的`RESOURCE GROUP`列中查看特定查询任务最终匹配的资源组，参见[查看查询命中的资源组](#查看查询命中的资源组)。

匹配度的计算方式如下：

- 如果`user`一致，该分类器的匹配度增加 1。
- 如果`role`一致，该分类器的匹配度增加 1。
- 如果`query_type`一致，该分类器的匹配度增加 1 + 1/分类器的`query_type`数量。
- 如果`source_ip`一致，该分类器的匹配度增加 1 + (32 - cidr_prefix)/64。
- 如果查询的`db`匹配，匹配度增加 10。
- 如果查询的 CPU 消耗在`plan_cpu_cost_range`范围内，该分类器的匹配度增加 1。
- 如果查询的内存消耗在`plan_mem_cost_range`范围内，该分类器的匹配度增加 1。

例如，对于多个与查询任务匹配的分类器，条件数量越多的分类器，其匹配度越高。

```Plain
-- 因为分类器B的条件比A多，所以B的匹配度比A高。
classifier A (user='Alice')
classifier B (user='Alice', source_ip = '192.168.1.0/24')
```

如果分类器的条件数量相同，条件描述越精确的分类器，其匹配度越高。

```Plain
-- 因为分类器B指定的`source_ip`范围较小，相比于A，B的匹配度更高。
classifier A (user='Alice', source_ip = '192.168.1.0/16')
classifier B (user='Alice', source_ip = '192.168.1.0/24')

-- 因为分类器C指定的查询类型数量较少，与D相比，C的匹配度更高。
classifier C (user='Alice', query_type in ('select'))
classifier D (user='Alice', query_type in ('insert','select'))
```

如果多个分类器的匹配度相同，则随机选择其中一个。

```Plain
-- 如果一个查询同时查询了db1和db2，并且匹配的分类器中E和F的匹配度最高，那么会从E和F中随机选择一个。
classifier E (db='db1')
classifier F (db='db2')
```
...
您可以通过创建资源组并为不同的查询任务设置相应的分类器来隔离计算资源。

### 启用资源组

要使用资源组，需要通过设置会话变量来启用 Pipeline 引擎。
...
对于导入任务，还需要开启 FE 配置项`enable_pipeline_load`以启用 Pipeline 引擎。该参数从 v2.5.0 版本起支持。
...
> **说明**
>
> 从 v3.1.0 版本起，默认启用资源组功能。会话变量`enable_resource_group`不再使用。
...
WITH (
    ...
    "type" = "str" -- 资源组类型，可为 normal 或 short_query。
);
```
