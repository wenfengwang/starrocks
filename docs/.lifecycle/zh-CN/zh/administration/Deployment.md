...
```sql
ALTER SYSTEM ADD FOLLOWER "主机:端口";
```

* 添加 Observer FE  节点。

```sql
ALTER SYSTEM ADD OBSERVER "主机:端口";
```

参数：

* `主机`：机器的 IP 地址。如果机器存在多个 IP 地址，则该项为 `priority_networks` 设置项下设定的唯一通信 IP 地址。
* `端口`：`edit_log_port` 设置项下设定的端口，默认为 `9010`。
...
```sql
ALTER SYSTEM DROP FOLLOWER "主机:端口";
```

* 删除 Observer FE  节点。

```sql
ALTER SYSTEM DROP OBSERVER "主机:端口";
```
...
```shell
./bin/start_fe.sh --helper 主机:端口 --daemon
```

参数：

* `主机`：机器的IP 地址。如果机器存在多个 IP 地址，则该项为 `priority_networks` 设置项下设定的唯一通信 IP 地址。
* `端口`：`edit_log_port` 设置项下设定的端口，默认为 `9010`。
