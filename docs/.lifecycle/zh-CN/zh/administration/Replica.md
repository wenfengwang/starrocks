...
This article introduces how to manage data replicas in StarRocks.

> **注意**
>
> To ensure metadata consistency, you need to perform replica management operations on the Leader FE node.

## Fixing Replicas

The TabletChecker is a resident background process that regularly checks the status of all tablets. For unhealthy tablets, the TabletScheduler will schedule and repair them. The actual repair operation is performed by the Clone task on the BE node. The FE node is only responsible for generating these Clone tasks.

> Note:
>
> * The main idea behind replica repair is to first create or supplement replicas to reach the desired number, and then delete the extra replicas.
> * A Clone task is the process of copying specified data from a specified remote BE to a specified destination BE.

### Viewing Replica Status

You can view the health status of replicas.

1. **Check the status of all replicas in the cluster.**
    ...
    The `UnhealthyTabletNum` column shows how many tablets in the corresponding Database are in an unhealthy state.
    The `InconsistentTabletNum` column shows how many tablets in the corresponding Database are in an inconsistent replica state.

    The `Total` row provides statistics for the entire cluster. Under normal circumstances, `UnhealthyTabletNum` and `InconsistentTabletNum` should be 0. If not, you can further check which tablets are affected. For example, in the above figure, the UM_TEST database has 1 unhealthy tablet.

    You can use the following command to query unhealthy tablet statuses.

    ```sql
    SHOW PROC '/statistic/DbId';
    ```

    `DbId` is the ID corresponding to the database.
    ...
    The query returns the unhealthy tablet ID as 40467980.
2. **Check the replica status of a table or partition.**
    You can use the following command to view the replica status of a specified table or partition and filter the status using the WHERE clause.
    ...
    The result shows the status of all replicas. If the `IsBad` column is `true`, it means the replica is damaged. The `Status` column will display other status. For more information, please refer to [ADMIN SHOW REPLICA STATUS](../sql-reference/sql-statements/Administration/ADMIN_SHOW_REPLICA_STATUS.md).

    You can also use the following command to view some additional information about replicas in a specified table.
