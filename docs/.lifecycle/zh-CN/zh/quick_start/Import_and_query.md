...
为了适应不同的数据导入需求，StarRocks 系统提供了五种不同的导入方式，以支持不同的数据源或者导入方式。
...
我们建议您通过设置Pipeline执行引擎变量来实现。您还可以通过调整 [Fragment](../introduction/Features.md#mpp-分布式执行框架) 实例的并行数量 `set parallel_fragment_exec_instance_num = 8;` 来设置查询并行度，以提高 CPU 资源利用率和查询效率。有关详细的参数介绍及设置，请参考[查询并行度相关参数](../administration/Query_management.md)。
...
- 查看查询计划。
  ...
  > **注意**
  >
  > StarRocks 1.19以前版本需要使用`explain sql`来查看查询计划。
...
- 开启Profile上报。
  ...
  > **注意**
  >
  > 通过此方式设置Profile上报仅在当前session中生效。

- 社区版用户可以通过`http://FE_IP:FE_HTTP_PORT/query`查看当前的查询和Profile信息。
- 企业版用户可以在StarRocks Manager的查询页面查看图形化Profile展示，点击查询链接可以在**执行时间**页面看到树状展示，并可以在**执行详情**页面看到完整的Profile详细信息。如果以上方法仍然不达到预期效果，您可以发送执行详情页面的文本到社区或者技术支持群以寻求帮助。

有关Plan和Profile的详细介绍，请参考[查询分析](../administration/Query_planning.md)和[性能优化](../administration/Profiling.md)章节。