...
#### 数据类型映射
StarRocks支持导入所有类型的Avro数据。导入Avro数据至StarRocks时，其类型映射关系如下：
...
| Avro           | StarRocks                                        |
| -------------- | ------------------------------------------------ |
| record         | 将RECORD类型的字段或者其子字段中作为JSON导入 |
| enums          | STRING                                           |
| arrays         | ARRAY                                            |
| maps           | JSON                                             |
...
- StarRocks暂时不支持schema evolution，每次仅从schema registry服务获取最新版本的schema信息。

- 每条Kafka消息中应仅包含单条Avro数据。
...
> **注意**
>
> StarRocks只支持查看当前正在运行中的导入作业，不支持查看已经停止和未开始的导入作业。

### 查看导入任务

执行 [SHOW ROUTINE LOAD TASK](../sql-reference/sql-statements/data-manipulation/SHOW_ROUTINE_LOAD_TASK.md)，查看导入作业`example_tbl2_ordertest2`中一个或多个导入任务的信息。比如当前有多少任务正在运行，消费分区及进度`DataSourceProperties`，以及对应的Coordinator BE节点`BeId`。
...
执行[RESUME ROUTINE LOAD](../sql-reference/sql-statements/data-manipulation/RESUME_ROUTINE_LOAD.md)，恢复导入作业。导入作业会先短暂地进入**NEED_SCHEDULE**状态，表示正在重新调度导入作业，一段时间后会恢复至**RUNNING**状态，继续消费Kafka消息并且导入数据。您可以执行[SHOW ROUTINE LOAD](../sql-reference/sql-statements/data-manipulation/SHOW_ROUTINE_LOAD.md)语句查看已恢复的导入作业。
...
Before the modification, you need to execute PAUSE ROUTINE LOAD to pause the import job. Then execute the [ALTER ROUTINE LOAD](../sql-reference/sql-statements/data-manipulation/ALTER_ROUTINE_LOAD.md) statement to modify the parameter configuration of the import job. After the modification is successful, you need to execute [RESUME ROUTINE LOAD](../sql-reference/sql-statements/data-manipulation/RESUME_ROUTINE_LOAD.md) to resume the import job. Then execute the [SHOW ROUTINE LOAD](../sql-reference/sql-statements/data-manipulation/SHOW_ROUTINE_LOAD.md) statement to view the modified import job.

For example, when the number of live BE nodes increases to 6 and the partitions to be consumed are `"0,1,2,3,4,5,6,7"`, if you want to increase the actual import parallelism, you can use the following statement to increase the desired task parallelism `desired_concurrent_number` to `6` (greater than or equal to the number of live BE nodes), and adjust the partitions to be consumed and the starting consumption positions.

> **Note**
>
> As the actual import parallelism is determined by the minimum value of multiple parameters, you also need to ensure that the value of the FE dynamic parameter `max_routine_load_task_concurrent_num` is greater than or equal to `6`.
...
Executing [STOP ROUTINE LOAD](../sql-reference/sql-statements/data-manipulation/STOP_ROUTINE_LOAD.md) can stop the import job. The import job will enter the **STOPPED** state, indicating that the import job has ended and cannot be resumed. When executing the SHOW ROUTINE LOAD statement again, the stopped import job will not be visible.

For example, you can use the following statement to stop the import job `example_tbl2_ordertest2`:
...
Please refer to [Routine Load FAQ](../faq/loading/Routine_load_faq.md).