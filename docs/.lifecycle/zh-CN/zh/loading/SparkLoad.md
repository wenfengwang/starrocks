...
* 步骤 3: 提交 load 命令，要求导入的 StarRocks 表中的列必须在 hive 外部表中存在。
...
**4.** Perform a left join of distinct-value-table and dict-table, calculate the added set of unique values, and then use window functions to encode this set. At this point, the original values of the unique column will have an additional column for the encoded values, and finally write back these two columns of data to dict-table.

**5.** Join dict-table with hive-table to replace the original values in hive-table with integer-encoded values.

**6.** The hive-table will be read by the next step of data preprocessing, and after calculation, it will be imported into StarRocks.

### Spark Program Import

For a complete example of spark load import, refer to the demo on github: [sparkLoad2StarRocks](https://github.com/StarRocks/demo/blob/master/docs/03_sparkLoad2StarRocks.md).

---

## Common Issues

* Q: Error - When running with master 'yarn' either HADOOP-CONF-DIR or YARN-CONF-DIR must be set in the environment.

  A: When using Spark Load, the HADOOP-CONF-DIR environment variable is not configured in the spark-env.sh file of the Spark client.

* Q: Error - When submitting Spark job using the spark-submit command, "Cannot run program "xxx/bin/spark-submit": error = 2, No such file or directory".

  A: When using Spark Load, the `spark_home_default_dir` configuration item is not specified or specified incorrectly as the root directory of the spark client.

* Q: Error - File xxx/jars/spark-2x.zip does not exist.

  A: When using Spark Load, the spark-resource-path configuration item does not point to the packaged zip file. Check whether the file path and file name in the configuration are consistent.

* Q: Error - yarn client does not exist in path: xxx/yarn-client/hadoop/bin/yarn.

  A: When using Spark Load, the yarn-client-path configuration item is not specified for the yarn executable file.

* Q: Error - Cannot execute hadoop-yarn/bin/../libexec/yarn-config.sh.

  A: When using CDH Hadoop, the HADOOP_LIBEXEC_DIR environment variable needs to be configured. Due to the different locations of hadoop-yarn and hadoop directories, the default libexec directory will look for hadoop-yarn/bin/../libexec, but libexec is actually under the hadoop directory. This causes an error when obtaining the Spark task status with the yarn application status command, leading to import job failure.