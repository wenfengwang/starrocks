...
| 标签       | 导入作业的标识，数据库内唯一。如果未指定，StarRocks 会自动为作业生成一个 Label。建议您指定 Label。否则，如果当前导入作业因网络错误无法返回结果，您将无法得知该导入操作是否成功。如果指定了 Label，可以通过 SQL 命令 `SHOW LOAD WHERE label="label";` 查看作业结果。 |
| 值         | 通过使用 VALUES 语法插入一条或者多条数据，多条数据用逗号（,）分隔。 |

## 通过使用 INSERT INTO SELECT 语句导入数据

您可以通过使用 INSERT INTO SELECT 语句将源表中的数据导入至目标表中。INSERT INTO SELECT 将源表中的数据进行 ETL 转换之后，导入到 StarRocks 内表中。源表可以是一张或多张内部表或者外部表，甚至云存储或 HDFS 中的数据文件。目标表必须是 StarRocks 的内表。执行该语句之后，系统将选择语句的结果导入目标表。详细使用方式，请参考 [INSERT](../sql-reference/sql-statements/data-manipulation/INSERT.md)。详细参数信息，请参考 [INSERT 参数](../sql-reference/sql-statements/data-manipulation/INSERT.md#参数说明)。
...
- 以下示例以 `insert_load_wikipedia_1` 为标签将源表中的数据导入至目标表中。

```SQL
INSERT INTO insert_wiki_edit
WITH 标签 insert_load_wikipedia_1
SELECT * FROM source_wiki_edit;
```

- 以下示例以 `insert_load_wikipedia_2` 为标签将源表中的数据导入至目标表的 `p06` 和 `p12` 分区中。如果不指定目标分区，数据将会导入全表；如果指定目标分区，数据只会导入指定的分区。

```SQL
INSERT INTO insert_wiki_edit PARTITION(p06, p12)
WITH 标签 insert_load_wikipedia_2
SELECT * FROM source_wiki_edit;
```
...
- 以下示例以 `insert_load_wikipedia_3` 为标签将源表中 `event_time` 和 `channel` 列的数据导入至目标表的对应列中。未被导入的列将被赋予默认值。

```SQL
INSERT INTO insert_wiki_edit
WITH 标签 insert_load_wikipedia_3 
(
    event_time, 
    channel
)
...
### 通过使用 INSERT INTO SELECT 和表函数 FILES() 导入外部数据文件
...
## 通过使用 INSERT OVERWRITE VALUES 语句覆盖写入数据

您可以通过使用 INSERT OVERWRITE VALUES 语句向指定的表中覆盖写入数据。此导入方式中，多条数据用逗号（,）分隔。详细使用方式，请参考 [INSERT](../sql-reference/sql-statements/data-manipulation/INSERT.md)。详细参数信息，请参考 [INSERT 参数说明](../sql-reference/sql-statements/data-manipulation/INSERT.md#参数说明)。
...
将 `insert_load_wikipedia_ow` 作为 Label，覆盖写入两条数据到源表 `source_wiki_edit` 中。
...
使用 INSERT OVERWRITE SELECT 语句将源表数据覆盖写入目标表。INSERT OVERWRITE SELECT 将进行 ETL 转换后的源表数据覆盖写入 StarRocks 内部表。源表可为一张或多张内部表或外部表，目标表必须为 StarRocks 内部表。执行该语句后，系统将使用 SELECT 语句结果覆盖目标表数据。有关详细使用方式，请参考 [INSERT](../sql-reference/sql-statements/data-manipulation/INSERT.md)。有关详细参数信息，请参考 [INSERT 参数](../sql-reference/sql-statements/data-manipulation/INSERT.md#参数说明)。

> 说明
>
> 下面示例仅展示导入内部表数据，操作过程与导入外部表数据相同，因此不再重复演示导入外部表数据过程。

- 以 `insert_load_wikipedia_ow_1` 作为 Label，将源表中的数据覆盖写入目标表。
...
- 以 `insert_load_wikipedia_ow_2` 作为 Label，将源表中的数据覆盖写入目标表的 `p06` 和 `p12` 分区。如果不指定目标分区，数据将覆盖整个表；如果指定目标分区，数据将只覆盖指定的分区。
...
如果清空 `p06` 和 `p12` 分区，则不会查询到先前覆盖写入到相应分区的数据。
...
- 以 `insert_load_wikipedia_ow_3` 作为 Label，将源表中的 `event_time` 和 `channel` 列的数据覆盖写入目标表的对应列。未被导入的列将赋予默认值。
...
生成列（Generated Columns）是一种特殊列，其值会根据列定义中的表达式自动计算得出。此外，您无法直接写入或更新生成列的值。生成列在涉及对表达式的计算时非常有用，例如查询 JSON 类型的字段或针对 ARRAY 数据进行计算。在数据导入时，StarRocks将计算表达式，然后将结果存储在生成列中，从而避免在查询过程中计算表达式，提高查询性能。

您可以使用 INSERT 语句将数据导入包含生成列的表中。

下面示例创建表 `insert_generated_columns` 并插入一行数据。该表包含两个生成列：`avg_array` 和 `get_string`。`avg_array` 计算 `data_array` 中 ARRAY 类型数据的平均值，`get_string` 从 `data_json` 中提取 JSON 路径为 `a` 的字符串。
