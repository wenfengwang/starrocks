...
DataX 导入支持多种数据源，您可以参考 [DataX - 支持的数据通道](https://github.com/alibaba/DataX#support-data-channels) 了解详情。
...
如果源数据库与目标数据库时区不同，您需要在命令行中添加 `-Duser.timezone=GMTxxx` 选项来设置源数据库的时区信息。

例如，如果源数据库使用 UTC 时区，则启动任务时需要添加参数 `-Duser.timezone=GMT+0`。

## 查看导入任务状态

DataX 的导入是基于封装好的 Stream Load 来实现的，您可以在 `datax/log/$date/` 目录下查找相应的导入作业日志。日志文件的名字包含了使用的 JSON 文件名以及任务开始的时间（小时、分钟、秒），例如：**t_datax_job_job_json-20_52_19.196.log**。

- 如果日志中有生成 "http://fe_ip:fe_http_port/api/db_name/tbl_name/_stream_load" 的记录，那么表明 Stream Load 作业已经成功触发了。作业成功触发之后，您可以参考 [Stream Load 返回值](../sql-reference/sql-statements/data-manipulation/STREAM_LOAD.md#返回值) 来查看任务的情况。
- 如果日志中没有上述信息，请根据错误信息进行问题排查，或者到 [DataX 社区问题](https://github.com/alibaba/DataX/issues) 中寻找解决方案。

> 注意
>
> 当使用 DataX 进行导入的时候，如果存在不符合目标表格式的数据，且 StarRocks 中的字段允许 NULL 值，则不合法的数据会被转换为 NULL 后正常导入。如果字段不允许 NULL 值，同时任务中没有配置容错率（max_filter_ratio），那么任务会因错误而中断，并停止导入作业。

## 取消或停止导入任务

您可以通过终止 DataX 的 Python 进程来停止导入任务。
