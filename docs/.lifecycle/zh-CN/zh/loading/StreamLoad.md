...
手动创建主键模型表 `table2`，包含 `id` 和 `city` 两列，主键为 `id` 列，相关语句如下：
...
使用以下语句将 `example2.json` 文件中的数据导入到 `table2` 表中：
...
`example2.json` 文件中包含 `name` 和 `code` 两个键，与 `table2` 表中的列之间的对应关系如下图所示。
...
- 并发访问单点的 Broker 容易成为瓶颈，并且并发越高反而越容易造成超时、OOM 等问题。您可以通过设置 `pipeline_dop`（参见[调整查询并发度](../administration/Query_management.md#调整查询并发度)）来限制 Broker Load 的并行度，对于单个 Broker 的访问并行度建议设置小于 `16`。
...
- 将NAS作为本地文件系统，然后按照之前“[使用 Broker Load 从本地导入](#使用-broker-load-从本地导入)”中介绍的方法来执行导入。

- 【推荐】将NAS作为云存储设备，这样就无需部署Broker，可以通过Broker Load实现导入。

本小节主要介绍这种方法。具体操作步骤如下：

1. 将NAS挂载到所有的BE、FE节点，并确保所有节点的挂载路径完全一致。这样，所有BE可以像访问BE自己的本地文件一样访问NAS。
...
2. 使用Broker Load导入数据。
   ...
   - `LABEL`：导入作业的标签，字符串类型，可用于查询导入作业的状态。
   - `LOAD`声明：包括源数据文件所在的URI、源数据文件的格式以及目标表的名称等作业描述信息。注意，`DATA INFILE` 这里用于指定挂载路径，如上面示例所示，`file:///`为前缀，`/home/disk1/sr`为挂载路径。
   - `BROKER`：无需指定。
   - `PROPERTIES`：用于指定超时时间等可选的作业属性。

   有关详细的语法和参数说明，请参见[BROKER LOAD](../sql-reference/sql-statements/data-manipulation/BROKER_LOAD.md)。

提交导入作业后，您可以查看导入进度，或者取消导入作业。具体操作请参见本文“[查看 Broker Load 导入进度](#查看-broker-load-导入进度)”和“[取消 Broker Load 作业](#取消-broker-load-作业)”中的介绍。