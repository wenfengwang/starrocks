...
```json
      "Message": "标签 [streamload_txn_example1_table1] 已经被使用。"
  }
  ```
...
> **说明**
>
> 调用 `/api/transaction/load` 接口时，必须通过 `-T <文件路径>` 指定数据文件所在的路径。
...
> **说明**
>
> 上述示例中，由于数据文件 `example1.csv` 中使用的列分隔符为逗号 (`,`)，而不是 StarRocks 默认的列分隔符 (`\t`)，因此在调用 `/api/transaction/load` 接口时必须通过 `"column_separator: <列分隔符>"`  指定列分隔符为逗号 (`,`)。
